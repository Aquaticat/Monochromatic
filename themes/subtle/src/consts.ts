...
const langMappings = [
  { codes: ['en', 'en-US', 'en-CA', 'en-UK'], path: '' },
  { codes: ['zh', 'zh-CN', 'zh-TW'], path: 'zh' },
];

export const I18N = new Map(
  langMappings.flatMap((langMapping) => langMapping.codes.map((key) => [key, langMapping.path])),
);

export const LANG_PATHS = langMappings.map((langMapping) => langMapping.path);
