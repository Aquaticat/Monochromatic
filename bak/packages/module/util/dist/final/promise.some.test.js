...
var l = getLogger(["m", "promise.some"]);
// @__NO_SIDE_EFFECTS__
...
// src/boolean.equal.ts
var l2 = getLogger2(["m", "boolean.equal"]);
// @__NO_SIDE_EFFECTS__
function isPrimitive(value) {
  ...
  l2.debug`value ${value} is not a primitive`;
  return false;
}
__name(isPrimitive, "isPrimitive");
// @__NO_SIDE_EFFECTS__
function equal(a, b) {
  l2.debug`a: ${String(a)}, b: ${String(b)}`;
  if (Object.is(a, b)) {
    return true;
  }
  l2.debug`ref/primitive eq failed on a: ${String(a)}, b: ${String(b)}`;
  if (/* @__PURE__ */ isPrimitive(a) || /* @__PURE__ */ isPrimitive(b)) {
    l2.debug`At least one of a: ${String(a)} and b: ${String(b)} is primitive`;
    return false;
  }
  l2.debug`non-primitives a: ${a}, b: ${b}`;
  ...
  if (typeof a === "function") {
    if (typeof b !== "function") {
      l2.info`Is it intentional trying to compare a function a: ${a} to not function b: ${b}?`;
      return false;
    }
    l2.warn`cannot compare two functions accurately due to the inherent unpredictability of functions.
  ...
  if (Array.isArray(a)) {
    if (!Array.isArray(b)) {
      l2.info`Is it intentional trying to compare an array a: ${a} to not array b: ${b}?`;
      return false;
    }
    l2.debug`arrays a: ${a} b: ${b}`;
  ...
  if (typeof a === "object") {
    if (typeof b !== "object") {
      l2.warn`b is not a primitive, not a Promise, not a function, not an array, not an object.
        What is b: ${b}?`;
      return false;
    }
    l2.debug`objects a: ${a} b: ${b}`;
    ...
    if (isObjectDate(a)) {
      if (!isObjectDate(b)) {
        l2.info`Is it intentional trying to compare a Date a: ${a} to not a Date b: ${b}?`;
        return false;
      }
      l2.debug`dates a: ${a} b: ${b}`;
    ...
    if (aPrototype === "[object Boolean]") {
      l2.warn`The use of Boolean() the object wrapper is greatly discouraged. Found a: ${a}`;
      if (bPrototype !== "[object Boolean]") {
        l2.info`Is it intentional trying to compare a Boolean wrapped a: ${a} to not a Boolean wrapped b: ${b}?`;
        return false;
      }
      l2.debug`Boolean wraps a: ${a} b: ${b}`;
    ...
    if (isError(a)) {
      if (!isError(b)) {
        l2.info`Is it intentional trying to compare a error a: ${a} to not error b: ${b}?`;
        return false;
      }
      l2.debug`Errors a: ${a} b: ${b}`;
      if (a.message !== b.message) {
        return false;
      }
      if (a.name !== b.name) {
        l2.info`Is it intentional,
    ...
    if (isGenerator(a)) {
      if (!isGenerator(b)) {
        l2.info`Is it intentional trying to compare a Generator a: ${a} to not a Generator b: ${b}?`;
        return false;
      }
      l2.info`comparing two Generators would only succeed
        if both of them never takes any parameters.`;
      return /* @__PURE__ */ equal(arrayFrom(a), arrayFrom(b));
    }
    if (isRegexp(a)) {
      if (!isRegexp(b)) {
        l2.info`Is it intentional trying to compare a regex a: ${a} to not a regex b: ${b}?`;
        return false;
      }
      l2.debug`regexps a: ${a} b: ${b}`;
      return `${a}` === `${b}`;
    }
    if (isMap(a)) {
      if (!isMap(b)) {
        l2.info`Is it intentional trying to compare a map a: ${a} to not a map b: ${b}?`;
        return false;
      }
      l2.debug`maps a: ${a} b: ${b}`;
      if (a.size !== b.size) {
        return false;
      }
      if (a.size === 0) {
    ...
    if (isSet(a)) {
      if (!isSet(b)) {
        l2.info`Is it intentional trying to compare a set a: ${a} to not a set b: ${b}?`;
        return false;
      }
      l2.debug`sets a: ${a} b: ${b}`;
      if (a.size !== b.size) {
        return false;
      }
      if (a.size === 0) {
    ...
    if (isObject(a)) {
      l2.info`Comparing two objects cannot rule out Proxy objects. See https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Proxy#constructor`;
      if (!isObject(b)) {
        l2.info`Is it intentional trying to compare an object Object a: ${a} to not an object Object b: ${b}?`;
        return false;
      }
      l2.debug`Objects a: ${a} b: ${b}`;
...
var l3 = getLogger3(["t", testedFileName]);
// @__NO_SIDE_EFFECTS__
async function suite(name, testOrSuites, options) {
  l3.debug`suite ${name} started: ${testOrSuites.length} tests or suites`;
  ...
  if (options) {
    ...
    if (Object.hasOwn(options, "todo")) {
      l3.info`suite ${name} finished: ${JSON.stringify(result)}`;
  ...
  l3.debug`suite ${name} finished: ${JSON.stringify(result)}`;
  return { name, result };
}
__name(suite, "suite");
// @__NO_SIDE_EFFECTS__
async function test(name, callback, options) {
  ...
  if (options) {
    if (Object.hasOwn(options, "skip")) {
      l3.warn`${name} skipped: ${options.skip}`;
      return { name, skip: options.skip };
    }
    if (Object.hasOwn(options, "timeLimit")) {
      timeLimit = typeof options.timeLimit === "number" ? options.timeLimit : await (async () => {
        ...
        try {
          ...
          l3.info`timeLimit fn threw ${timeLimitFnError}
      ...
      l3.debug`${name} timeLimit: ${timeLimit}ms`;
    }
    if (Object.hasOwn(options, "todo")) {
      l3.info`${name} started: ${callback} with todo: ${options.todo}`;
      try {
        ...
        l3.debug`${name} took: ${took}ms`;
        if (result !== void 0 && result !== null) {
          if (took > timeLimit) {
            const tooLongPercentage = 100 * (took - timeLimit) / timeLimit;
            l3.error`${name} with todo: ${options.todo} took: ${took}ms, ${took - timeLimit}ms longer than timeLimit: ${timeLimit}ms, that's ${tooLongPercentage}% too long. finished: ${result}`;
            return {
              name,
              todo: options.todo,
              result,
          ...
          l3.warn`${name} with todo: ${options.todo} finished: ${result}`;
          return { name, todo: options.todo, result };
        }
        if (took > timeLimit) {
          const tooLongPercentage = Math.round(100 * (took - timeLimit) / timeLimit);
          l3.error`${name} with todo: ${options.todo} took: ${Math.round(took)}ms, ${Math.round(took - timeLimit)}ms longer than timeLimit: ${timeLimit}ms, that's ${tooLongPercentage}% too long.`;
          return { name, todo: options.todo, tooLongPercentage };
        }
        l3.warn`${name} with todo: ${options.todo} finished`;
        return { name, todo: options.todo };
      } catch (e) {
        l3.error`${name} with todo: ${options.todo} errored: ${e}`;
        return { name, todo: options.todo, result: e };
      }
    }
  }
  l3.debug`${name} started: ${String(callback).slice(0, 64)}`;
  try {
    const result = await callback();
    if (took > timeLimit) {
      const tooLongPercentage = Math.round(100 * (took - timeLimit) / timeLimit);
    ...
    if (result !== void 0 && result !== null) {
      l3.info`${name} finished: ${result}`;
      return { name, result };
    }
    l3.debug`${name} finished`;
    return name;
  } catch (callbackError) {
...
suite("array", [
  suite("sync callback", [
    test("first", async () => {
      assertTrue(
        await somePromises((input) => typeof input === "number", [
          (async () => 1)(),
          (async () => "")()
        ])
      );
...
//# sourceMappingURL=promise.some.test.js.map
