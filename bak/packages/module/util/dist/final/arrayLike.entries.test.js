...
suite("entriesArrayLikeAsync", [
  suite("Array", [
    ...
    test("holey", async () => {
      ...
      assert([[0, 1], [1, void 0], [2, 2]], entries);
    })
  ]),
  suite("Iterable", [
    test("empty", async () => {
    ...
    test("one", async () => {
      const entries = [];
      for await (const entry of entriesArrayLikeAsync(function* () {
        yield 1;
      }())) {
        entries.push(entry);
      }
  ...
  suite("AsyncIterable", [
    ...
    test("one", async () => {
      const entries = [];
      for await (const entry of entriesArrayLikeAsync(async function* () {
        yield 1;
      }())) {
        entries.push(entry);
      }
...
suite("entriesArrayLike", [
  ...
  suite("Iterable", [
    ...
    test("one", async () => {
      const entries = [];
      for await (const entry of entriesArrayLike(function* () {
        yield 1;
      }())) {
        entries.push(entry);
      }
...
//# sourceMappingURL=arrayLike.entries.test.js.map
