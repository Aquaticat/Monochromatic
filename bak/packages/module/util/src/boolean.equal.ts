...
import { isObjectDate } from './numeric.ts';
import {
  isPromise,
  type NotPromise,
} from './promise.ts';
...
const l = getLogger(['m', 'boolean.equal']);
...
/* @__NO_SIDE_EFFECTS__ */ export function equal(a: NotPromise, b: NotPromise): boolean {
  ...
  if (typeof a === 'object') {
    ...
    if (isGenerator(a)) {
      ...
      return equal(arrayFrom(a), arrayFrom(b));
    }
  ...
  b: any): Promise<boolean>
{
  ...
  if (
    ...
    if both of them never takes any parameters.`;
    return equal(await arrayFromAsync(a), await arrayFromAsync(b));
  }
