...
`packages/cli/git/src/rules/require-root.ts` independently used `find-up` with `type: 'both'` and made the same
assumption.
...
- `packages/cli/git/src/rules/require-root.unit.test.ts` expected invalid ancestors to pass through to real Git.
