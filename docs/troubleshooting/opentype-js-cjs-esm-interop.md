...
```ts
// oxlint-disable-next-line import/no-named-as-default-member -- opentype.js's UMD bundle defeats cjs-module-lexer's named-export detection (see docs/troubleshooting/opentype-js-cjs-esm-interop.md); the default import's .Path/.Glyph/.Font members are the only ones that work under Node's CJS/ESM interop
