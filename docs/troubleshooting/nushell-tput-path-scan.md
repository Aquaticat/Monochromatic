...
- Put `/usr/bin` (and `/bin`) early in PATH, or trim the injected workspace-bin entries.
  This makes the `tput` lookup hit on the first or second directory, collapsing ~564 failed `execve` to ~0.
  Tradeoff: changes command-resolution precedence; workspace-bin shims would no longer shadow system tools
  if moved after system dirs, so reorder deliberately rather than blindly prepending `/usr/bin`.
  Trimming is safer than reordering where shadowing matters.
