...
workspace root.
`packages/cli/git/node_modules/@monochromatic-dev` did not contain `config-typescript`.
...
`packages/cli/git/node_modules/@monochromatic-dev/config-typescript` existed.
...
`packages/cli/git/mise.toml` makes `pack:npm` perform a forced package-filtered install with
