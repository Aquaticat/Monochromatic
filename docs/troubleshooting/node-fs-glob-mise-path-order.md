...
The operating system chooses the directory-entry order, so a regenerated file can
...
The current checkout does not show a run-to-run flake: twenty isolated
...
`file-enforcer.config.ts:616-621` collects Node's async iterator, maps each match
straight to TOML, and joins the array without normalizing it:
...
iterates it by index, with no intervening sort:
...
Node's own directory API documentation says this source order is not stable:
...
In the isolated evaluation, its first package paths appeared in the same sequence
as `_.path`, after conversion to absolute paths.
...
Use a disposable worktree, then create directories in a deliberately non-lexical
...
- Works cleanly: a literal `_.path` list stays in its declared order;
  `mise env --json` preserved that sequence in each isolated evaluation.
- Works cleanly: sorting a copy of the fixture output yields
  `amber`, `birch`, `quartz`, then `zebra`.
- Fails determinism: the raw fixture output was `zebra`, `quartz`, `birch`, then
  `amber`, which differs from the normalized order.
- Fails determinism: the production generator serializes raw results, so its
...
The semantic tradeoff is intentional: it changes precedence for two workspace
packages that expose the same executable, because command lookup uses the first
...
- Re-running the generator on one machine does not prove portability.
  ...
  entries did not change, but Node explicitly leaves their order to the filesystem.
...
- Filing this as a Node.js defect would misidentify the cause.
  Node preserves native directory enumeration; this repository decides to give
...
1. **Is this upstream's fault?** No. Node exposes underlying directory order;
   the repository omits the normalization required for a stable generated file.
2. **Can upstream fix it?** Technically yes, but a global sort would be an
   upstream behavior change rather than a repair of a Node defect.
3. **Does upstream support this use case?** No documented Node guarantee covers
   deterministic glob ordering for generated configuration.
4. **Would upstream welcome a contribution?** `nodejs/node@v26.5.0:CONTRIBUTING.md`
   welcomes contributions, but it requires explicit authorization before an
   external automation interacts with the project.
   No such authorization exists for this diagnosis.
5. **Will upstream likely fix it?** No basis exists to expect a change to
   documented native-order behavior.
6. **Was a compatible upstream fix prototyped?** No. The first constraint fails,
