...
A VM provides a complete, bootable, self-contained development environment
that runs on any hypervisor.
Unlike dotfile managers or setup scripts that assume a compatible host OS,
a VM image carries the entire OS, packages, and configuration.
The portability boundary is the hypervisor, not the distro.

## Why immutable

Immutability is not just for long-lived systems.
Config drift happens in hours during a debugging session:
a quick `dnf install strace`, a manual edit to `/etc/something`,
...
install a package, staged for next boot, atomic, reversible.
...
**Bazzite**: gaming-focused desktop (Steam, MangoHud, gamescope, controller drivers).
...
**openSUSE Kalpa**: immutable KDE on Tumbleweed (rolling), which sounds ideal.
In practice: still alpha after years, single maintainer ("sfalken"),
...
**openSUSE MicroOS (server)**: mature, immutable, transactional-update with btrfs snapshots.
...
libvirt, KVM, virsh, virt-install, cockpit-machines, podman, ZFS, tailscale.
...
Starting from a desktop image (Kinoite, Bazzite) means inheriting
...
The result is a smaller, faster, more predictable image
...
For single-file configs (ghostty, contour, xremap, git, mise, crush),
separate repos add overhead with no benefit:
separate clone commands, separate git history, no atomic cross-tool commits.

One `Aquaticat/dotfiles` repo with subdirectories.
nvim may stay separate due to its complexity (plugins, Lua config, lazy.nvim).

## Why KDE config is not portable

`~/.config/` contains ~40 KDE-generated files:
`plasmarc`, `kwinrc`, `kdeglobals`, `kglobalshortcutsrc`, etc.
These encode machine-specific state:
display IDs, GPU device paths, monitor geometry and arrangement, device UUIDs.

Porting these between machines (host to VM, VM to VM)
causes more breakage than it prevents.
KDE regenerates sane defaults on fresh profiles.
The few settings worth preserving
(custom global shortcuts, theme preferences)
