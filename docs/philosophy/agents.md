...
#### Command execution conventions: cwd resets in worktrees (WCD)

The Bash tool's working directory is not stable across calls when a session operates inside a linked git worktree. It silently resets to the primary checkout between commands, most visibly after a background task completes or a notification arrives, and even after an `EnterWorktree` reports the session moved. An unprefixed `mise`, `pnpm`, `git`, or `bun` then runs against the main checkout, so a build or lint quietly targets the wrong tree and reports success for code that was never touched. `Read`, `Write`, and `Edit` take absolute paths and are immune, so file edits land correctly while shell commands drift, which makes the mismatch hard to notice. Prepending `cd <worktree-abs-path> &&` to every Bash command is the fix; this is the one place where the usual "avoid `cd`" and "never prepend `cd` to a git command" guidance is overridden, because the working directory is unreliable rather than known. The cue is being about to run any shell command in a worktree without a leading `cd` to that worktree's absolute path.
...
- 2026-06-06: Merged EC2 and EC3 into EC1 under 100 words, preserving concurrent-worktree handling,
  no-revert/no-move-aside behavior, and ask-before-changing blocked unrelated edits. Merged EC5 into EC4 so unsupported
  tool behavior stays with non-functional implementation guidance.
