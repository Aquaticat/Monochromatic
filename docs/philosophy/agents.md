...
A shell command's working directory is not reliably stable across calls, and the instability is broader than the Bash tool alone: any shell-running tool can have its cwd reset between commands. It was first observed inside linked git worktrees, where the cwd silently resets to the primary checkout between commands, most visibly after a background task completes or a notification arrives, and even after an `EnterWorktree` reports the session moved. An unprefixed `mise`, `pnpm`, `git`, or `bun` then runs against the wrong tree, so a build or lint quietly targets the wrong directory and reports success for code that was never touched. `Read`, `Write`, and `Edit` take absolute paths and are immune, so file edits land correctly while shell commands drift, which makes the mismatch hard to notice. The preservation behavior turned out to be more unstable than the worktree-only framing suggested, so the rule is now unconditional: pin the target dir explicitly on every shell command, by prepending `cd <abs-path> &&` or by passing absolute paths, rather than trusting an inherited cwd. In a worktree the pinned dir is the worktree absolute path. This is the one place where the usual "avoid `cd`" and "never prepend `cd` to a git command" guidance is overridden, because the working directory is unreliable rather than known. The cue is being about to run any shell command without explicitly pinning its target directory.
