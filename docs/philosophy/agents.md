...
md alone.
This philosophy doc and every other doc under `docs/` stay normal prose:
...
Do not add per-rule "(see docs/philosophy/agents.
md)" / "(rationale in docs/philosophy/agents.
...
When the review touches `cli-git`'s linked-worktree guard,
 account for the baked-in tool-cache allowlist (`DEFAULT_ALLOWED_WORKTREE_DIRS` in `packages/cli/git/src/allowed-worktree-dirs.ts`,
...
- JSR (`.out-of-scope/jsr.md`):
   the workspace consumes no JSR-hosted packages (`docs/philosophy/tool-choices.md` covers tool selection),
...
parity audit,
 ...
 and the
`docs/decisions/<project>.md` maintenance rule.
