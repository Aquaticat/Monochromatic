...
Root `.claude/` is ignored because it can contain machine-local permissions, hooks, subagents, skills,
and agent memory.
Commit Claude Code project configuration only when it is intentionally shared infrastructure.
Force-add reviewed shared config when needed, and keep permission-bearing files minimal.
