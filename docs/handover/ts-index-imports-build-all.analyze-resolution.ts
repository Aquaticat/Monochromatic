...
type Pkg = { name: string; dir: string; exports: unknown };
...
for (const rel of new Glob('packages/**/package.json').scanSync(root)) {
  ...
  if (typeof json.name === 'string' && json.name.startsWith(SCOPE)) {
    pkgByName.set(json.name, { name: json.name, dir: dirname(file), exports: json.exports });
  }
}
...
function condTarget(v: unknown): string | null {
  ...
  if (v && typeof v === 'object') {
    ...
    for (const k of ['default', 'node', 'import', 'types']) {
      if (typeof o[k] === 'string') return o[k] as string;
    }
  }
  return null;
}
function resolveExport(exportsObj: unknown, subkey: string): string | null {
  ...
  for (const [pat, val] of Object.entries(map)) {
    ...
    if (subkey.startsWith(pre) && subkey.endsWith(post) && subkey.length >= pre.length + post.length) {
      const star = subkey.slice(pre.length, subkey.length - post.length);
      const tv = condTarget(val);
      return tv === null ? null : tv.replace('*', star);
    }
  }
  return null;
}
...
function subkeyFor(unscoped: string, specifier: string): string {
...
const re = new RegExp(
  `(?<![A-Za-z0-9_$])(?:from|import)\\s*\\(?\\s*['"](${SCOPE.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[^'"]+)['"]`,
  'g',
);

type Finding = { file: string; importer: string; specifier: string; subkey: string; resolved: string | null; bucket: string };
const findings: Finding[] = [];
const names = [...pkgByName.keys()].map(n => n.slice(SCOPE.length)).sort((a, b) => b.length - a.length);
...
for (const rel of new Glob('packages/**/*.{ts,tsx,mts,cts}').scanSync(root)) {
  ...
  for (const m of text.matchAll(re)) {
    ...
    if (!unscoped) { findings.push({ file: rel, importer: importer ?? '?', specifier, subkey: '?', resolved: null, bucket: 'unknown-target' }); continue; }
    const targetName = SCOPE + unscoped;
    const isSelf = importer === targetName;
    const exempt = isExemptTarget(unscoped);
    const subkey = subkeyFor(unscoped, specifier);
    const resolved = resolveExport(pkgByName.get(targetName)!.exports, subkey);
    let bucket: string;
    ...
    findings.push({ file: rel, importer: importer ?? '?', specifier, subkey, resolved, bucket });
  }
}

const byBucket = new Map<string, Finding[]>();
for (const f of findings) (byBucket.get(f.bucket) ?? byBucket.set(f.bucket, []).get(f.bucket)!).push(f);

console.log(`total @monochromatic-dev import-context specifiers: ${findings.length}\n`);
for (const [bucket, fs] of [...byBucket.entries()].sort((a, b) => b[1].length - a[1].length)) {
