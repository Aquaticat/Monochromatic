...
`docs/troubleshooting/slint-embedded-mcp-server.md`;
...
- Where:
  ...
  (guards the two-way `value <=> ...` bindings;
   see `docs/troubleshooting/slint-slider-binding-breaks-on-input.md`).
...
Do not install niri with `mise install cargo:niri@latest`:
 ...
 not a runnable
compositor.
 Details and the source trace live in `docs/troubleshooting/niri-mise-cargo-install.md`.
