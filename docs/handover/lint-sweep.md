...
- Cap concurrency at about 5 children. Past roughly 12 lingering `ghostty -e claude` terminals, new sessions hang at startup (Claude exits before initializing; the terminal becomes an empty husk). If new children stop producing sessions, the user must close stale terminal windows (killing them needs explicit user authorization; the auto-mode classifier blocks mass-kills).
