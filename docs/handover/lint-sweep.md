...
- Empirical burst-size finding (2026-05-25 sweep): leaf waves of 7 children launched on top of a small base succeeded cleanly (waves L1-wave2). A wave of 9 launched in one burst on top of 7 existing terminals (16 total) reproduced the stall/husk: all 9 spawned (spawn-result JSON created at `status:running`) but ~5 husked (terminal open, no claude process) and ~4 stayed alive but stalled with zero progress, zero source edits, and zero commits for 45+ minutes. No git lock, no resource error surfaced; cause undiagnosed (could be API overload at simultaneous spawn, terminal count, or the same one-off). Practical rule: keep spawn BURSTS small (about 5 at a time) and total terminals well under ~12, re-checking live count between bursts, even though steady-state 16 sessions are reported fine. The failure correlates with large simultaneous spawn bursts, not steady-state count.
