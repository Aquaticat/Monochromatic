...
type ImportStmt = { isType: boolean; specifier: string };
function parseImports(text: string): ImportStmt[] {
  ...
  while (i < lines.length) {
    ...
    if (trimmed.startsWith('import ') || trimmed.startsWith('import{') || trimmed === 'import' || trimmed.startsWith('import type')) {
      ...
      if (m) out.push({ isType, specifier: m[1] });
      i = j + 1;
      continue;
    }
    i += 1;
...
const mergeFiles: { file: string; valueCount: number; typeCount: number }[] = [];
for (const rel of new Glob('packages/**/*.{ts,tsx,mts}').scanSync(root)) {
  ...
  if (valueCount + typeCount > 1) mergeFiles.push({ file: rel, valueCount, typeCount });
}
