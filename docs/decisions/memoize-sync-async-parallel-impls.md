...
`@monochromatic-dev/module-memoize` ships two wrappers, `memoize` (`src/memoize.ts`)
and `memoizeAsync` (`src/memoize-async.ts`).
They share structure: destructuring the function and key derivation,
creating a default per-instance LRU store with a random `storeId`,
building the cache key, the get-then-compute-then-set core,
...
parameterized over the store primitives, with sync and async as thin adapters.
We considered it and decided against merging; the two stay parallel.

The reasons a future explorer needs, so this is not re-suggested:
...
- It matches the deliberate split already in the substrate.
  ...
  implementations rather than abstracting over both, and memoize sits directly on that contract.
...
- Merging forces an abstraction over `Promisable` (the sync-or-async return),
  which kv-store intentionally avoids. The cost is a layer of conditional-async indirection
...
- The part that genuinely differs is the get-then-compute-then-set core plus the async
  in-flight deduplication (the in-flight `Map`, the `using`/`Disposable` cleanup,
  and the reject-clears-in-flight path). That is exactly the part that resists clean unification,
