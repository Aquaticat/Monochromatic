...
- Valibot is already in the workspace catalog
  (`PHILOSOPHY.tool-choices.md:27`:
   ...
   Standard
  Schema-compatible).
   It is used in `packages/pi/advisor/` and
...
Use `@optique/valibot` **selectively**:
...
`https://registry.npmjs.org/@optique/valibot/latest`,
 and
...
- Maintainer:
  ...
  (3315 contributions vs second contributor's 2).
   Same author as
  LogTape and Fedify;
   bus factor identical to `@optique/core` (already
  accepted).
- Release:
   1.1.0 at HEAD,
  ...
  2025-08-19.
- Weekly downloads:
   3 078 (`@optique/valibot`) vs 15 832
  (`@optique/core`),
   16 398 (`@optique/run`).
