...
- Severity: soundness. The hardened path drops a real match.
- Affected: default vs `hardened(true)`.
- Discovery: the HARDDIFF oracle over the adversarial corpus (4 distinct
  triggers).
...
The default and hardened engines must accept the same language; hardening only
substitutes an O(N*S) forward scan to bound worst-case time. On `~(\A|\n+){2}`
over `"\n\n"` the default driver reports matches at `1:1` and `2:2` while the
hardened driver reports only `2:2`, dropping the `1:1` match. One side is wrong;
the disagreement alone is the bug.

Other distinct HARDDIFF triggers in the corpus: `^{3}([\w]{2,}0{3}|_?)`,
`~([a-c]+|\A{3}\s?)+`, and `[\x00-\x10]*(Z){2,}|(?!_{0}\A{3} {0,2}){3}`. All
...
reported fixed. The 06-04 triggers (`~(_a+)`, `(?<=^)~(0+)`) are now consistent
(verified), but the disagreement still occurs on new triggers, so the underlying
two-driver divergence is not eliminated.

## Source pointer

The hardened driver is the `find_all_dfa` path; the default driver is the
reverse-collect / prefix path. The divergence is in how each handles a zero-width
