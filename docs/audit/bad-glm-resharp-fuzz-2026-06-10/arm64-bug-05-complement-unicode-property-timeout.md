...
- Phase: compile time.
- Severity: complement `~(\p{L}iv>_*)` combined with trailing literal causes
...
derivative blowup as ARM64-BUG-1. The complement negation requires computing
the full character class complement of the Unicode Letter set, which
materializes a very large minterm set. The `attempt_rw_union_2` rewrite
