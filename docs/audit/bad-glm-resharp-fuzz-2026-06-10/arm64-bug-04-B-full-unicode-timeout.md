...
- Phase: compile time.
- Severity: `\B` compiles in ~831µs on x86_64 but exceeds 10s timeout under
...
character class. Under `UnicodeMode::Full`, the character class expands to the
full Unicode Letter set (thousands of codepoints). On ARM64, the derivative
construction for this large class under the `Kind::Inter` branch at
`resharp-algebra/src/lib.rs:1370` recurses deeply, same root as ARM64-BUG-1.

On x86_64, the same pattern compiles in ~831µs; on ARM64 without ASAN it
compiles in ~817µs. The ASAN instrumentation adds ~12x overhead on ARM64,
pushing the compile time past the 10-second timeout. This suggests the raw
