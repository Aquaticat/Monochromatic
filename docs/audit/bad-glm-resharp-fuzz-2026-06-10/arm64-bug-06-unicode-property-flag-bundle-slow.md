...
- Type: performance, compile-time slow.
- Phase: compile time.
- Severity: slow-unit (>1s compile) for `\p{L}` followed by mixed bytes
...
`dot_matches_new_line(true)` flag bundle, the pattern parser may interpret
more of the input as valid regex tokens (whitespace is ignored, case is
folded). This causes the Unicode property class `\p{L}` to be followed by
a longer effective pattern, increasing the derivative computation cost.
