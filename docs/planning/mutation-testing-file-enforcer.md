...
manifests, root `.pnpmfile.mjs` and its imported policy JSON, workspace package manifests, and the
mutation-test runtime source, not heavyweight artifacts such as Rust `target/`, VM `output/`, fuzz corpora,
`node_modules`, or `dist`. The runtime image tag includes the lockfile hash, platform, and a hash of
mutation-test runtime source, `runtime/Containerfile`, root `mise.toml`, root `.pnpmfile.mjs`, its imported
policy JSON, root workspace manifests, and workspace package metadata so baked entrypoint and
...
4.  Install mise with the official Fedora/COPR path, then use mise to install the repo-pinned `node` and
    ...
    and the missing-tool warning is acceptable. Package tests selected by mutation runs must execute fixture
    config files through Node instead of spawning Bun.
