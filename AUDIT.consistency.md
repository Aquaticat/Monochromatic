...
**Mixed / unique:**
`# Tsconfig-monochromatic` (title-case with slug), `# @monochromatic-dev/build-css` (scope + different slug than directory `build-tool/css`),
`# cli-vm (mvm)` (parenthetical alias), `# clauce-code-plugins-terminal-title` (typo: "clauce")
...
Packages with **blank/empty** README first lines: `config/stylelint`, `dev-script/backup-path`.
...
(dev-scripts, claude-code-plugins, single-use configs):
`dev-script/file-enforcer`, `dev-script/backup-path`, `dev-script/inference-canary`,
`dev-script/inference-canary-viewer`, `dev-script/catalog-tighten`,
`claude-code-plugins/terminal-title`, `claude-code-plugins/hook-types`,
`claude-code-plugins/session-start-housekeeping`, `claude-code-plugins/stop-reminders`,
`claude-code-plugins/hook-utils`, `claude-code-plugins/bash-output-filter`,
`claude-code-plugins/claude-spawn`, `claude-code-plugins/guardrail`.
...
(config/dprint, config/stylelint, config/typescript, module/es, module/hyperscript,
and a few others). The vast majority omit both.
...
- 43 packages have no `files` field, meaning they would publish their entire directory
- Among packages with `files`, the value patterns vary: `["src"]`, `["src/"]`, `["dist/final"]`, `["dist/final","src"]`, `["src/","dist/"]`
...
- `dev-script/vm-builder`: `bin` only

## Exports shape inconsistency

Among packages that have `exports`, the value structure differs:

- Some use **string** values: `{ ".": "./index.mjs" }`
- Some use **condition objects**: `{ ".": { "types": "...", "node": "...", "default": "..." } }`
...
Task names use mixed quoting: some use `[tasks."lint:types"]` (double-quoted)
...
**Fixed:** renamed `BUGREPORT.*` files to `BUG-REPORT.*` for consistency.
...
This is probably intentional (dots = hierarchy, hyphens = slug),
...
while every other file family (TROUBLESHOOTING, TODO, PHILOSOPHY, BUGREPORT) uses **dots**.
...
- `dev-script/inference-canary`: `why-is-node-running@^3.2.2`
- `dev-script/inference-canary-viewer`: `@lobehub/icons-static-svg@^1.82.0`
