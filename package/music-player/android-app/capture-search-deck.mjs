...
// Capture D50 design-only Search comparisons that retain the real unfolded deck.
// Keyboard-closed frames do not establish visibility after native query focus.
...
const waitForCompose = (markers) => {
  for (let attempt = 0; attempt < 25; attempt += 1) {
    try {
      adbText(['shell', 'uiautomator', 'dump', '/sdcard/fold-search-deck-ready.xml']);
      const xml = adbText(['exec-out', 'cat', '/sdcard/fold-search-deck-ready.xml']);
...
try {
  ...
  for (const screen of screens) {
    ...
    for (const mode of ['dark', 'light']) {
      ...
      writeFileSync(join(evidenceDirectory, `search-deck-${screen.name}-roles-${mode}.json`),
      ...
      const variants = screen.name === 'inner' ? ['left', 'right', 'wide'] : ['left'];
      for (const variant of variants) {
        const captures = screen.name === 'cover' || variant === 'left' ? states : states.filter((stage) => stage.name !== 'player');
        for (const capture of [...captures.map((stage) => ({ stage, scale: '1.0' })),
          { stage: states[1], scale: '2.0' }, { stage: states[2], scale: '2.0' }]) {
          ...
          const candidate = `search-deck-${variant}-${stage.name}${mode === 'light' ? '-light' : ''}`;
          adbText(['shell', 'am', 'start', '-W', '-n', activity, '--es', 'candidate', candidate]);
          const markers = screen.name === 'inner' ? [...stage.markers, 'content-desc="Pause"', 'content-desc="Shuffle all folders"'] : stage.markers;
          const xml = waitForCompose(markers);
          const base = `search-deck-${screen.name}-${variant}-${stage.name}-${mode}-s${scale === '1.0' ? '100' : '200'}`;
          if (screen.name === 'inner') {
            const nodes = [...xml.matchAll(/<node\b[^>]*>/g)].map((match) => match[0]);
            for (const label of ['Previous track', 'Pause', 'Next track', 'Repeat track', 'Shuffle all folders']) {
              const node = nodes.find((entry) => entry.includes(`content-desc="${label}"`));
              const bounds = node?.match(/bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"/);
              if (!bounds || Number(bounds[1]) >= 1038 || Number(bounds[2]) >= 2074 || Number(bounds[4]) <= 136) {
                throw new Error(`${base}: unfolded deck action ${label} is not visible in its left region: ${node?.slice(0, 260) ?? 'absent'}`);
              }
            }
          }
