...
/**
 * Resolves the actual Git worktree root for selected path reconciliation.
 *
 ...
 * ```
 */
export async function selectedWorktreeRoot({ gitPath, cwd, }: Readonly<{
  gitPath: string;
  cwd: string;
}>,): Promise<string> {
  ...
  return realpath(DECODER.decode(result.stdout,).trim(),);
}

/**
 * Records selected final-newline files only when their original staged bytes
 * and ordinary worktree bytes match exactly. Partially staged edits cannot be
 * replaced by a commit correction. The installer rechecks before replacement.
 *
 * @param gitPath - real Git executable
 * @param cwd - effective Git directory
 ...
 * @returns exact original and intended blob identities eligible for completion
 * @example
 * ```ts
...
export async function selectedWorktreeRecords({
  ...
  const settled = await loadIndexEntries({
    ...
    cwd,
    indexPath,
    paths: selected.map(function selectedPath(candidate,) { return candidate.path; },),
  },);
  ...
  for (const candidate of selected) {
    ...
    if ((entry === undefined) || (entry.modeText !== gitMode)
      ...
      continue;
    const destination = join(repositoryRoot, candidate.path,);
    try {
