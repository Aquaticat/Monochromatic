...
const JOURNAL_RELATIVE_ROOT = join('cli-git-worktree-copy', 'v1',);
 ...
 * @example
 * ```ts
 * validateJournal(parsed, '/journal.json');
 * ```
 */
function validateJournal(value: unknown, path: string,): WorktreeCopyJournal {
  if (isRecord(value,)
    && (value.version === 1)
 ...
 *
 * @returns nothing after rename installs record
 *
 * @example
 * ```ts
...
export async function createWorktreeCopyJournal({
  ...
  const root = join(commonDir, JOURNAL_RELATIVE_ROOT,);
  await mkdir(root, { recursive: true, mode: PRIVATE_DIRECTORY_MODE, },);
  /** Unique final journal path. */
  const path = join(root, `${randomUUID()}${JOURNAL_SUFFIX}`,);
  ...
  await writeJournal({ path, record, },);
  return { path, record, };
}
...
export async function readPendingWorktreeCopyJournals(
  ...
  return Promise.all(names.map(async function readJournal(name,): Promise<PendingWorktreeCopyJournal> {
    ...
    try {
      ...
      return {
        path,
        record: validateJournal(parsed, path,),
      };
    }
    catch (error: unknown) {
...
export async function removeWorktreeCopyJournal(
  ...
  await rm(pending.record.stageContainer, { recursive: true, force: true, },);
}
