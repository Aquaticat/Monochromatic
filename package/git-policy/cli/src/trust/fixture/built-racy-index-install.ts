 ...
 *
 * A hook runs after cli-git prepared `post.index`,
 * rewrites the pinned file with same-size bytes,
 * and gives `post.index` the pinned second,
 ...
 *
 * @returns hook statements that edit the entry and pin `post.index`
 *
 * @example
 * ```ts
...
export async function verifyRacyIndexInstall({ env, }: Readonly<{
  ...
  /**
   * Hook statements editing the entry after `post.index` exists.
   */
  ...
  await writeFile(
    `${installRepository}/.git/hooks/pre-commit`,
    `#!/usr/bin/env node\n${installStatements}`,
  ...
  await rm(`${installRepository}/.git/hooks/pre-commit`,);
  ...
  await writeFile(
    `${recoveryRepository}/.git/hooks/post-commit`,
    `${KILL_WRAPPER_SOURCE.replace(
      '\n',
      `\n${recoveryStatements}`,
    )}`,
    { mode: EXECUTABLE_MODE, },
  ...
  await waitForOrphan();
  await rm(`${recoveryRepository}/.git/hooks/post-commit`,);
