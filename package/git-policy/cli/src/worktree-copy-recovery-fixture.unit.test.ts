/**
 Hand-built interrupted worktree-copy transactions and leftover probes shared by recovery tests.
 ...
 */
export const SOURCE_DIRECTORY_MODE = 0o755;

/**
 Source file mode.
 */
export const SOURCE_FILE_MODE = 0o644;
 ...
 */
export type InterruptedTransaction = Readonly<{
  /**
   Journal file.
   */
  journalPath: string;
...
/**
 ...
 ```
 */
export async function writeInterruptedInstallation({
  repositoryRoot,
  destinationRoot,
  recordCreated,
}: Readonly<{
...
/**
 ...
 ```
 */
export async function journalNames(repositoryRoot: string,): Promise<readonly string[]> {
  /**
...
/**
 ...
 ```
 */
export async function stageNames(destinationRoot: string,): Promise<readonly string[]> {
...
/**
 ...
 ```
 */
export async function sourceAndDestination({
  fixturePath,
  branch,
}: Readonly<{
  fixturePath: string;
