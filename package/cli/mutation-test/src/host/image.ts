...
export async function imageReference(options: {
  ...
  const containerSourceDirs = await Promise.all([
    ...
    'package/cli/mutation-test/src/engine',
  ...
  const sourceFiles = [
    ...
    join(
      options.repoRoot,
      'package/cli/mutation-test/src/shard-schema.ts',
    ),
    join(
      options.repoRoot,
      'package/cli/mutation-test/src/mounts.ts',
    ),
    join(
      options.repoRoot,
      'package/cli/mutation-test/src/is-record.ts',
    ),
  ]
