...
async function runCliFy({ args, }: { args: readonly string[]; },): Promise<{
  ...
  try {
    const result = await spawn('node', ['package/cli/fy/dist/final/node/index.mjs', ...args,], {
      cwd: REPO_ROOT,
    },);
    return {
      exitCode: 0,
...
await describe({
  ...
  children: [
    ...
    it({
      ...
      fn: async () => {
        const result = await runCliFy({
          args: ['./package/cli/fy/src/fixture/return1.ts', 'default',],
        },);
    ...
    it({
      ...
      fn: async () => {
        const result = await runCliFy({
          args: ['./package/cli/fy/src/fixture/return1-fn.ts', 'default',],
        },);
        expect(result.stdout,).toBe('1',);
        expect(result.exitCode,).toBe(0,);
      },
