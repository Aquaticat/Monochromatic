...
The package default export resolves to the prebuilt,
 self-contained
`dist/final/node/index.mjs` (run `mise run //package/oxlint-plugin/tsdoc:build` first).
...
```bash
mise run buildAndTest -- package/oxlint-plugin/tsdoc/src/oxlint-tsdoc.unit.test.ts
mise run //package/oxlint-plugin/tsdoc:test:unit
```

Test fixtures live in `package/test-fixture/oxlint-tsdoc/src/` with `valid/` and `invalid/` directories.
