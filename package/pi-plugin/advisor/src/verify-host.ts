...
import { parseAdvisorHostJsonl, verifyAdvisorHostEvidence, } from './verify-host-results.ts';

/** Fixed scenarios prevent a caller or model from supplying an unbounded task queue. */
const SCENARIOS = ['serial-credit', 'explicit-credit', 'collect', 'straggler', 'slash', 'slash-error',] as const;
...
async function verifyScenario(mode: typeof SCENARIOS[number],): Promise<void> {
  ...
  const root = await mkdtemp(join(tmpdir(), 'advisor-host-',),);
  ...
  await writeFile(join(agentDir, 'extensions', 'pi-advisor.json',), JSON.stringify({
    enabled: true, timeoutMs: 3000, maxAdvisorOutputTokens: 1000,
    hedgingEnabled: mode === 'collect' || mode === 'straggler' || mode === 'explicit-credit',
    hedgeDelayMs: 5, collectionGraceMs: 30,
  },), { flag: 'wx', },);
  ...
  await writeFile(sessionFile, [
    ...
    { type: 'message', id: '00000001', parentId: null, timestamp, message: {
      ...fauxAssistantMessage('Synthetic session seed',), provider: 'fixture-main', model: 'main', api: 'fixture-main-api',
    }, },
  ].map(function jsonLine(value): string { return JSON.stringify(value,); },).join('\n',) + '\n', { flag: 'wx', },);
  ...
  const fixture = fileURLToPath(new URL('./verify-host-fixture.ts', import.meta.url,),);
  ...
  const args = [
    piCli, '--offline', '--no-extensions', '--no-skills', '--no-prompt-templates', '--no-context-files',
    '--no-builtin-tools', '--no-themes', '--approve', '--mode', 'json', '--print',
    '--session', sessionFile, '--provider', 'fixture-main', '--model', 'main', '--models', 'fixture-*/*',
    '--extension', extension, '--extension', fixture, '--system-prompt', 'Fixture execution only.', '--', prompt,
  ];
  /** Child inherits no ambient provider credentials or live-session metadata. */
  const result = spawnSync(process.execPath, args, {
    cwd: root, encoding: 'utf8', timeout: HOST_WATCHDOG_MS, maxBuffer: MAX_HOST_OUTPUT_BYTES,
    env: {
      PATH: process.env.PATH ?? '', HOME: home, PI_CODING_AGENT_DIR: agentDir,
      PI_OFFLINE: '1', PI_TELEMETRY: '0', PI_SKIP_VERSION_CHECK: '1', TERM: 'dumb', NO_COLOR: '1',
      PI_ADVISOR_VERIFY_HOST: '1', PI_ADVISOR_FIXTURE_ROOT: root, PI_ADVISOR_FIXTURE_MODE: mode,
    },
  },);
  if (result.error !== undefined)
    throw result.error;
  assert.equal(result.status, 0, `Pi host ${mode} failed: ${result.stderr}\n${result.stdout}`,);
  assert.equal(result.stderr, '', `Pi host ${mode} emitted stderr`,);
  ...
  const entries = parseAdvisorHostJsonl(await readFile(sessionFile, 'utf8',),);
  /** Independent provider trace proves actual dispatch and cancellation. */
  const trace = parseAdvisorHostJsonl(await readFile(join(root, 'dispatch.jsonl',), 'utf8',),);
  verifyAdvisorHostEvidence({ mode, events, entries, trace, },);
  if (mode !== 'slash' && mode !== 'slash-error')
    assert.ok(result.stdout.includes('HOST_FIXTURE_COMPLETE',), 'primary agent did not continue after Advisor',);
  console.log(`Advisor real Pi host verified: ${mode}`,);
}

if (process.env.PI_ADVISOR_VERIFY_HOST !== '1')
