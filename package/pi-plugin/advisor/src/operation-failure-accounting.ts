...
export function registerAdvisorFailureAccounting(pi: ForeignHostCapability<ExtensionAPI>,): (failure: AdvisorFailureRecord) => void {
  ...
  pi.on('tool_result', function restoreAccounting(event) {
    ...
    return { details: { operation, }, usage: operation.usage, };
  },);
  pi.on('session_shutdown', function clearPending(): void { pending.clear(); },);
  return function recordFailure(failure: AdvisorFailureRecord,): void {
    pending.set(failure.toolCallId, failure.operation,);
    pi.appendEntry(ADVISOR_OPERATION_TYPE, failure.operation,);
  };
}
