...
const OperationViewSchema = v.object({
  operation: v.object({
    attempts: v.array(v.object({ model: v.string(), attempt: v.number(), state: v.string(), },),),
...
/**
 ...
 ```
 */
export function renderAdvisorOperationSummary({ details, text, expanded, theme, }: {
  ...
  const parsed = v.safeParse(OperationViewSchema, details,);
  if (!parsed.success)
  ...
  const succeeded = operation.reviews.length > 0 && operation.end !== 'caller';
  /** Text labels and color jointly distinguish operation states. */
  const heading = theme.fg(succeeded ? 'success' : 'error',
    `advisor: ${String(operation.reviews.length,)} usable review${operation.reviews.length === 1 ? '' : 's'}; ${String(operation.attempts.length,)} attempt${operation.attempts.length === 1 ? '' : 's'}; ${operation.end ?? 'running'}`,
  );
  /** Every attempted endpoint remains visible after partial progress is replaced. */
  const attempts = operation.attempts.map(function attemptLabel(attempt: { readonly model: string; readonly attempt: number; readonly state: string; }): string {
    ...
    return theme.fg(color, `${icon} ${attempt.model} #${String(attempt.attempt,)} ${attempt.state}`,);
  },).join('; ',);
