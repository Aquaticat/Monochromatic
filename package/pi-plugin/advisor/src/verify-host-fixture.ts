...
import { fauxAssistantMessage, fauxProvider, fauxToolCall, type AssistantMessage, type Context, type FauxResponseFactory, } from '@earendil-works/pi-ai';
...
export default function advisorHostFixture(pi: ForeignHostCapability<ExtensionAPI>,): void {
  ...
  const enabled = process.env.PI_ADVISOR_VERIFY_HOST;
  /** Fixture runner creates this private directory before spawning Pi. */
  const root = process.env.PI_ADVISOR_FIXTURE_ROOT;
  /** Fixed scenario chosen by the runner, never by a model prompt. */
  const mode = process.env.PI_ADVISOR_FIXTURE_MODE;
  if (enabled !== '1' || root === undefined || !['serial-credit', 'explicit-credit', 'collect', 'straggler', 'slash', 'slash-error',].includes(mode ?? '',))
  ...
  const tracePath = join(root, 'dispatch.jsonl',);
  ...
  const main = fauxProvider({ provider: 'fixture-main', api: 'fixture-main-api', models: [{ id: 'main', maxTokens: 1000, },], },);
  main.setResponses([
    fauxAssistantMessage(fauxToolCall('advisor', mode === 'explicit-credit' ? { model: 'fixture-credit/a', } : {}, { id: 'fixture-advisor-call', },),),
  ...
  const initial = fauxProvider({ provider: 'fixture-credit', api: 'fixture-credit-api', models: [
    { id: 'a', maxTokens: 1000, cost: { input: 10, output: 10, cacheRead: 0, cacheWrite: 0, }, },
    { id: 'b', maxTokens: 1000, cost: { input: 9, output: 9, cacheRead: 0, cacheWrite: 0, }, },
  ], },);
  initial.setResponses([
     ...
     */
    async function initialResponse(_context: ReadonlyDeep<Context>, options: ForeignHostCapability<Parameters<FauxResponseFactory>[1]>,): Promise<AssistantMessage> {
      await trace('initial-dispatch',);
      if (mode === 'serial-credit' || mode === 'explicit-credit' || mode === 'slash-error') {
        return fauxAssistantMessage('', { stopReason: 'error', errorMessage: '402: {"message":"You are out of credits. PRIVATE_PROVIDER_PAYLOAD","type":"billing_error"}', },);
      }
      if (mode === 'straggler') {
        ...
        return fauxAssistantMessage('', { stopReason: 'aborted', errorMessage: 'fixture cancellation acknowledged', },);
      }
      if (mode === 'collect')
  ...
  const alternate = fauxProvider({ provider: 'fixture-alternate', api: 'fixture-alternate-api', models: [
    { id: 'c', maxTokens: 1000, cost: { input: 1, output: 1, cacheRead: 0, cacheWrite: 0, }, },
  ], },);
