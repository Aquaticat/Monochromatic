...
import { nextAdvisorCandidate, NO_ADVISOR_CANDIDATE, } from './operation-candidates.ts';
...
import { observeAdvisorAttempt, type AdvisorAttemptCompletion, type AdvisorDispatch, type AdvisorOperationCandidate, } from './operation-attempt.ts';
import type { AdvisorOperationEnd, AdvisorOperationSnapshot, } from './operation-types.ts';
...
export async function runAdvisorOperation(options: ForeignHostCapability<AdvisorOperationOptions>,): Promise<AdvisorOperationSnapshot> {
  ...
  const { now = Date.now, wait = waitForAdvisorEvent, } = options;
  /** Single source of truth for attempts, credit exclusions, and collection. */
  const ledger = createAdvisorOperationLedger({ startedAtMs: options.startedAtMs, deadlineAtMs: options.deadlineAtMs, },);
  /** At most two active reviewer attempts are retained. */
  const active = new Map<number, { readonly candidate: AdvisorOperationCandidate; readonly promise: Promise<AdvisorAttemptCompletion>; }>();
  ...
  const innerL = tagged({ tag: runAdvisorOperation.name, l, },);

  /** Notify from ledger transitions, never from individual streamed usage deltas. */
  function notify(): void {
    wake.current.resolve(ADVISOR_CLOCK_BOUNDARY,);
    try { options.onUpdate?.(ledger.snapshot(),); }
    catch (error) { innerL.warn('Advisor progress callback failed', error,); }
  }
  ...
  function start(candidate: AdvisorOperationCandidate,): void {
    ...
    const records = ledger.snapshot().attempts;
    /** New operation-local attempt identity. */
    const id = records.length + 1;
    ledger.record({
      ...candidate, id, attempt: records.filter(record => record.model === candidate.model).length + 1,
      state: 'preparing', startedAtMs: now(), usageIncomplete: true,
    },);
    innerL.debug(`preparing ${candidate.model}; attemptId=${String(id,)}`,);
    active.set(id, { candidate, promise: observeAdvisorAttempt({ id, candidate, ledger, complete: options.complete, signal, now, graceMs: options.collectionGraceMs, notify, },), },);
    notify();
  }
   ...
   */
  function next(): AdvisorOperationCandidate | typeof NO_ADVISOR_CANDIDATE {
    /** Exclusions and dispatched identities are read just in time. */
    const current = ledger.snapshot();
    return nextAdvisorCandidate({
      candidates: options.candidates,
      attemptedModels: current.attempts.map(record => record.model),
      blockedProviders: current.blockedProviders,
      runningProviders: [...active.values(),].map(value => value.candidate.provider),
    },);
  }
  ...
  while (true) {
    ...
    for (const [id, entry,] of active) {
      ...
      if (record.state === 'running' || record.state === 'preparing')
        continue;
      active.delete(id,);
      if (record.state === 'empty' && record.attempt < 2 && ledger.snapshot().reviews.length === 0
        && !ledger.snapshot().blockedProviders.includes(entry.candidate.provider,))
        start(entry.candidate,);
    }
    if (current.reviews.length > 0) {
      if (![...active.keys(),].some(id => ledger.attempt(id,).state === 'running'))
      ...
      const firstDispatch = ledger.snapshot().attempts.find(record => record.dispatchedAtMs !== undefined)?.dispatchedAtMs;
      if (options.hedgeDelayMs !== undefined && firstDispatch !== undefined
        && now() >= firstDispatch + options.hedgeDelayMs && active.size < 2) {
        /** Recompute after a possible serial replacement was started. */
        const alternate = next();
        if (alternate !== NO_ADVISOR_CANDIDATE)
          start(alternate,);
      }
    }
    /** Current collection or deadline cutoff. */
    const boundary = ledger.snapshot().collectionEndsAtMs ?? options.deadlineAtMs;
    ...
    const hedgeAt = current.reviews.length === 0 && options.hedgeDelayMs !== undefined && firstDispatch !== undefined
