 ...
 */
const SOURCE_MARKER = `${sep}package${sep}module${sep}p-memoize-fork${sep}src${sep}`;
...
/**
 ...
 script outside the package) is never a target. The test-only helper
 `test-support.ts` is excluded explicitly: it is unreachable from the
 package's public API and is covered by its own unit test and the mutation
 run instead.
...
/**
 ...
 @example
 ```ts
 packageRelative('file:///repo/package/module/p-memoize-fork/src/p-memoize.ts');
 // => 'parse.ts'
 ```
 */
