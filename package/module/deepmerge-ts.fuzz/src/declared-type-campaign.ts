 ...
 */
function summarize(
  { failures, cases, }: { readonly failures: readonly CaseFailure[]; readonly cases: readonly RunCase[]; },
  ...
  for (const failure of failures) {
     ...
     */
    const drawn = cases[failure.id]?.drawn;
    /**
     Distinct diagnostic codes in this case.
     */
    const codes = new Set(failure.diagnostics.map(function codeOf(diagnostic,) {
      return diagnostic.slice(diagnostic.indexOf('error TS',) + 'error '.length,).split(':',)[0] ?? 'unknown';
    },),);
    for (const key of [...codes, `${drawn?.kind ?? '?'}:${drawn?.option ?? '?'}`,])
      tally.set(key, (tally.get(key,) ?? 0) + 1,);
  }
  return Object.fromEntries([...tally,].toSorted(function byCount(left, right,) {
...
export async function runCampaign(
   ...
   */
  const dir = join(PACKAGE_ROOT, 'dist', 'declared-type', `${String(seed,)}${unions ? '' : '-no-unions'}`,);
  /**
   Drawn and run cases.
   */
  const cases = drawCases({ seed, size, unions, },);
  /**
   Type-check outcome.
   */
  const { failures, stray, } = await checkCases({ cases, dir, },);
  /**
   Report path.
   */
  const reportPath = join(dir, 'report.json',);
  await writeFile(reportPath, `${JSON.stringify({ cases: cases.length, failing: failures.length, failures, seed, stray, summary: summarize({ cases, failures, },), }, undefined, 2,)}\n`,);
...
export async function buildCorpus(): Promise<void> {
   ...
   */
  const dir = join(PACKAGE_ROOT, 'dist', 'declared-type', 'corpus',);
  /**
   Fixed-seed cases.
   */
  const cases = drawCases({ seed: CORPUS_SEED, size: CORPUS_DRAWS, unions: true, },);
  /**
   Failures of the unfiltered draw.
   */
  const first = await checkCases({ cases, dir, },);
  if (first.stray.length > 0)
    throw new Error(`corpus header or emitter produced diagnostics:\n${first.stray.join('\n',)}`,);
  /**
   Ids of failing cases, dropped from the corpus.
   */
  const dropped = new Set(first.failures.map(function idOf(failure,) {
    return failure.id;
  },),);
  /**
   Cases that type-check.
   */
  const kept = cases.filter(function passes(_case, id,) {
    return !dropped.has(id,);
  },);
  /**
   Re-check of the kept cases in isolation.
   */
  const second = await checkCases({ cases: kept, dir: join(dir, 'kept',), },);
  if ((second.failures.length > 0) || (second.stray.length > 0))
    throw new Error(`filtered corpus still fails in ${String(second.failures.length,)} cases`,);
  await mkdir(dir, { recursive: true, },);
  await writeFile(join(dir, 'dropped.json',), `${JSON.stringify(first.failures, undefined, 2,)}\n`,);
  /**
   Committed module, with the drop count in its first line.
   */
  const emitted = emitModule({ cases: kept, exportName: 'DECLARED_TYPE_CASES', },);
  await writeFile(
    join(PACKAGE_ROOT, 'src', 'declared-type-soundness.generated.ts',),
...
if (import.meta.main) {
   ...
   */
  const [mode, seedText, sizeText, unionsText,] = process.argv.slice(2,);
  if (mode === 'corpus')
    await buildCorpus();
  else if (mode === 'campaign')
    await runCampaign({ seed: Number(seedText ?? Date.now(),), size: Number(sizeText ?? '1000',), unions: unionsText !== 'no-unions', },);
  else
