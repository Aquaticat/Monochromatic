 ...
 */
export type Sampled = { readonly value: unknown; readonly source: string; };
 ...
 */
type Entry = { readonly key: PropertyKey; readonly keySource: string; readonly sampled: Sampled; };
...
function scalar(value: unknown,): Sampled {
  return { source: value === undefined ? 'undefined' : JSON.stringify(value,), value, };
}
...
/**
 ...
 ```
 */
function entryOf({ key, symbolKey, sampled, }: { readonly key: string; readonly symbolKey: boolean; readonly sampled: Sampled; },): Entry {
  if (!symbolKey)
    return { key, keySource: JSON.stringify(key,), sampled, };
  /**
  ...
  return { key: symbol, keySource: `[${key}]`, sampled, };
}
...
function objectOf(entries: readonly Entry[],): Sampled {
  return {
    source: `{ ${entries.map(function property(entry,) {
      return `${entry.keySource}: ${entry.sampled.source}`;
    },).join(', ',)} }`,
    value: Object.defineProperties({}, Object.fromEntries(entries.map(function toDescriptor(entry,) {
      return [entry.key, { configurable: true, enumerable: true, value: entry.sampled.value, writable: true, },];
    },),),),
  };
}
...
function indexKeys(keyKind: 'number' | 'string' | 'symbol' | 'template',): readonly string[] {
  return { number: ['0', '1',], string: ['x', 'y',], symbol: ['SYM_INDEX',], template: ['k1', 'kz',], }[keyKind];
}
...
function sampleEntries(node: ObjectNode,): Arbitrary<readonly Entry[]> {
   ...
   */
  const props = node.props.map(function sampleProp(prop: PropNode,): Arbitrary<readonly Entry[]> {
    /**
     ...
     */
    const present = sampleValue(prop.type,).map(function toEntry(sampled,): readonly Entry[] {
      return [entryOf({ key: prop.key, sampled, symbolKey: prop.symbolKey, },),];
    },);
    return omittable ? oneof(constant<readonly Entry[]>([],), present,) : present;
  },);
  /**
   Extra index-signature entries.
   */
  const extras: Arbitrary<readonly Entry[]> = node.index.keyKind === 'none'
    ? constant([],)
    : uniqueArray(tuple(constantFrom(...indexKeys(node.index.keyKind,),), sampleValue(node.index.value,),), {
      ...
      return pairs.map(function toEntry([key, sampled,],) {
        return entryOf({ key, sampled, symbolKey: key === 'SYM_INDEX', },);
      },);
    },);
  return tuple(tuple(...props,), extras,).map(function flatten([declared, extra,],) {
    return [...declared.flat(), ...extra,];
  },);
}
...
export function sampleValue(node: TypeNode,): Arbitrary<Sampled> {
  ...
  if (node.kind === 'string')
    return constantFrom('a', 'b', 'zz',).map(scalar,);
  if (node.kind === 'boolean')
    return boolean().map(scalar,);
  ...
  if (node.kind === 'date') {
    return LEAF_INTEGER.map(function toDate(offset,): Sampled {
      return { source: `new Date(${String(Math.abs(offset,),)})`, value: new Date(Math.abs(offset,),), };
    },);
  }
  if (node.kind === 'regexp') {
    return constantFrom('x', 'y',).map(function toRegExp(pattern,): Sampled {
      return { source: `new RegExp(${JSON.stringify(pattern,)}, "g")`, value: new RegExp(pattern, 'g',), };
    },);
  }
  if ((node.kind === 'array') || (node.kind === 'tuple') || (node.kind === 'set')) {
    /**
     Element samples: one per tuple slot, or up to a few for arrays and Sets.
     */
    const items = node.kind === 'tuple'
      ? tuple(...node.elements.map(sampleValue,),)
      : array(sampleValue(node.element,), { maxLength: MAX_ELEMENTS, },);
    return items.map(function toCollection(samples,): Sampled {
      ...
      if (node.kind === 'set')
        return { source: `new Set<${emitType(node.element,)}>([${inner}])`, value: new Set(values,), };
      return { source: `[${inner}]`, value: values, };
    },);
  }
  if (node.kind === 'map') {
    return uniqueArray(tuple(node.keyKind === 'string' ? constantFrom<number | string>('k', 'm',) : constantFrom<number | string>(1, 2,), sampleValue(node.value,),), {
  ...
  if (node.kind === 'union')
    return oneof(...node.members.map(sampleValue,),);
  if (node.kind === 'record') {
    return tuple(...node.keys.map(function entry(key,) {
      return sampleValue(node.value,).map(function toEntry(sampled,) {
        return entryOf({ key, sampled, symbolKey: false, },);
      },);
    },),).map(objectOf,);
  }
  if (node.kind === 'box') {
    return sampleValue(node.inner,).map(function toBox(inner,) {
      return objectOf([entryOf({ key: 'v', sampled: inner, symbolKey: false, },), entryOf({ key: 'tag', sampled: scalar('s',), symbolKey: false, },),],);
    },);
  }
  if (node.kind === 'intersection') {
    return tuple(sampleEntries(node.left,), sampleEntries(node.right,),).map(function combine([left, right,],) {
