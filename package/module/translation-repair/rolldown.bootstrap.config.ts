...
const bootstrapManifest: Plugin = {
  ...
  generateBundle: {
    order: 'post',
    handler(_options: unknown, bundle: BootstrapBundle): void {
      /** Declaration output is not executable bootstrap content. */
      const chunks = Object.values(bundle).filter(function executable(value): boolean {
        return value.type === 'chunk' && value.fileName.endsWith('.mjs');
      });
      /** The host and child execute the same one-file bootstrap. */
      const [entry] = chunks;
      if (chunks.length !== 1 || entry === undefined || entry.type !== 'chunk' || entry.fileName !== 'producer-prepare.mjs')
        throw new PreparationBootstrapBuildError('Preparation bootstrap must emit exactly one producer-prepare.mjs executable.');
      if (entry.imports.some(function external(name): boolean { return !isBuiltin(name); }) || entry.dynamicImports.length !== 0)
      ...
      const manifest = {
        ...
        file: { path: entry.fileName, bytes: Buffer.byteLength(entry.code, 'utf8'), sha256: createHash('sha256').update(entry.code).digest('hex') },
        staticNodeImports: entry.imports.toSorted(),
      ...
      this.emitFile({ type: 'asset', fileName: 'producer-bootstrap.json', source: JSON.stringify(manifest, null, 2) });
    },
  },
};
...
const bootstrapConfig: ReturnType<typeof nodeConfig> = { ...config, plugins: [config.plugins, bootstrapManifest] };
