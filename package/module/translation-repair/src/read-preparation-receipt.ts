...
import type { PreparationReceiptBinding, PreparationReceiptQuestion, } from './preparation-receipt-model.ts';
 ...
 * ```
 */
function receiptKeys({ value, keys, }: { readonly value: Record<string, unknown>; readonly keys: readonly string[]; },): boolean {
 ...
 * ```
 */
function receiptOutcome({ value, modelIds, }: { readonly value: unknown; readonly modelIds: readonly RosterModelId[]; },): RoundOutcome<BlockPairingWire> {
  if ((!isJsonRecord(value,)) || (!receiptKeys({ value, keys: ['modelId', 'voice'], },)))
    throw new PreparationReceiptError({ kind: 'outcomes', },);
  /** Only an identity already registered by the caller can be reconstructed. */
  const modelId = modelIds.find(function registered(candidate,): boolean { return candidate === value.modelId; },);
  ...
  if (!isJsonRecord(voice,))
    throw new PreparationReceiptError({ kind: 'outcomes', },);
  if (voice.heard === true) {
    if ((!receiptKeys({ value: voice, keys: ['heard', 'value'], },)) || (!isBlockPairingWire(voice.value,)))
      throw new PreparationReceiptError({ kind: 'outcomes', },);
    return { modelId, voice: { heard: true, value: structuredClone(voice.value,), }, };
  }
  if ((voice.heard !== false)
    || (!receiptKeys({ value: voice, keys: ['heard', 'answered', 'unreachable'], },))
    || ((typeof voice.answered) !== 'boolean')
    || ((typeof voice.unreachable) !== 'boolean'))
    throw new PreparationReceiptError({ kind: 'outcomes', },);
  return { modelId, voice: { heard: false, answered: voice.answered, unreachable: voice.unreachable, }, };
}
...
export function readPreparationReceipt({ value, binding, question, }: {
  ...
  if ((!isJsonRecord(value,))
    || (!receiptKeys({ value, keys: ['version', 'state', 'binding', 'question', 'outcomes'], },))
    || (value.version !== 1)
    || (value.state !== 'complete'))
    throw new PreparationReceiptError({ kind: 'state', },);
  if (!isDeepStrictEqual(value.binding, binding,))
    throw new PreparationReceiptError({ kind: 'binding', },);
  if (!isDeepStrictEqual(value.question, question,))
    throw new PreparationReceiptError({ kind: 'question', },);
  if ((!Array.isArray(value.outcomes,)) || (value.outcomes.length === 0) || (value.outcomes.length > binding.modelIds.length))
    throw new PreparationReceiptError({ kind: 'outcomes', },);
