...
export function archiveDiffHunks({
  ...
  for (const line of patch.split('\n',)) {
    ...
    if (line.startsWith('-',)) {
      ...
      if ((state.lastSide === 'old') && (state.hunk
        .removed
        .length
        === state.hunk
        .oldCount))
        state.hunk
          .oldTerminated = false;
      else if ((state.lastSide === 'new') && (state.hunk
        .added
        .length
        === state.hunk
        .newCount))
  ...
  return hunks.map(function finalize(hunk: ArchiveDiffHunk,): ArchiveDiffHunk {
    if ((hunk.removed
      .length
    ...
    return {
      ...
      removed: hunk.removed
        .map(function previousLine(
          text,
          index,
        ): string {
        return foldGitDocumentLine({
          text,
          terminated: (index
            < (hunk.removed
              .length
              - 1)) || hunk.oldTerminated,
        },);
      },),
      added: hunk.added
