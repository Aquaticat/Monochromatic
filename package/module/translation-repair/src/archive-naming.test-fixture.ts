 ...
 * ```
 */
export async function namingFixtureGit({ cloneDir, args, }: {
  readonly cloneDir: string;
  readonly args: readonly string[];
},): Promise<string> {
  /** Metadata-only command result. */
  const result = await spawn(REAL_GIT, [
    '--literal-pathspecs', '-C', cloneDir,
    '-c', `core.hooksPath=${devNull}`,
    '-c', 'user.name=archive-fixture',
    '-c', 'user.email=archive-fixture@example.invalid',
    '-c', 'commit.gpgSign=false',
    ...args,
  ], { env: { GIT_CONFIG_GLOBAL: devNull, GIT_CONFIG_SYSTEM: devNull, }, },);
 ...
 * ```
 */
export async function commitNamingArchive({ cloneDir, relPath, text, }: {
  readonly cloneDir: string;
  readonly relPath: string;
  readonly text: string;
},): Promise<string> {
  /** Location of the fixture's next archive version. */
  const path = join(cloneDir, relPath,);
  await mkdir(dirname(path,), { recursive: true, },);
  await writeFile(path, text, 'utf8',);
  await namingFixtureGit({ cloneDir, args: ['add', '--', relPath], },);
  await namingFixtureGit({ cloneDir, args: ['commit', '--message', 'record invented archive'], },);
...
export async function makeNamingArchive({
  before = BEFORE_ARCHIVE, after = AFTER_ARCHIVE, relPath = ARCHIVE_PATH,
}: {
  ...
  const cloneDir = await mkdtemp(join(tmpdir(), 'translation-naming-',),);
  await namingFixtureGit({ cloneDir, args: ['init'], },);
  /** Sole predecessor of the naming revision. */
  const parentCommit = await commitNamingArchive({ cloneDir, relPath, text: before, },);
  /** Pinned current naming revision. */
  const commitSha = await commitNamingArchive({ cloneDir, relPath, text: after, },);
  return {
 ...
 * ```
 */
export function namingUse({ archiveText, quotedText = NAME_QUOTE,
  ...
  const node = nonNullishOrThrow(document.nodes.find(function contains(candidate,): boolean {
  ...
  const configuredModelIds = Array.from({ length: ROSTER_SIZE, }, function reader(_value, index,): string {
    return `reader-${String(index,)}`;
  },);
  return {
    archiveHash: hashContent({ content: archiveText, },),
    anchor: { nodeId: node.id, nodeHash: node.contentHash, startOffset, endOffset, quotedText, },
    configuredModelIds,
    ballots: configuredModelIds.slice(0, SUPPORTING_READERS,).map(function ballot(modelId,) {
      return { modelId, kind, };
    },),
  };
}
