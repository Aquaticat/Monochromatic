 ...
 * ```
 */
export function readPreparationOccurrence({
  expected,
  receipt,
  sourceText,
  targetText,
   ...
   */
  const pl = tagged({
    tag: readPreparationOccurrence.name,
    l,
  },);
  /**
   * The core owns all current parsing and independently checked terminal evidence.
   */
  const current = readPreparationEvidenceOccurrence({
    expected,
    receipt,
    sourceText,
    targetText,
  ...
  const details = queriedBlockPairingDetails({
    outcome: current.evidence
      .outcome,
    pair: current.pair,
    pairIndex: current.expected
      .pairIndex,
    targetContainers: current.target
      .containers,
  },);
  /**
  ...
  pl.info(`reconstructed registered parent ${String(current.expected
    .pairIndex,)} from ${String(current.evidence
      .outcome
      .outcomes
      .length,)} final seat records without calls`,);
  return structuredClone({
    scope: 'receipt-bound-occurrence',
    expected: current.expected,
    source: current.source,
    target: current.target,
    pair: current.pair,
    alignmentFindings: current.alignmentFindings,
    prepared,
  },);
}
