 ...
 *
 * `length` counts surrogate halves, so an emoji or a rare CJK character counts
 * twice on one side of a ratio and once on the other. The comparison here is
 * between a Chinese source and an English translation, which is exactly where
 * that asymmetry lands.
 *
