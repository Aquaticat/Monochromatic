...
function sameAttemptDirectory({
  ...
  return first.isDirectory() && current.isDirectory()
    && ((first.mode & SHARED_DIRECTORY_BITS) === 0n)
    && ((current.mode & SHARED_DIRECTORY_BITS) === 0n)
...
export async function verifyPreparationAttempt({
  ...
  readonly expected: PreparationAttemptLocation;
  readonly l: Logger
},): Promise<void> {
  /**
  ...
  if (((typeof requestedDir) !== 'string') || (requestedDir.trim()
    .length
    === 0)
    || (!validPreparationAttemptIdentity({
      attemptId,
      rootPlanDigest,
      rootPlanBytes,
    },)))
  ...
  try {
    ...
    if ((identity.bytes !== Buffer.byteLength(
      marker,
      'utf8',
    )) || (identity.digest !== hashContent({ content: marker, },)))
      throw new PreparationAttemptError({
