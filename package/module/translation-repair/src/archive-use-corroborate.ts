...
const USE_KINDS: readonly ArchiveUseKind[] = [
  ...
  'creative-work-title', 'mentioned-form', 'ordinary-prose', 'unresolved',
];
 ...
 * ```
 */
function referenceKind({ use, findings, }: {
  readonly use: InitialArchiveUse;
  ...
  if (configured.size === 0 || configured.size !== use.configuredModelIds.length
    || configured.has('',)) {
    findings.push(`archive-use-withheld (${use.anchor.nodeId}: invalid electorate)`,);
    return undefined;
  }
  /** Repeated reader identities abstain rather than manufacture independent support. */
  const multiplicity = new Map<string, number>();
  for (const ballot of use.ballots)
    multiplicity.set(ballot.modelId, (multiplicity.get(ballot.modelId,) ?? 0) + 1,);
  /** Valid unique readings; missing/unknown/duplicate output is not ordinary prose. */
  const eligible = use.ballots.filter(function eligibleBallot(ballot,): boolean {
    return configured.has(ballot.modelId,) && multiplicity.get(ballot.modelId,) === 1
      && USE_KINDS.includes(ballot.kind,);
  },);
  if (eligible.length !== use.ballots.length)
    findings.push(`archive-use-abstentions (${use.anchor.nodeId}: malformed or repeated reader metadata)`,);
  ...
  if (corroborated.length !== 1) {
    findings.push(`archive-use-withheld (${use.anchor.nodeId}: ${corroborated.length === 0
  ...
  if (reference === undefined)
    findings.push(`archive-use-withheld (${use.anchor.nodeId}: non-reference use ${corroborated[0] ?? 'unresolved'})`,);
  return reference;
}
...
export function corroboratedArchiveReferences({ uses, }: {
  ...
  const rl = tagged({ tag: corroboratedArchiveReferences.name, l, },);
  ...
  const references = uses.flatMap(function corroborate(use,): readonly CorroboratedArchiveReference[] {
    ...
    const kind = referenceKind({ use, findings, },);
    return kind === undefined ? [] : [{ use, kind, }];
  },);
