...
export function assertProducerInputNotInterrupted(signal: AbortSignal): void {
  ...
  if (signal.reason !== 'SIGINT' && signal.reason !== 'SIGTERM')
    throw new ProducerInputRunError({ operation: 'launch-container', locator: 'host interruption reason', });
...
export function producerInputSignals(): ProducerInputSignals {
  ...
  listeners.set('SIGINT', interrupted);
  listeners.set('SIGTERM', terminated);
  for (const [name, listener] of listeners)
