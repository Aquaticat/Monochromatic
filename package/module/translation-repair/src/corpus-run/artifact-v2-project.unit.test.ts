    ...
    it({
      ...
      fn: async () => {
        expect([
          ...
          return toArtifactOutcomeV2({ outcome: { kind, } as LaneSliceOutcome, },);
        },),)
          .toEqual([
            { kind: 'not-evaluated', },
            { kind: 'unfilled', },
    ...
    it({
      ...
      fn: async () => {
        const refusalOfNewMember = caught(function newMember() {
          toArtifactOutcomeV2({ outcome: { kind: 'napped', } as unknown as LaneSliceOutcome, },);
        },);
...
await describe({
  name: toArtifactDeliveryV2.name,
  children: [
    it({
    ...
    it({
      ...
      fn: async () => {
        expect([
          ...
          return toArtifactDeliveryV2({ delivery: { kind, } as SliceDelivery, },);
        },),)
          .toEqual([
    ...
    it({
      ...
      fn: async () => {
        const refusalOfNewMember = caught(function newMember() {
          toArtifactDeliveryV2({ delivery: { kind: 'napped', } as unknown as SliceDelivery, },);
        },);
...
await describe({
  name: toArtifactDecisionsV2.name,
  children: [
    it({
      name: 'carries a comparable reading`s verdict',
      fn: async () => {
        expect(toArtifactDecisionsV2({
          decisionComparison: {
            kind: 'comparable',
            verdict: 'same',
          },
    ...
    it({
      ...
      fn: async () => {
         ...
         */
        const projected = toArtifactDecisionsV2({
          decisionComparison: {
            kind: 'not-comparable',
  ...
  children: [
    it({
      ...
      fn: async () => {
        ...
        expect(keysOf({ value: toArtifactRowV2({ record, },), },),)
          .toEqual([
            'delivery',
            'incumbentKind',
            'incumbentText',
...
await describe({
  name: toArtifactComparisonRowV2.name,
  children: [
    it({
      ...
      fn: async () => {
         ...
         */
        const projected = toArtifactComparisonRowV2({ row, },);
    ...
    it({
      ...
      fn: async () => {
         ...
         * Row as version 2 records it.
         */
        const projected = toArtifactComparisonRowV2({ row, },) as {
