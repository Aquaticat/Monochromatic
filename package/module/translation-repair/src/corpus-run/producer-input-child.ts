...
export async function runProducerInputChild(): Promise<ProducerInputCompletion> {
  ...
  const expectedLaunch = { bytes: identity.launchBytes, sha256: identity.launchSha256, };
  /** Host locators remain metadata; all child file reads use fixed role mounts. */
  const launch = await readProducerInputLaunch({ path: PRODUCER_INPUT_PATHS.launch, expected: expectedLaunch });
  /** Bootstrap identity is a cross-check, not authentication of the program already executing. */
  await verifyProducerInputFile({ path: PRODUCER_INPUT_PATHS.bootstrap, expected: launch.bootstrap, operation: 'verify-runtime' });
  /** Independently bound manifest bytes precede all interpretation of application filenames. */
  const manifest = await readProducerRuntimeManifest({ dir: PRODUCER_INPUT_PATHS.runtime, expected: launch.runtime.manifest });
  await verifyProducerNodeRuntime(manifest);
  await verifyProducerInputFile({ path: PRODUCER_INPUT_PATHS.atomicLibrary, expected: launch.atomicLibrary, operation: 'verify-runtime' });
  await verifyProducerRuntimeInventory({ dir: PRODUCER_INPUT_PATHS.runtime, manifest });
  await verifyProducerInputFile({ path: PRODUCER_INPUT_PATHS.launch, expected: expectedLaunch, operation: 'read-launch' });
  /** URL conversion preserves the one fixed entry while preventing build-time application inclusion. */
  const entry = pathToFileURL(PRODUCER_INPUT_PATHS.application).href;
  ...
  if (!inputApplication(application))
    throw new ProducerInputRunError({ operation: 'invoke-application', locator: PRODUCER_INPUT_PATHS.application, });
  return await application.prepareProducerInputs({ launch, runId: identity.runId, launchSha256: identity.launchSha256 });
}
