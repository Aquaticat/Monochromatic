...
/** Native state access bypasses caller-owned public property replacements. */
const NATIVE_ABORTED = nonNullishOrThrow(Object.getOwnPropertyDescriptor(AbortSignal.prototype, 'aborted')?.get);
/** Native registration is retained before caller code can replace public methods. */
const NATIVE_ADD = EventTarget.prototype.addEventListener;
/** Native removal never consults the caller's removal property. */
...
/** No caller reason accompanies native cancellation forwarding. */
const NATIVE_ABORT = AbortController.prototype.abort;
...
export function ownProducerInputComparisonSignal(source: AbortSignal): ProducerInputComparisonCancellation {
  ...
  } = { unreadable: false, released: false, attempted: false, subscription: undefined };
  ...
  function readNativeState(): boolean {
    /** Reflective native invocation is unknown until its primitive result is checked. */
    const aborted: unknown = Reflect.apply(NATIVE_ABORTED, source, []);
    if (typeof aborted !== 'boolean') throw new ProducerInputComparisonError({ kind: 'contract' });
    return aborted;
  }
  ...
  function forwardNativeCancellation(): void {
    try {
      if (readNativeState()) Reflect.apply(NATIVE_ABORT, controller, []);
    }
    catch (error) {
  ...
  function assertReadable({ directory }: { readonly directory?: string }): void {
    if (state.unreadable) throw new ProducerInputComparisonError({ kind: 'contract', ...(directory === undefined ? {} : { directory }) });
  }
  ...
  function releaseSubscription(): void {
    ...
    try {
      ...
      else Reflect.apply(NATIVE_REMOVE, source, ['abort', forwardNativeCancellation]);
    }
    catch (error) {
  ...
  /**
   Private registration callback preserves opaque Node options while preventing synthetic-event consumption.
   ...
   @throws ProducerInputComparisonError when the source-verified registration protocol changes

   @example
   ```ts
   addNativeListener('abort', forwardNativeCancellation, { once: true });
   ```
   */
  const addNativeListener: AbortSignal['addEventListener'] = function addNativeListener(type: string, listener: EventListener | EventListenerObject, options?: Readonly<AddEventListenerOptions> | boolean): void {
    if ((type !== 'abort') || (listener !== forwardNativeCancellation) || (typeof options !== 'object') || (options === null)) throw new ProducerInputComparisonError({ kind: 'contract' });
    Reflect.apply(NATIVE_ADD, source, [type, listener, { ...options, once: false }]);
  };
  /**
   Private removal callback uses native receiver semantics without looking up caller methods.
   ...
   @example
   ```ts
   removeNativeListener('abort', forwardNativeCancellation);
   ```
   */
  const removeNativeListener: AbortSignal['removeEventListener'] = function removeNativeListener(type: string, listener: EventListener | EventListenerObject, options?: Readonly<EventListenerOptions> | boolean): void {
    if ((type !== 'abort') || (listener !== forwardNativeCancellation)) throw new ProducerInputComparisonError({ kind: 'contract' });
    Reflect.apply(NATIVE_REMOVE, source, [type, listener, options]);
  };

  try {
    if (isProxy(source) || (Object.getPrototypeOf(source) !== AbortSignal.prototype)) throw new ProducerInputComparisonError({ kind: 'contract' });
    /** Only Node's source-verified registration helper receives this private protocol view. */
    const registrationView = new Proxy(source, { get: function readRegistrationProperty(_target: AbortSignal, property: string | symbol): unknown {
      if (property === 'aborted') return readNativeState();
      if (property === 'addEventListener') return addNativeListener;
      if (property === 'removeEventListener') return removeNativeListener;
      throw new ProducerInputComparisonError({ kind: 'contract' });
    } });
    if (readNativeState()) Reflect.apply(NATIVE_ABORT, controller, []);
    else {
      state.attempted = true;
      state.subscription = addAbortListener(registrationView, forwardNativeCancellation);
      forwardNativeCancellation();
      assertReadable({});
    }
  }
  ...
  return { signal: controller.signal, assertReadable, [Symbol.dispose]: releaseSubscription };
}
