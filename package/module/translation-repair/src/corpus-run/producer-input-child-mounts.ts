 ...
 */
const ACCOUNT_MOUNTS: readonly string[] = [
  '/etc/passwd',
  '/etc/group'
];
/**
...
function mountMode({
  ...
  readonly mount: ProducerInputHostMount;
  readonly mode: 'ro' | 'rw'
},): boolean {
...
export async function verifyProducerInputChildMounts(): Promise<void> {
  try {
    ...
    const paths = [
      ...
      PRODUCER_INPUT_PATHS.launch,
      PRODUCER_INPUT_PATHS.runtime,
      PRODUCER_INPUT_PATHS.atomicLibrary,
      PRODUCER_INPUT_PATHS.selection,
      PRODUCER_INPUT_PATHS.supporting,
      PRODUCER_INPUT_PATHS.corpus,
