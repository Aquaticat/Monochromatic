...
const CAPABILITY_FIELDS: readonly string[] = ['CapInh', 'CapPrm', 'CapEff', 'CapBnd', 'CapAmb'];
...
async function verifyChildResources(): Promise<void> {
  ...
  const observed = await Promise.all(Object.entries(expected).map(async function resource([name, value]): Promise<boolean> {
  ...
  const temporary = await statfs('/tmp', { bigint: true });
  if (observed.some(function differs(value): boolean { return !value; })
    ...
    throw new ProducerInputRunError({ operation: 'verify-runtime', locator: 'child resource limits', });
}
...
async function verifyChildPrivileges(): Promise<void> {
  ...
  const lines = (await readFile('/proc/self/status', 'utf8')).split('\n');
  /** Only the fixed fields used by this gate are retained. */
  const fields = Object.fromEntries(lines.flatMap(function field(line): readonly (readonly [string, string])[] {
  ...
  if (fields.NoNewPrivs !== '1' || CAPABILITY_FIELDS.some(function enabled(name): boolean { return fields[name] !== EMPTY_CAPABILITIES; }))
    throw new ProducerInputRunError({ operation: 'verify-runtime', locator: 'child privilege state', });
}
...
async function verifyChildOutput(identity: ProducerInputChildIdentity): Promise<void> {
  ...
  const states = await Promise.all([PRODUCER_INPUT_PATHS.output, PRODUCER_INPUT_PATHS.home].map(async function directory(path): Promise<boolean> {
    /** Symlink leaves cannot substitute for host-created directories. */
    const state = await lstat(path);
    return state.isDirectory() && state.uid === identity.callerUid && (state.mode & NON_PRIVATE_BITS) === 0;
  }));
  if (states.some(function differs(value): boolean { return !value; })
    ...
    throw new ProducerInputRunError({ operation: 'verify-runtime', locator: 'child output namespace', });
}
...
export async function verifyProducerInputChildContext(identity: ProducerInputChildIdentity): Promise<void> {
  if (process.platform !== 'linux' || process.arch !== 'x64' || process.execPath !== PRODUCER_INPUT_PATHS.node
    ...
    throw new ProducerInputRunError({ operation: 'verify-runtime', locator: 'child execution context', });
  ...
  try {
    ...
    if (error instanceof ProducerInputRunError)
      throw error;
    throw new ProducerInputRunError({ operation: 'verify-runtime', locator: 'child context metadata', });
  }
}
