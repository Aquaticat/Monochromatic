 ...
 * ```
 */
async function statProducerSupportingFile({ hostRoot, reference, l, }: {
  ...
  const pl = tagged({ tag: statProducerSupportingFile.name, l, },);
  /** Path mapping never changes the locator supplied to semantic byte matching. */
  const mapped = relative(hostRoot, reference.path);
  if (mapped === '' || mapped === '..' || mapped.startsWith(`..${sep}`) || isAbsolute(mapped))
  ...
  const mountedPath = join(PRODUCER_INPUT_PATHS.supporting, mapped);
  ...
  try {
    ...
    const state = await lstat(mountedPath, { bigint: true, });
    if (!state.isFile() || state.size < 0n || state.size > BigInt(Number.MAX_SAFE_INTEGER))
      throw new ProducerInputRunError({ operation: 'read-support', locator: reference.path, });
    return { locator: reference.path, mountedPath, identity: { bytes: Number(state.size), sha256: reference.hash, }, };
  }
  catch (error) {
    if (error instanceof ProducerInputRunError)
      throw error;
    pl.warn(`supporting input metadata failed with ${isNativeError(error) ? error.name : typeof error}; native details were not retained`);
    throw new ProducerInputRunError({ operation: 'read-support', locator: reference.path, });
  }
}
 ...
 * ```
 */
export async function readProducerSupportingFiles({ selection, hostRoot, maximumBytes, l, }: {
  ...
  const pl = tagged({ tag: readProducerSupportingFiles.name, l, },);
  if (!Number.isSafeInteger(maximumBytes) || maximumBytes < 0)
    throw new ProducerInputRunError({ operation: 'read-support', locator: hostRoot, });
  ...
  const files = await mapOverlapped({
    ...
    oneItem: async function measured({ item, }): Promise<ProducerSupportingFile> {
      return await statProducerSupportingFile({ hostRoot, reference: item, l: pl, });
    },
  });
  /** Aggregate allowance is explicit launch authority, not an inferred package-wide ceiling. */
  const total = files.reduce(function add(sum, file): number { return sum + file.identity.bytes; }, 0);
  ...
  return await mapOverlapped({
    ...
    oneItem: async function loaded({ item, }): Promise<PreparationArtifactInput> {
      ...
      const rl = tagged({ tag: loaded.name, l: pl, });
      try {
        /** Descriptor-backed reading freshly checks the exact stat extent and frozen raw hash. */
        const content = await readProducerInputFile({ path: item.mountedPath, expected: item.identity, operation: 'read-support', });
        ...
        if (canonical !== item.mountedPath)
          throw new ProducerInputRunError({ operation: 'read-support', locator: item.locator, });
        return { path: item.locator, content, };
      }
      catch (error) {
        if (error instanceof ProducerInputRunError)
          throw error;
        rl.warn(`supporting input read failed with ${isNativeError(error) ? error.name : typeof error}; native details were not retained`);
        throw new ProducerInputRunError({ operation: 'read-support', locator: item.locator, });
      }
    },
  });
