...
import type { ProducerInputCompletion, ProducerInputLaunch, } from './producer-input-model.ts';
 ...
 * ```
 */
function ownProducerInputLaunch({ launch, l, }: {
  readonly launch: ProducerInputLaunch;
  readonly l: Logger;
},): ProducerInputLaunch {
  /** Snapshot failures do not retain descriptor or clone diagnostics. */
  const pl = tagged({ tag: ownProducerInputLaunch.name, l, },);
  try {
    return structuredClone(launch);
  }
  catch (error) {
    pl.warn(`input launch snapshot failed with ${isNativeError(error) ? error.name : typeof error}; details were not retained`);
 ...
 * ```
 */
function producerSelectionText({ bytes, l, }: {
  readonly bytes: Uint8Array;
  readonly l: Logger;
},): string {
  /** No decoder message or source excerpt is needed for this refusal. */
  const pl = tagged({ tag: producerSelectionText.name, l, },);
  try {
    return new TextDecoder('utf-8', { fatal: true, ignoreBOM: true, }).decode(bytes);
  }
  catch (error) {
    pl.warn(`selection decoding failed with ${isNativeError(error) ? error.name : typeof error}; details were not retained`);
    throw new ProducerInputRunError({ operation: 'read-selection', locator: PRODUCER_INPUT_PATHS.selection, });
  }
}
 ...
 * ```
 */
export async function prepareProducerInputs({ launch, runId, launchSha256, }: {
  readonly launch: ProducerInputLaunch;
  ...
  const fixed = ownProducerInputLaunch({ launch, l: pl, });
  ...
  const selectionBytes = await readProducerInputFile({ path: PRODUCER_INPUT_PATHS.selection, expected: fixed.selection, operation: 'read-selection', });
  /** Strict decoding preserves the complete raw selection rather than normalizing its text. */
  const text = producerSelectionText({ bytes: selectionBytes, l: pl, });
  /** The existing reader supplies the exact frozen reference inventory and independent parent identities. */
  const selection = readFrozenPreparationSelection({ text, expectedDigest: fixed.selection.sha256, l: pl, });
  /** Every support extent is checked against explicit launch authority before loading any supporting body. */
  const artifacts = await readProducerSupportingFiles({ selection, hostRoot: fixed.supporting.dir, maximumBytes: fixed.supporting.maximumBytes, l: pl, });
  /** The native owner freshly rechecks all bytes and current corpus semantics without resampling or model work. */
  const inputs = await buildPreparationRootInputs({ text, expectedDigest: fixed.selection.sha256, artifacts, pin: {
    cloneDir: PRODUCER_INPUT_PATHS.corpus,
    commitSha: fixed.corpus.commitSha,
    gitPath: '/usr/bin/git',
  }, l: pl, });
  /** Exact output bytes contain corpus-derived evidence and remain only in the private run directory. */
  const artifact = await writeProducerInputOutput({ file: 'unqualified-inputs.json', text: JSON.stringify(inputs), l: pl, });
  ...
  const completion: ProducerInputCompletion = {
    ...
    runId,
    launchSha256,
    artifact: { file: 'unqualified-inputs.json', ...artifact, },
    parentCount: inputs.parents.length,
    registrationCount: inputs.registry.length,
  };
  await writeProducerInputOutput({ file: 'complete.json', text: JSON.stringify(completion), l: pl, });
