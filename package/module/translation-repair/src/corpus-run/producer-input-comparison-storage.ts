...
export async function writeProducerInputComparisonRecord({
  ...
  const pl = tagged({ tag: writeProducerInputComparisonRecord.name, l });
  try {
...
export async function createProducerInputComparisonRun({
  ...
  const directory = join(parent, `producer-input-comparison-${runId}`);
  /** Namespace telemetry names no input document or provider body. */
  const pl = tagged({ tag: createProducerInputComparisonRun.name, l });
  try {
