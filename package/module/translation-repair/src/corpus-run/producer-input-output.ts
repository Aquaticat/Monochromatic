 ...
 * ```
 */
export async function writeProducerInputOutput({ file, text, l, }: {
  ...
  const pl = tagged({ tag: writeProducerInputOutput.name, l, },);
  ...
  const path = join(PRODUCER_INPUT_PATHS.output, file);
  ...
  try {
    ...
    await using handle = await open(path, 'wx', PRIVATE_OUTPUT_MODE);
    await handle.writeFile(text, 'utf8');
    await handle.sync();
  }
  catch (error) {
    pl.warn(`preparation input output failed with ${isNativeError(error) ? error.name : typeof error}; native details were not retained`);
  ...
  const identity = { bytes: Buffer.byteLength(text, 'utf8'), sha256: createHash('sha256').update(text, 'utf8').digest('hex'), };
