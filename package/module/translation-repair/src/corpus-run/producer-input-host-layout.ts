...
import {
  producerInputPathWithin,
  readProducerInputHostMounts,
  type ProducerInputHostMount,
} from './producer-input-host-mount.ts';
...
function directoryMount({
  ...
  readonly directory: ObservedDirectory;
  readonly mounts: readonly ProducerInputHostMount[]
},): string {
  if (mounts.some(function nested(mount): boolean {
    return (mount.point !== directory.path) && producerInputPathWithin({
  ...
  const covering = mounts.filter(function ancestor(mount): boolean {
    return producerInputPathWithin({
      parent: mount.point,
 ...
 * ```
 */
async function observeHostLayout({
  directories,
  callerUid,
}: {
   ...
   */
  const observed = await mapOverlapped({
    items: directories,
    overlap: 1,
    oneItem: async function one({ item }): Promise<ObservedDirectory> {
  ...
  return {
    scope: 'single-host-mount-input-layout',
    mountId,
...
export async function inspectProducerInputHostLayout(launch: ProducerInputLaunch): Promise<ProducerInputHostLayout> {
  ...
  const directories: readonly {
    ...
    {
      role: 'output-parent',
      path: launch.outputParent
    },
  ];
  try {
    return await observeHostLayout({
      directories,
      callerUid
    });
  }
...
export async function revalidateProducerInputHostLayout(layout: ProducerInputHostLayout): Promise<void> {
  ...
  try {
     ...
     */
    const current = await observeHostLayout({
