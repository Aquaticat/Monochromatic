import {
  producerInputBindings,
  type ProducerInputBinding,
} from './producer-input-bindings.ts';
import {
...
function producerInputMount({
  ...
  target,
  writable,
}: ProducerInputBinding): string {
  return [
    'type=bind',
    `source=${source}`,
    `target=${target}`,
    writable ? 'rw' : 'ro'
  ].map(function quoted(field): string {
...
export function producerInputContainerArgs(host: ProducerInputHost): readonly string[] {
  ...
  return [
    ...
    ...producerInputBindings(host)
      .flatMap(function mount(binding): readonly string[] { return [
        '--mount',
        producerInputMount(binding)
      ]; }),
    host.launch
      .imageId,
