...
import { namesWithoutQuoting, refusalText, } from '../refusal-text.ts';
...
async function main(): Promise<void> {
  ...
  if (basename(bootstrapPath) !== 'producer-prepare.mjs')
    throw new ProducerInputRunError({ operation: 'verify-runtime', locator: 'standalone bootstrap filename', });
  if (input.kind === 'child') {
  ...
  const result = await runProducerInputHost({ launchPath: input.launchPath, expected: input.expected, bootstrapPath });
  console.log(JSON.stringify({ kind: 'producer-preparation-input-host-complete', ...result }));
}
