...
import { readProducerInputFile, type ProducerInputFileIdentity, } from './producer-input-file.ts';
import type { ProducerInputLaunch, ProducerInputLocatedFile, } from './producer-input-model.ts';
 ...
 * ```
 */
function launchRecord({ value, keys, locator, }: {
  ...
  if (actual.length !== keys.length || !actual.every(function expected(key): boolean { return keys.includes(key); }))
    throw new ProducerInputRunError({ operation: 'read-launch', locator, });
 ...
 * ```
 */
function launchHex({ value, length, locator, }: {
  readonly value: unknown;
  readonly length: number;
  readonly locator: string;
},): string {
  if (typeof value !== 'string' || value.length !== length
    || ![...value].every(function hex(character): boolean { return '0123456789abcdef'.includes(character); }))
    throw new ProducerInputRunError({ operation: 'read-launch', locator, });
  return value;
}
 ...
 * ```
 */
function launchBytes({ value, locator, }: { readonly value: unknown; readonly locator: string; }): number {
 ...
 * ```
 */
function launchPath({ value, locator, }: { readonly value: unknown; readonly locator: string; }): string {
 ...
 * ```
 */
function launchIdentity({ value, locator, }: { readonly value: unknown; readonly locator: string; }): ProducerInputFileIdentity {
  /** Identity records accept no location or executable-selection field. */
  const record = launchRecord({ value, keys: ['bytes', 'sha256'], locator, });
  /** Executables, manifests and the original selection cannot be empty in this launch contract. */
  const bytes = launchBytes({ value: record.bytes, locator, });
  if (bytes === 0)
    throw new ProducerInputRunError({ operation: 'read-launch', locator, });
  return { bytes, sha256: launchHex({ value: record.sha256, length: SHA256_HEX_LENGTH, locator, }), };
}
 ...
 * ```
 */
function launchLocatedFile({ value, locator, }: { readonly value: unknown; readonly locator: string; }): ProducerInputLocatedFile {
  /** The file's location and identity have one fixed representation. */
  const record = launchRecord({ value, keys: ['path', 'bytes', 'sha256'], locator, });
  return { path: launchPath({ value: record.path, locator, }), ...launchIdentity({ value: { bytes: record.bytes, sha256: record.sha256 }, locator, }), };
}
 ...
 * ```
 */
export async function readProducerInputLaunch({ path, expected, }: {
  ...
  const bytes = await readProducerInputFile({ path, expected, operation: 'read-launch', });
  try {
    ...
    const record = launchRecord({ value, keys: ['version', 'kind', 'bootstrap', 'podman', 'imageId', 'runtime', 'atomicLibrary', 'selection', 'supporting', 'corpus', 'outputParent'], locator: 'launch', });
    if (record.version !== 1 || record.kind !== 'producer-preparation-input-launch')
      throw new ProducerInputRunError({ operation: 'read-launch', locator: path, });
    /** Runtime location does not choose the fixed application entry. */
    const runtime = launchRecord({ value: record.runtime, keys: ['dir', 'manifest'], locator: 'launch.runtime', });
    /** Supporting file discovery is limited to this root and caller-authorized total. */
    const supporting = launchRecord({ value: record.supporting, keys: ['dir', 'maximumBytes'], locator: 'launch.supporting', });
    /** Corpus semantics remain independently checked by the native input owner. */
    const corpus = launchRecord({ value: record.corpus, keys: ['dir', 'commitSha'], locator: 'launch.corpus', });
    return {
      version: 1,
      kind: 'producer-preparation-input-launch',
      bootstrap: launchIdentity({ value: record.bootstrap, locator: 'launch.bootstrap', }),
      podman: launchLocatedFile({ value: record.podman, locator: 'launch.podman', }),
      imageId: launchHex({ value: record.imageId, length: SHA256_HEX_LENGTH, locator: 'launch.imageId', }),
      runtime: { dir: launchPath({ value: runtime.dir, locator: 'launch.runtime.dir', }), manifest: launchIdentity({ value: runtime.manifest, locator: 'launch.runtime.manifest', }), },
      atomicLibrary: launchLocatedFile({ value: record.atomicLibrary, locator: 'launch.atomicLibrary', }),
      selection: launchLocatedFile({ value: record.selection, locator: 'launch.selection', }),
      supporting: { dir: launchPath({ value: supporting.dir, locator: 'launch.supporting.dir', }), maximumBytes: launchBytes({ value: supporting.maximumBytes, locator: 'launch.supporting.maximumBytes', }), },
      corpus: { dir: launchPath({ value: corpus.dir, locator: 'launch.corpus.dir', }), commitSha: launchHex({ value: corpus.commitSha, length: CORPUS_HEX_LENGTH, locator: 'launch.corpus.commitSha', }), },
      outputParent: launchPath({ value: record.outputParent, locator: 'launch.outputParent', }),
    };
  }
  catch (error) {
