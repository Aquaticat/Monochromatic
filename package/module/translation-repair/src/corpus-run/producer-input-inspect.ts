import { fieldMatches, inspectionRecord, record, strings, } from './producer-input-inspect-fields.ts';
 ...
 * ```
 */
function verifyCreatedBindings({
  host,
  value,
}: {
...
export function verifyCreatedProducerInputContainer({
  ...
  if ((!record(limits.Tmpfs)) || (Object.keys(limits.Tmpfs)
    ...
    || (!isDeepStrictEqual(
      limits.Tmpfs['/tmp'].split(',').toSorted(),
      ['rw', 'noexec', 'nosuid', 'nodev', `size=${PRODUCER_INPUT_LIMITS.temporaryBytes}`, 'rprivate', 'tmpcopyup'].toSorted()
    )))
    throw new ProducerInputRunError({
      operation: 'launch-container',
