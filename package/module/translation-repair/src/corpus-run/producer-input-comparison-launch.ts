...
async function writeComparisonLaunch({
  ...
  const pl = tagged({ tag: writeComparisonLaunch.name, l });
  try {
    if (((file !== 'base-launch.json') && (file !== 'derived-launch.json'))
      || (bytes.byteLength === 0) || (bytes.byteLength > MAX_LAUNCH_BYTES))
    ...
    const expected = { bytes: owned.byteLength, sha256: createHash('sha256').update(owned).digest('hex') };
    ...
    const observed = await verifyProducerInputOutputFile({ path, expected, ownerUid: run.uid, ownerGid: run.gid });
    await verifyProducerInputComparisonRun({ run, l: pl });
...
export async function deriveProducerInputComparisonLaunch({
  ...
  const pl = tagged({ tag: deriveProducerInputComparisonLaunch.name, l });
  try {
    ...
    if ((dirname(run.directory) !== base.outputParent)
      || (run.inputParent !== join(run.directory, 'producer-runs')))
    ...
    const baseBytes = await readProducerInputFile({ path: request.baseLaunchPath, expected: request.baseLaunchIdentity, operation: 'read-launch' });
    /** Explicit construction cannot forward hidden or unrecognized source properties. */
    const derived: ProducerInputLaunch = {
      version: base.version,
      kind: base.kind,
      bootstrap: { bytes: base.bootstrap.bytes, sha256: base.bootstrap.sha256 },
      podman: { path: base.podman.path, bytes: base.podman.bytes, sha256: base.podman.sha256 },
      imageId: base.imageId,
      runtime: { dir: base.runtime.dir, manifest: { bytes: base.runtime.manifest.bytes, sha256: base.runtime.manifest.sha256 } },
      atomicLibrary: { path: base.atomicLibrary.path, bytes: base.atomicLibrary.bytes, sha256: base.atomicLibrary.sha256 },
      selection: { path: base.selection.path, bytes: base.selection.bytes, sha256: base.selection.sha256 },
      supporting: { dir: base.supporting.dir, maximumBytes: base.supporting.maximumBytes },
      corpus: { dir: base.corpus.dir, commitSha: base.corpus.commitSha },
    ...
    const unchangedBindings = { ...derived, outputParent: base.outputParent };
    if (!isDeepStrictEqual(unchangedBindings, base))
      throw new ProducerInputComparisonError({ kind: 'contract', directory: run.directory });
    /** Exactly matched source bytes remain available even if later derivation persistence fails. */
    const baseIdentity = await writeComparisonLaunch({ run, file: 'base-launch.json', bytes: baseBytes, l: pl });
    if (!isDeepStrictEqual(baseIdentity, request.baseLaunchIdentity))
      throw new ProducerInputComparisonError({ kind: 'contract', directory: run.directory });
    /** Derived bytes have their own identity, not a self-issued approval claim. */
    const derivedLaunchIdentity = await writeComparisonLaunch({ run, file: 'derived-launch.json', bytes: new TextEncoder().encode(JSON.stringify(derived)), l: pl });
    await writeProducerInputComparisonRecord({
      run,
      file: 'derivation.json',
      value: { version: 1, kind: 'producer-preparation-input-launch-derivation', baseLaunchIdentity: baseIdentity, derivedLaunchIdentity, changedField: 'outputParent', baseOutputParent: base.outputParent, derivedOutputParent: run.inputParent },
      l: pl,
    });
    ...
    return {
      ...
      derivedLaunchPath: join(run.directory, 'derived-launch.json'),
