 ...
 * ```
 */
export async function writeProducerInputControl({ dir, file, bytes, }: {
  readonly dir: string;
  readonly file: 'launch.json' | 'start.json' | 'container-terminal.json' | 'verified-completion.json';
  readonly bytes: Uint8Array;
},): Promise<void> {
  if (!['launch.json', 'start.json', 'container-terminal.json', 'verified-completion.json'].includes(file))
    throw new ProducerInputRunError({ operation: 'write-output', locator: dir, });
  ...
  try {
    ...
    await using handle = await open(path, 'wx', PRIVATE_FILE_MODE);
    ...
    if (error instanceof ProducerInputRunError)
      throw error;
    throw new ProducerInputRunError({ operation: 'write-output', locator: path, });
  }
}
 ...
 * ```
 */
export async function createProducerInputRun({ parent, launchBytes, expected, }: {
  ...
  if (owned.length !== bytes || createHash('sha256').update(owned).digest('hex') !== sha256
    || typeof process.getuid !== 'function' || typeof process.getgid !== 'function')
    throw new ProducerInputRunError({ operation: 'read-launch', locator: parent, });
  ...
  const dir = join(parent, `producer-input-${runId}`);
  try {
    ...
    if (canonical !== parent || !state.isDirectory() || state.uid !== uid || (state.mode & NON_PRIVATE_MODE) !== 0)
      throw new ProducerInputRunError({ operation: 'write-output', locator: parent, });
    await mkdir(dir, { mode: PRIVATE_DIRECTORY_MODE });
    await writeProducerInputControl({ dir, file: 'launch.json', bytes: owned });
    await writeProducerInputControl({ dir, file: 'start.json', bytes: new TextEncoder().encode(JSON.stringify({
      version: 1, kind: 'producer-preparation-input-start', runId, launchSha256: sha256, launchBytes: bytes, uid, gid,
    })) });
    ...
    await mkdir(join(outputDir, 'home'), { mode: PRIVATE_DIRECTORY_MODE });
    return { dir, outputDir, launchPath: join(dir, 'launch.json'), containerIdPath: join(dir, 'container.id'), runId, uid, gid, };
  }
  catch (error) {
    if (error instanceof ProducerInputRunError)
      throw error;
