...
function createdContainerId(text: string): string {
  ...
  if (id.length !== CONTAINER_ID_WIDTH)
    throw new ProducerInputRunError({
      ...
      locator: 'created container identity',
    });
...
export async function createProducerInputContainer({
  ...
  try {
    ...
    if (image.trim() !== `${host.launch
      .imageId} linux amd64`)
      throw new ProducerInputRunError({
        operation: 'launch-container',
        locator: 'local image identity and target',
      });
    await runProducerInputCommand({
      host,
      stage: 'create',
      arguments_: producerInputContainerArgs(host),
      signal
    });
    /**
