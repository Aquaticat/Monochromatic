...
export function readProducerInputArguments(arguments_: readonly string[]): ProducerInputArguments {
  ...
  try {
    ...
    const { values, tokens } = parseArgs({ args: [...arguments_], strict: true, allowPositionals: false, tokens: true, options: {
      launch: { type: 'string' },
    ...
    const names = tokens.map(function option(token): string {
      if (token.kind !== 'option')
        throw new ProducerInputRunError({ operation: 'read-launch', locator: 'CLI argument tokens', });
      return token.name;
    });
    if (new Set(names).size !== names.length)
      throw new ProducerInputRunError({ operation: 'read-launch', locator: 'duplicate CLI option', });
    if (values.help === true) {
      if (arguments_.length !== 1)
        throw new ProducerInputRunError({ operation: 'read-launch', locator: 'CLI arguments', });
      return { kind: 'help' };
    }
    ...
    for (let index = 0; index < sha256.length; index += 1) {
      if (!'0123456789abcdef'.includes(sha256.charAt(index)))
        throw new ProducerInputRunError({ operation: 'read-launch', locator: 'CLI launch digest', });
    }
    return { kind: 'host', launchPath, expected: { bytes, sha256 } };
  }
  catch (error) {
