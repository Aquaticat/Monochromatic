...
export async function createProducerInputPodmanContext(run: ProducerInputRun): Promise<ProducerInputPodmanContext> {
  try {
    ...
    const fips = await readFile('/proc/sys/crypto/fips_enabled', 'utf8');
    if (fips.trim() !== '0')
      throw new ProducerInputRunError({ operation: 'launch-container', locator: 'host FIPS profile', });
    /** Host hook discovery is replaced, not merged with an ambient directory. */
    const hooks = join(run.dir, 'empty-hooks');
    await mkdir(hooks, { mode: PRIVATE_DIRECTORY_MODE });
    /** Only generated configuration is read through CONTAINERS_CONF. */
    const configPath = join(run.dir, 'containers.conf');
    {
      /** JSON quoting supplies basic escapes; TOML additionally requires escaping ASCII DEL. */
      const encodedHooks = JSON.stringify(hooks).replaceAll('\u007f', '\\u007F');
