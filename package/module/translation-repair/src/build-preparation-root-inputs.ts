...
import { CORPUS_COMMIT_SHA, CorpusReadError, type CorpusPin, } from './corpus-source.ts';
...
import { readPreparationRootPopulation, type PreparationRootPopulation, } from './preparation-root-population.ts';
...
function preparationRootPin(pin: CorpusPin,): CorpusPin {
  ...
  try {
    ...
    if ((fixed.commitSha !== CORPUS_COMMIT_SHA) || (typeof fixed.cloneDir !== 'string') || (fixed.cloneDir.trim().length === 0)
    ...
    return { cloneDir: resolve(origin, fixed.cloneDir,), commitSha: fixed.commitSha,
 ...
 * ```
 */
async function currentRootPopulation({ pin, selection, l, }: {
  ...
  const pl = tagged({ tag: currentRootPopulation.name, l, },);
  try {
    return await readPreparationRootPopulation({ pin, selection, l: pl, },);
  }
  catch (error) {
    ...
    throw new PreparationRootError({ kind: 'corpus-read', input: 'people/', },);
  }
}
 ...
 * ```
 */
export async function buildPreparationRootInputs({ text, expectedDigest, artifacts, pin, l, }: {
  ...
  const pl = tagged({ tag: buildPreparationRootInputs.name, l, },);
  /** Fresh raw matching establishes private byte ownership before any asynchronous corpus work. */
  const evidence = readPreparationSelectionEvidence({ text, expectedDigest, artifacts, l: pl, },);
  ...
  const reader = preparationRootArtifacts({ evidence, l: pl, },);
  /** Batch ownership resolves native Git once rather than rereading its executable for every corpus file. */
  const resolvedPin: CorpusPin = { ...fixed, gitPath: fixed.gitPath ?? await resolveGit(), };
  /** Full current population reconstruction cannot redraw or replace a failed frozen parent. */
  const current = await currentRootPopulation({ pin: resolvedPin, selection: evidence.selection, l: pl, },);
  matchPreparationRootPopulation({ selection: evidence.selection, record, artifacts: reader, current, },);
  /** Complete entry context is established before interpreting any parent-level carry-forward claim. */
  const entryReadings = preparationRootEntryReadings({ artifacts: reader, current, },);
  /** Rebuilt notes must retain the same unresolved source obligations as the frozen artifact. */
  const obligations = preparationRootParentReadings({ parents: current.parents, entryReadings, artifacts: reader, },);
  assertPreparationRootEqual({ actual: obligations, expected: evidence.selection.obligations, kind: 'reading-provenance', },);
  pl.info(`built unqualified root inputs for ${String(current.parents.length,)} frozen parents and ${String(current.registry.length,)} initial scope records; no acquisition or writer authority`,);
  return { scope: 'unqualified-preparation-root-inputs', selection: evidence.selection, references: reader.references(),
    rawDocuments: current.rawDocuments, listedEntryIds: current.listedEntryIds, population: current.population, excluded: current.excluded,
    entries: current.entries, parents: current.parents, obligations, registry: current.registry, unalignedDefinitions: current.unalignedDefinitions,
    questionAliases: preparationRootQuestionAliases(current.registry,), sectionPairing: 'not-registered', };
}
