...
const BOUNDED_SHELL_KEYS = ['shellDigest', 'slots',] as const;
...
export function assertBoundedMedia({ media, manifest, }: {
...
export function boundedAuthorMessages({
  ...
  assertKeys({ value: packet, expected: BOUNDED_AUTHOR_PACKET_KEYS, });
  assertKeys({ value: packet.shell, expected: BOUNDED_SHELL_KEYS, });
  return [
    { role: 'system', content: BOUNDED_AUTHOR_SYSTEM, },
    {
      role: 'user',
...
export function boundedVerifierMessages({
  ...
  const anonymousCandidates = candidates.map(function anonymous(candidate,) {
    ...
    assertKeys({ value: evidence, expected: BOUNDED_CANDIDATE_KEYS, });
    return evidence;
  },);
  const packet = {
    manifestDigest: manifest.manifestDigest,
  ...
  assertKeys({ value: packet, expected: BOUNDED_VERIFIER_PACKET_KEYS, });
  assertKeys({ value: packet.shell, expected: BOUNDED_SHELL_KEYS, });
  return [
    { role: 'system', content: BOUNDED_VERIFIER_SYSTEM, },
    {
      role: 'user',
      content: promptContent({
