 ...
 * ```
 */
export function readRegisteredPreparationParent({
  expected,
  sourceText,
  targetText,
  l,
   ...
   */
  const pl = tagged({
    tag: readRegisteredPreparationParent.name,
    l,
  },);
  ...
  if ([
    ...
    fixed.sourceIndex,
    fixed.targetIndex,
  ].some(function invalid(index,): boolean {
  ...
  const alignment = alignDocumentSections({
    source,
    target,
    ...((fixed.sectionPairing === undefined) ? {} : { sectionPairing: fixed.sectionPairing, }),
  },);
  /**
  ...
  return structuredClone({
    scope: 'registered-preparation-parent',
    source,
    target,
    pair,
