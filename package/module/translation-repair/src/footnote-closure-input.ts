...
export function readFootnoteClosureInput(
  ...
  if (archive.has('',) || original.has('',))
    return { kind: 'open', detail: 'a footnote namespace contains an empty normalized identifier', };
  ...
  for (const relation of map) {
    ...
    if (!archive.has(from,))
      return { kind: 'open', detail: `correspondence names unavailable archive label [^${relation.from}]`, };
    if (!original.has(to,))
      return { kind: 'open', detail: `correspondence names unavailable original label [^${relation.to}]`, };
    ...
    if ((earlierTo !== undefined && earlierTo !== to) || (earlierFrom !== undefined && earlierFrom !== from))
      return { kind: 'open', detail: `correspondence [^${relation.from}]->[^${relation.to}] conflicts after identifier normalization`, };
    if (earlierTo === undefined)
      correspondences.push({ from: relation.from, to: relation.to, },);
    forward.set(from, to,);
    backward.set(to, from,);
  }
  return { kind: 'ready', archive, original, correspondences, };
}
