...
import type { PreparationModelRequestBounds, PreparationProviderRequest, PreparationRequestManifest, } from './preparation-request-model.ts';
 ...
 * This follows prepareBlockPairing's windowed caller and promptUniqueClient's routed chatText path.
 ...
 * @param exchangeTimeoutMs - actual native per-call timeout to register
 ...
 * ```
 */
export async function captureBlockPairingRequests({ question, modelIds, exchangeTimeoutMs, signal, l, }: {
  ...
  const pl = tagged({ tag: captureBlockPairingRequests.name, l, },);
  signal.throwIfAborted();
  /** Owned inputs remain consistent across asynchronous native client construction. */
  const numbered = structuredClone({ sourceBlocks: question.sourceBlocks, targetBlocks: question.targetBlocks, },);
  /** Caller mutation cannot change the registered electorate during capture. */
  const registeredModels = [...modelIds,];
  assertPairingSeats({ modelIds: registeredModels, l: pl, },);
  if ((!Number.isFinite(exchangeTimeoutMs,)) || (exchangeTimeoutMs <= 0))
    throw new PreparationRequestCaptureError({ kind: 'timeout', },);
  if ((numbered.sourceBlocks.length === 0) || (numbered.targetBlocks.length === 0)
    || ((numbered.sourceBlocks.length === 1) && (numbered.targetBlocks.length === 1)))
  ...
  const models = await mapOverlapped({ items: registeredModels, overlap: 1, oneItem: async function captureModel({ item: modelId, }): Promise<{
    ...
    const providers = PROVIDER_ORDER.filter(function serves(provider,): boolean { return reach[provider]; },);
    ...
    const requests = await mapOverlapped({ items: providers, overlap: 1, oneItem: async function captureProvider({ item: provider, }): Promise<PreparationProviderRequest> {
      return await capturePreparationProviderRequest({ provider,
        request: { modelId, messages: protocol.messages, responseFormat: protocol.responseFormat, signal, exchangeTimeoutMs, }, l: pl, },);
    }, },);
    /** Conservative route-fallback envelope records one model denominator. */
    const bounds: PreparationModelRequestBounds = { modelId, providers, maxStageCalls, maxHttpAttemptsPerProvider,
      maxModelPosts: maxStageCalls * providers.length * maxHttpAttemptsPerProvider, };
    return { requests, bounds, };
  }, },);
  /** Explicit data excludes mutable signal state, occurrence coordinates and headers. */
  const data = { version: 1 as const, stage: 'block-pairing' as const,
    question: { ...numbered, protocol, },
    modelIds: registeredModels, exchangeTimeoutMs,
  ...
  pl.info(`captured ${String(data.requests.length,)} provider bodies for ${String(registeredModels.length,)} identities without transmission`,);
  return structuredClone({ ...data, requestConfigurationDigest: hashContent({ content: JSON.stringify(data,), },), },);
}
