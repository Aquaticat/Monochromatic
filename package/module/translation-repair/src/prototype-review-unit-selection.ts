// PROTOTYPE ONLY: Candidate K private candidate-scoped selection.
 ...
 */
const l = tagged({ tag: 'translation-repair-review-unit', },);
 ...
 * @example
 * ```ts
 * const independent = reviewUnitModelsIndependent({ authorModelId, verifierModelId, });
 * ```
 */
export function reviewUnitModelsIndependent({
  authorModelId,
  verifierModelId,
}: {
...
function ballotIsAdmitted({
  ...
  try {
    ...
    l.debug(`review unit abstains after revalidation: ${caughtValueText(error,)}`,);
    return false;
  }
}
 ...
 * @example
 * ```ts
 * const selection = selectReviewUnit({
 *   authorSettlement,
 *   ballots,
 *   manifest,
 *   expectedManifestDigest,
 ...
 * ```
 */
export function selectReviewUnit({
  authorSettlement,
  ballots,
  manifest,
  expectedManifestDigest,
  ...
  if (candidates.length === 0)
    throw new Error('review unit selection candidate set is empty');
  ...
  if (ballots.length > (candidates.length * REVIEW_UNIT_VERIFIER_COUNT))
    throw new Error('review unit count exceeds finite bound');
  /**
  ...
  if (fallback === undefined)
    throw new Error('review unit private fallback is absent');
  /**
   * Valid unique ballots by candidate after exact revalidation.
   */
  const validByCandidate = new Map(priority.map(function reviewUnits(candidate,) {
    /**
    ...
    duplicated.forEach(function report(modelId,) {
      l.debug(`review unit abstains after duplicate identity: ${modelId}`,);
    },);
    /**
  ...
  const cleanByCandidate = new Map(priority.map(function cleanRow(candidate,) {
    ...
    const cleanIds = (validByCandidate.get(candidate.candidateId,) ?? [])
      .filter(function clean(ballot,) {
        return reviewUnitModelsIndependent({
