...
import type { FrozenPreparationObligation, FrozenPreparationParent, FrozenPreparationReference, } from './preparation-selection-model.ts';
...
export function selectionParents(value: unknown,): readonly FrozenPreparationParent[] {
  ...
  return ids.map(function parent(item,): FrozenPreparationParent {
    ...
    if ((parts.length !== PARENT_ID_SEGMENTS) || (entryId === undefined) || (entryId.trim().length === 0)
      || (entryId === '.') || (entryId === '..') || entryId.includes('\\',) || entryId.includes('\0',)
      || (parts[1] !== 'source-section') || (parts[TARGET_LABEL_INDEX] !== 'target-section'))
      throw new PreparationRootError({ kind: 'selection-parents', },);
    return { parentId, entryId, sourceIndex: parentCoordinate(parts[2],), targetIndex: parentCoordinate(parts[TARGET_COORDINATE_INDEX],), };
  },);
}
...
export function selectionReferences(value: unknown,): readonly FrozenPreparationReference[] {
  ...
  const references = selectionArray(value,).map(function reference(item,): FrozenPreparationReference {
    /** Parsed record has no executable meaning. */
    const record = selectionRecord(item,);
    return { path: selectionString(record.path,), hash: selectionDigest({ value: record.hash, kind: 'selection-references', },), };
  },);
  if ((references.length === 0) || (new Set(references.map(function path(reference,): string { return reference.path; },),).size !== references.length))
 ...
 * ```
 */
export function selectionObligations({ value, parents, }: { readonly value: unknown; readonly parents: readonly FrozenPreparationParent[]; },): readonly FrozenPreparationObligation[] {
  ...
  return records.map(function obligation(item, index,): FrozenPreparationObligation {
    /** Notes are data authored during reading, not execution directives. */
    const record = selectionRecord(item,);
    if ((record.parentId !== parents[index]?.parentId) || (record.disposition !== 'unqualified; failure aborts without substitution')
      || ((typeof record.pictureEvidenceNeeded) !== 'boolean') || ((typeof record.scopeQualificationOpen) !== 'boolean'))
      throw new PreparationRootError({ kind: 'selection-obligations', },);
    /** Each required context string is preserved rather than interpreted or rewritten. */
    const requiredContext = selectionArray(record.requiredContext,).map(selectionString,);
    if (requiredContext.length === 0)
      throw new PreparationRootError({ kind: 'selection-obligations', },);
    return { parentId: selectionString(record.parentId,), requiredContext, pictureEvidenceNeeded: record.pictureEvidenceNeeded, scopeQualificationOpen: record.scopeQualificationOpen, };
  },);
}
