...
function markerBoundaryIndex(
  { markers, offset, }: { readonly markers: readonly ActiveFootnoteMarker[]; readonly offset: number; },
): number {
  /** Owned half-open search interval, narrowed together as one algorithmic state. */
  const bounds = { start: 0, end: markers.length, };
...
export function sliceFootnoteLabels(
  ...
  const { startOffset, endOffset, } = chunk;
  if (!Number.isSafeInteger(startOffset,) || !Number.isSafeInteger(endOffset,) || startOffset < 0
    || endOffset < startOffset || endOffset > documentText.length
    || chunk.text !== documentText.slice(startOffset, endOffset,))
    throw new FootnoteRewriteError({ kind: 'slice-scope', },);
  /** First spelling per normalized identity within this slice. */
  const labels = new Map<string, string>();
  for (let index = markerBoundaryIndex({ markers, offset: startOffset, },); index < markers.length; index += 1) {
