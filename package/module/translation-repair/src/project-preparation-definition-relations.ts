 ...
 * ```
 */
function definitionEndpoint({ node, blockIndex, }: { readonly node: DocumentNode; readonly blockIndex: number; },): PreparationDefinitionEndpoint {
 ...
 * ```
 */
export function projectPreparationDefinitionRelations({ occurrence, domain, l, }: {
  ...
  const pl = tagged({ tag: projectPreparationDefinitionRelations.name, l, },);
  /** All data comes from an owned completed reconstruction, not a caller-mutated naked parent. */
  const { pair, prepared, expected, } = occurrence;
  ...
  const { question, outcome, requiredUsable, } = replayPreparedBlockEvidence({ pair, evidence: prepared.evidence, modelIds: expected.binding.modelIds, l: pl, },);
  ...
  const definitionRelations = outcome.pairs.filter(function bothDefinitions(relation,): boolean {
    ...
    const source = nonNullishOrThrow(pair.source.nodes[relation.source],);
    /** Target access shares the same checked index boundary. */
    const target = nonNullishOrThrow(pair.target.nodes[relation.target],);
    return {
      source: definitionEndpoint({ node: source, blockIndex: relation.source, },),
      target: definitionEndpoint({ node: target, blockIndex: relation.target, },),
  ...
  return structuredClone({
    qualification: 'pairing-only', scope: 'footnote-definitions', occurrence: expected,
    questionKey: question.key, modelIds: expected.binding.modelIds, requiredUsable, usable: outcome.usable,
    domain: currentDomain, definitionRelations,
  },);
}
