...
import { FROZEN_PREPARATION_PARENT_COUNT, selectionObligations, selectionParents, selectionReferences, } from './preparation-selection-records.ts';
import { selectionArray, selectionDigest, selectionRecord, selectionString, } from './preparation-selection-value.ts';
...
const SELECTION_KEYS = ['status', 'corpus', 'runtime', 'runtimeDigest', 'populationDigest', 'poolDigest', 'supersededPopulationDigest',
  'baselineCoordinatesOnly', 'rules', 'sampler', 'census', 'exclusions', 'orderedParentIds', 'dependencies',
  'completeEntryReadings', 'completeParentReadings', 'boundaries', 'references',] as const;
 ...
 * ```
 */
export function readFrozenPreparationSelection({ text, expectedDigest, l, }: {
  readonly text: string;
  readonly expectedDigest: string;
  readonly l: Logger;
},): FrozenPreparationSelection {
  /** Artifact identity checks precede all JSON interpretation. */
  const pl = tagged({ tag: readFrozenPreparationSelection.name, l, },);
  ...
  if ((keys.length !== SELECTION_KEYS.length) || SELECTION_KEYS.some(function missing(key,): boolean { return !Object.hasOwn(record, key,); },)
  ...
  if ((sampler.name !== 'pickSpreadSample') || (sampler.count !== FROZEN_PREPARATION_PARENT_COUNT) || (sampler.seed !== 'none'))
  ...
  const entries = new Set(parents.map(function entry(parent,): string { return parent.entryId; },),);
  ...
  if ((census.selectedParents !== parents.length) || (record.completeParentReadings !== parents.length)
    || (census.selectedEntries !== entries.size) || (record.completeEntryReadings !== entries.size))
    throw new PreparationRootError({ kind: 'selection-parents', },);
  // These policy records remain part of the independently matched bytes, not executable planning instructions.
  selectionArray(record.rules,).map(selectionString,);
  selectionArray(record.boundaries,).map(selectionString,);
  selectionArray(record.exclusions,).map(selectionRecord,);
  ...
  const obligations = selectionObligations({ value: record.dependencies, parents, },);
  ...
  return {
    scope: 'frozen-selection-identity', digest: expectedDigest, bytes: Buffer.byteLength(text, 'utf8',),
    corpusCommitSha: CORPUS_COMMIT_SHA, populationDigest: selectionDigest({ value: record.populationDigest, },), poolDigest: selectionDigest({ value: record.poolDigest, },),
    parents, references, obligations, selectionRuntimeDigest,
    samplerNodeVersion: selectionString(sampler.nodeVersion,), samplerIcuVersion: selectionString(sampler.icuVersion,), samplerDigest: hashContent({ content: JSON.stringify(sampler,), },),
  };
}
