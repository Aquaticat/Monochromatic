...
import { isMissingCorpusObject, readCorpusBytes, type CorpusPin, } from './corpus-source.ts';
 ...
 * ```
 */
function preparationRootText({ bytes, relPath, }: {
  ...
  return { text: folded.text, identity: { relPath, bytes: bytes.byteLength,
    rawHash: createHash('sha256',).update(bytes,).digest('hex',), decodedHash: hashContent({ content: decoded, },),
    effectiveHash: hashContent({ content: folded.text, },), foldedCrLf: folded.folded, }, };
}
 ...
 * ```
 */
export async function readPreparationRootCorpusPair({ pin, entryId, l, }: {
  ...
  const pl = tagged({ tag: readPreparationRootCorpusPair.name, l, },);
  /** Native order fixes source and archive roles before asynchronous reads begin. */
  const paths = [`people/${entryId}/page.md`, `people/${entryId}/page.en.md`,] as const;
  ...
  const reads = await Promise.allSettled(paths.map(async function read(relPath,): Promise<Uint8Array> {
    return await readCorpusBytes({ pin, relPath, },);
  },),);
  for (const [index, read,] of reads.entries()) {
    ...
    if (!isMissingCorpusObject(failure,)) {
      ...
      throw new PreparationRootError({ kind: 'corpus-read', input: nonNullishOrThrow(paths[index],), },);
    }
  ...
  const texts = reads.flatMap(function successful(read, index,): PreparationRootTextRead[] {
    if (read.status === 'rejected')
      return [];
    return [preparationRootText({ bytes: read.value, relPath: nonNullishOrThrow(paths[index],), },),];
  },);
  if (texts.length !== paths.length)
    return { kind: 'missing', rawDocuments: texts.map(function identity(read,): PreparationRootRawDocument { return read.identity; },), };
  pl.debug(`read complete pinned corpus pair for ${JSON.stringify(entryId,)}`,);
  return { kind: 'complete', source: nonNullishOrThrow(texts[0],), archive: nonNullishOrThrow(texts[1],), };
}
