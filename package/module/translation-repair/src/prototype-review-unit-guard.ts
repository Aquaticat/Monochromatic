 ...
 * @example
 * ```ts
 * const diagnosis = diagnoseReviewUnitResponse({ value, ledger, candidate, });
 * ```
 */
export function diagnoseReviewUnitResponse({
  ...
  const findingResult = value.findings
    .reduce<ReviewUnitGuardFailure | typeof FINDING_VALID>(
 ...
 * @example
 * ```ts
 * const guard = reviewUnitResponseGuard({ ledger, candidate, });
 * ```
 */
export function reviewUnitResponseGuard({
  ...
  return function isReviewUnitResponse(
    value: unknown,
  ): value is ReviewUnitResponse {
