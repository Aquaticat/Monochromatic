...
- Preserve every Markdown structure the ORIGINAL uses: block quotes, list markers, headings, footnote markers, links, and the paragraph breaks between blocks. WHERE THE EXISTING TRANSLATION SHAPES THE PASSAGE DIFFERENTLY, splitting one block into two, merging two into one, or quoting a line the ORIGINAL runs as prose, KEEP THE EXISTING TRANSLATION'S SHAPE. It is the shape the archive's page already has, and a rendering that drops one of its blocks deletes that block from the page. The same holds for a link whose destination the EXISTING TRANSLATION renders differently from the ORIGINAL (a moved domain, an article in the reader's language): keep the existing destination, and never carry both.
