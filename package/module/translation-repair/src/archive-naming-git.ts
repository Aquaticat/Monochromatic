...
const MAX_METADATA_BYTES = MAX_METADATA_MEBIBYTES * KIBI * KIBI;
 ...
 * ```
 */
export async function archiveGitOutput({ pin, relPath, args, signal, }: {
  ...
  const rl = tagged({ tag: archiveGitOutput.name, l, },);
  ...
  try {
    ...
    const result = await execFileAsync(gitPath, [
      '--no-replace-objects', '--literal-pathspecs', '-C', pin.cloneDir,
    ...
    return foldCarriageReturns({ text: result.stdout, },).text;
  }
  catch (cause) {
    rl.warn('history command did not complete; retaining its cause',);
    throw new ArchiveNamingEvidenceError({ kind: 'history-read', relPath, cause, },);
  }
}
