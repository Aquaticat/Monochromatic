import { sealedNodeIds, type ArchiveOriginalSpan, } from './archive-original-note.ts';
import type { ChunkPair, } from './chunk-document.ts';
import { hashContent, type DocumentNode, } from './document-node.ts';
import type { PreparationRootNode, PreparationRootParent, PreparationRootPopulationParent, } from './preparation-root-population-model.ts';
...
export function preparationRootNode(node: DocumentNode,): PreparationRootNode {
  return { id: node.id, kind: node.kind, zone: node.zone, startOffset: node.startOffset, endOffset: node.endOffset, contentHash: node.contentHash, };
}
 ...
 * ```
 */
export function preparationRootParent({ entryId, pairIndex, pair, spans, }: {
  ...
  const intersections = spans.filter(function intersects(span,): boolean {
    return (span.startOffset < pair.target.endOffset) && (span.endOffset > pair.target.startOffset);
  },).map(function intersection(span,): { readonly startOffset: number; readonly endOffset: number; readonly noteHash: string; } {
    return { startOffset: span.startOffset, endOffset: span.endOffset, noteHash: hashContent({ content: span.note, },), };
  },);
  /** A partly protected node must not become an unqualified writable fragment. */
  const straddlingNodeIds = pair.target.nodes.filter(function straddles(node,): boolean {
  ...
  return {
    id: `${entryId}/source-section/${String(pair.source.sliceIndex,)}/target-section/${String(pair.target.sliceIndex,)}`,
    entryId,
    index: pair.source.sliceIndex,
    pairIndex,
    sourceSectionIndex: pair.source.sliceIndex,
    targetSectionIndex: pair.target.sliceIndex,
    sourceText: pair.source.text,
    incumbentText: pair.target.text,
    source: { startOffset: pair.source.startOffset, endOffset: pair.source.endOffset, hash: hashContent({ content: pair.source.text, },), nodes: pair.source.nodes.map(preparationRootNode,), },
    target: { startOffset: pair.target.startOffset, endOffset: pair.target.endOffset, hash: hashContent({ content: pair.target.text, },), nodes: pair.target.nodes.map(preparationRootNode,), },
    originalProtection: { intersections, sealedTargetNodeIds: [...sealed,], straddlingNodeIds, allTargetNodesSealed: (pair.target.nodes.length > 0) && (sealed.size === pair.target.nodes.length), },
  };
}
...
export function preparationRootPopulationParent(parent: PreparationRootParent,): PreparationRootPopulationParent {
  return { id: parent.id, entryId: parent.entryId, pairIndex: parent.pairIndex, sourceSectionIndex: parent.sourceSectionIndex,
    targetSectionIndex: parent.targetSectionIndex, source: parent.source, target: parent.target, originalProtection: parent.originalProtection, };
}
