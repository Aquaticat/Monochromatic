 ...
 * ```
 */
function uniqueInitialUse({ use, uses, }: {
  ...
  return uses.filter(function sameRange(other,): boolean {
    return other.archiveHash === use.archiveHash
      && other.anchor.startOffset === use.anchor.startOffset
      && other.anchor.endOffset === use.anchor.endOffset;
  },).length === 1;
}
 ...
 * ```
 */
export function archiveNamingScopes({ archiveText, uses, references, origins, relPath, }: {
  readonly archiveText: string;
  readonly uses: readonly InitialArchiveUse[];
  readonly references: readonly CorroboratedArchiveReference[];
  readonly origins: readonly ArchiveLineOrigin[];
  readonly relPath: string;
},): { readonly scopes: readonly ArchiveNamingScope[]; readonly findings: readonly string[]; } {
  ...
  const nodes = new Map(document.nodes.map(function nodeEntry(node,): readonly [string, typeof node] {
    return [node.id, node];
  },),);
  ...
  const scopes = references.flatMap(function locate(reference,): readonly ArchiveNamingScope[] {
    ...
    if (node === undefined || node.contentHash !== anchor.nodeHash
      || !Number.isSafeInteger(anchor.startOffset,) || !Number.isSafeInteger(anchor.endOffset,)
      || anchor.startOffset < node.startOffset || anchor.endOffset > node.endOffset
      || anchor.endOffset <= anchor.startOffset
      || archiveText.slice(anchor.startOffset, anchor.endOffset,) !== anchor.quotedText) {
      throw new ArchiveNamingEvidenceError({ kind: 'anchor-mismatch', relPath, },);
    }
    if (!uniqueInitialUse({ use, uses, },)) {
    ...
    const text = archiveText.slice(startOffset, endOffset,);
    ...
    const matching = origins.filter(function sameLine(origin,): boolean {
      return foldInvisibleVariants({ text: origin.text, },).text === text;
    },);
    ...
    if (origin.boundary || origin.ignored || !sameGitPath({ wirePath: origin.filename, relPath, },)) {
      findings.push(`archive-revision-withheld (${anchor.nodeId}: root, ignored or renamed origin)`,);
      return [];
    }
    return [{ reference, text, startOffset, origin,
      lineNumber: archiveText.slice(0, startOffset,).split('\n',).length, }];
  },);
