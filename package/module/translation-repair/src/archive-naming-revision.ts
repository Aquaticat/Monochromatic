...
import { archiveBlameOrigins, isArchiveGitObjectId, } from './archive-blame.ts';
...
import type { ArchiveNamingRevisionResult, InitialArchiveUse, } from './archive-naming-model.ts';
 ...
 * ```
 */
export async function readQualifiedArchiveNamingRevisions({ pin, relPath, uses,
  signal, l = defaultLogger, }: {
  ...
  const rl = tagged({ tag: readQualifiedArchiveNamingRevisions.name, l, },);
  ...
  if (corroborated.references.length === 0)
    return { revisions: [], findings: corroborated.findings, };
  if (!isArchiveGitObjectId(pin.commitSha,) || relPath.startsWith('/',)
    || relPath.includes('\0',) || relPath.split('/',).some(function nonLiteralSegment(segment,): boolean {
  ...
  const rawArchive = await readCorpusFile({ pin: fixedPin, relPath, },);
  /** Exact initial normalization, not a later revised archive candidate. */
  const archiveText = passArchiveText({ text: rawArchive, l: rl, },);
  /** Repository-wide shallow state is deliberately unsupported for naming authority. */
  const shallow = (await archiveGitOutput({ pin: fixedPin, relPath,
    args: ['rev-parse', '--is-shallow-repository'],
    ...(signal === undefined ? {} : { signal, }), },)).trim();
  if (shallow === 'true') {
  ...
  const porcelain = await archiveGitOutput({ pin: fixedPin, relPath, args: [
    'blame', '--line-porcelain', '--no-textconv', '--no-progress', '--ignore-revs-file', '',
    fixedPin.commitSha, '--', relPath,
  ], ...(signal === undefined ? {} : { signal, }), },);
  /** Every current line is checked against the pinned read before scope mapping. */
  const origins = archiveBlameOrigins({ porcelain, archiveText: rawArchive, relPath, },);
  /** Exact initial observations joined to unique normalized pinned lines. */
  const scoped = archiveNamingScopes({ archiveText, uses: initialUses,
    references: corroborated.references, origins, relPath, },);
  /** Each required origin is read once; root and merge histories remain withheld. */
  const history = await readArchiveNamingHistories({ scopes: scoped.scopes, pin: fixedPin, relPath,
    ...(signal === undefined ? {} : { signal, }), },);
  /** Final whole-occurrence qualification, still distinct from translation policy. */
  const qualified = qualifyArchiveNamingScopes({ scopes: scoped.scopes,
    histories: history.histories, pin: fixedPin, relPath, },);
  rl.info(`qualified ${String(qualified.revisions.length,)} naming revisions for ${relPath}`,);
  return { revisions: qualified.revisions,
    findings: [...corroborated.findings, ...scoped.findings, ...history.findings, ...qualified.findings], };
}
