...
export function applyFootnoteRelabel(
  { text, map, }: { readonly text: string; readonly map: readonly FootnoteLabelRewrite[]; },
): string {
  if (map.length === 0)
    return text;
  ...
  const pieces = markers.map(function piece(marker, index,): string {
    /** End of the preceding copied marker, or start of the document. */
    const start = markers[index - 1]?.endOffset ?? 0;
    ...
    const replacement = destination === undefined || normalizeFootnoteIdentifier({ identifier: destination, },) === marker.identifier
      ...
      : `[^${destination}]`;
    return `${text.slice(start, marker.startOffset,)}${replacement}`;
  },);
  /** Trailing bytes include every definition body and final line ending. */
  const suffix = text.slice(markers.at(-1,)?.endOffset ?? 0,);
  /** Candidate remains local until the resulting syntax graph is verified. */
  const rewritten = [...pieces, suffix,].join('',);
  if (rewritten === text)
    return text;
  ...
  if (after.length !== markers.length || markers.some(function differs(marker, index,): boolean {
