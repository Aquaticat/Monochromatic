...
import { listCorpusPeople, type CorpusPin, } from './corpus-source.ts';
...
import { preparationRootRegistrations, preparationRootUnalignedDefinitions, } from './preparation-root-registration.ts';
import type { PreparationRootRegistration, PreparationRootUnalignedDefinitions, } from './preparation-root-registration-model.ts';
...
export type PreparationRootPopulation = {
  ...
  readonly observations: readonly { readonly entryId: string; readonly kind: 'section-alignment'; readonly finding: AlignmentFinding; }[];
 ...
 * ```
 */
export async function readPreparationRootPopulation({ pin, selection, l, }: {
  ...
  const pl = tagged({ tag: readPreparationRootPopulation.name, l, },);
  ...
  if ((new Set(listedEntryIds,).size !== listedEntryIds.length)
    ...
    throw new PreparationRootError({ kind: 'corpus-identity', input: 'people/', },);
  /** No sampler is invoked; these exact identities define the selected part of the current population. */
  const selectedParentIds = new Set(selection.parents.map(function identity(parent,): string { return parent.parentId; },),);
  /** Only selected complete entries can contribute definition-only dependencies. */
  const selectedEntryIds = new Set(selection.parents.map(function identity(parent,): string { return parent.entryId; },),);
  ...
  const observations: { readonly entryId: string; readonly kind: 'section-alignment'; readonly finding: AlignmentFinding; }[] = [];
  ...
  for (const entryId of listedEntryIds) {
    /** This entry's parsed documents do not escape into unrelated population records. */
    const current = await readPreparationRootEntry({ pin, entryId, l: pl, },);
    ...
    population.push(...current.parents.map(preparationRootPopulationParent,),);
    observations.push(...current.entry.alignmentFindings.map(function observation(finding,): { readonly entryId: string; readonly kind: 'section-alignment'; readonly finding: AlignmentFinding; } {
      return { entryId, kind: 'section-alignment', finding, };
    },),);
    ...
    for (const parent of current.parents) {
      ...
      if (selectedParents.has(parent.id,))
        throw new PreparationRootError({ kind: 'population', input: parent.id, },);
      selectedParents.set(parent.id, parent,);
    }
    registrations.push(...preparationRootRegistrations({ entryId, source: current.source, target: current.target, pairs: current.pairs, selectedParentIds, },),);
    /** Only nonempty unaligned inventories need a namespace record. */
    const namespace = preparationRootUnalignedDefinitions({ entryId, source: current.source, target: current.target, pairs: current.pairs, },);
    if ((namespace.source.length > 0) || (namespace.target.length > 0))
  ...
  const parents = selection.parents.map(function selected(frozen,): PreparationRootParent {
    /** Current parent identity is looked up under the original frozen spelling. */
    const parent = selectedParents.get(frozen.parentId,);
    if (parent === undefined)
      throw new PreparationRootError({ kind: 'population', input: frozen.parentId, },);
    return parent;
  },);
  /** Current registration identities must not merge distinct native parent occurrences. */
  const byParent = new Map(registrations.map(function keyed(record,): readonly [string, PreparationRootRegistration] { return [record.parentId, record,]; },),);
  if (byParent.size !== registrations.length)
    throw new PreparationRootError({ kind: 'population', input: 'initial parent registry', },);
  ...
  const registry = [
    ...
    ...registrations.filter(function dependency(record,): boolean { return !selectedParentIds.has(record.parentId,); },),
  ];
  ...
  return { listedEntryIds, rawDocuments, excluded, eligibleEntries: eligibleEntryIds.length, population, entries, parents, observations, registry, unalignedDefinitions, };
}
