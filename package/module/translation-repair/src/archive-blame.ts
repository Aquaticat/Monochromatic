...
export function isArchiveGitObjectId(value: string,): boolean {
  return (value.length === SHA1_WIDTH || value.length === SHA256_WIDTH)
    && Array.from(value,).every(function hexadecimal(character,): boolean {
...
function originHeader({ line, relPath, }: {
  ...
  const originalLine = archiveGitInteger({ text: fields[1] ?? '', relPath, },);
  /** Pin's one-based line. */
  const finalLine = archiveGitInteger({ text: fields[2] ?? '', relPath, },);
  /** Optional count is still validated even though each line is emitted separately. */
  const count = archiveGitInteger({ text: fields[3] ?? '1', relPath, },);
  ...
  if (!isArchiveGitObjectId(commit,) || originalLine === 0 || finalLine === 0
    || count === 0 || fields.length < minimumFields || fields.length > maximumFields) {
    throw new ArchiveNamingEvidenceError({ kind: 'history-shape', relPath, },);
  }
  return { commit, originalLine, finalLine, filename: undefined, boundary: false, ignored: false, };
}
...
export function archiveBlameOrigins({ porcelain, archiveText, relPath, }: {
  ...
  for (const line of porcelain.split('\n',)) {
    if (pending === undefined) {
      if (line !== '')
        pending = originHeader({ line, relPath, },);
      continue;
    }
    if (line.startsWith('\t',)) {
      /** Document line, distinct from metadata that may share its words. */
      const text = line.slice(1,);
      if (pending.filename === undefined || pending.finalLine !== origins.length + 1
        || text !== lines[pending.finalLine - 1]) {
        throw new ArchiveNamingEvidenceError({ kind: 'history-shape', relPath, },);
      }
      origins.push({ ...pending, filename: pending.filename, text, },);
      pending = undefined;
      continue;
    }
