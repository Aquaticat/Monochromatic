...
export function readPreparationEvidenceOccurrence({ expected, receipt, sourceText, targetText, l, }: {
  ...
  const { source, target, pair, alignmentFindings, } = readRegisteredPreparationParent({ expected: fixed, sourceText, targetText, l: pl, },);
  ...
  return structuredClone({ scope: 'receipt-bound-evidence', expected: fixed, source, target, pair, alignmentFindings,
