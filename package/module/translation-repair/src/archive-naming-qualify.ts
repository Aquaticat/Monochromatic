 ...
 * ```
 */
export function qualifyArchiveNamingScopes({ scopes, histories, pin, relPath, }: {
  ...
  const revisions = scopes.flatMap(function qualify(scope,): readonly QualifiedArchiveNamingRevision[] {
    ...
    const matching = history.hunks.filter(function covers(hunk,): boolean {
      return hunk.newStart <= scope.origin.originalLine
        && scope.origin.originalLine < hunk.newStart + hunk.newCount;
    },);
    /** Strictly unique textual replacement. */
    const hunk = matching.length === 1 ? matching[0] : undefined;
    if (hunk === undefined || hunk.removed.length !== 1 || hunk.added.length !== 1) {
      findings.push(`archive-revision-withheld (${scope.reference.use.anchor.nodeId}: no unique single-line replacement)`,);
      return [];
    }
    ...
    if (previousRaw === undefined || currentRaw === undefined || currentRaw !== scope.origin.text)
    ...
    if (scope.startOffset + change.start !== anchor.startOffset
      || scope.startOffset + change.start + change.added.length !== anchor.endOffset
    ...
    if (current.slice(0, change.start,) + change.removed
      + current.slice(change.start + change.added.length,) !== previous) {
      throw new ArchiveNamingEvidenceError({ kind: 'history-shape', relPath, },);
    }
    return [{ archiveCommit: pin.commitSha, archivePath: relPath,
      originCommit: scope.origin.commit, parentCommit: history.parentCommit,
      archiveHash: scope.reference.use.archiveHash, anchor,
      useKind: scope.reference.kind,
      reading: { configuredModelIds: scope.reference.use.configuredModelIds,
        ballots: scope.reference.use.ballots, },
      currentLine: scope.lineNumber, pinnedLine: scope.origin.finalLine,
      originLine: scope.origin.originalLine, currentLineStartOffset: scope.startOffset,
      currentLineText: current, previousLineText: previous,
      previousFragment: change.removed, currentFragment: change.added, }];
  },);
