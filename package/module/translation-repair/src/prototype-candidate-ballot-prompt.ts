// PROTOTYPE ONLY: Candidate I manifest-bound author and candidate-scoped prompts.
...
/**
 * Canonical Candidate I author protocol identity.
 */
export const CANDIDATE_BALLOT_AUTHOR_PROTOCOL_DIGEST: string = hashContent({
  content: JSON.stringify({
    system: CANDIDATE_BALLOT_AUTHOR_SYSTEM,
    packetMarker: 'CANDIDATE_BALLOT_AUTHOR_PACKET:',
...
/**
 * Canonical Candidate I verifier protocol identity.
 */
export const CANDIDATE_BALLOT_VERIFIER_PROTOCOL_DIGEST: string = hashContent({
  content: JSON.stringify({
    system: CANDIDATE_BALLOT_VERIFIER_SYSTEM,
    packetMarker: 'CANDIDATE_BALLOT_VERIFIER_PACKET:',
...
function assertKeys({
  ...
  if (JSON.stringify(Object.keys(value,),) !== JSON.stringify(expected,))
    throw new Error('candidate ballot canonical prompt packet shape differs');
}
 ...
 * @example
 * ```ts
 * assertCandidateBallotMedia({ media, manifest, });
 * ```
 */
export function assertCandidateBallotMedia({
  media,
  manifest,
}: {
  ...
  if ((JSON.stringify(actual,) !== JSON.stringify(manifest.sourcePictures,))
    ...
    throw new Error('candidate ballot prompt media binding differs from manifest');
}
 ...
 * @example
 * ```ts
 * const messages = candidateBallotAuthorMessages({
 *   plan,
 *   manifest,
 *   shell,
 *   ledger,
 ...
 * ```
 */
export function candidateBallotAuthorMessages({
  plan,
  manifest,
  shell,
  ledger,
  ...
  return [
    {
      role: 'system',
      content: CANDIDATE_BALLOT_AUTHOR_SYSTEM,
    },
    {
      role: 'user',
      content: promptContent({
        text: `CANDIDATE_BALLOT_AUTHOR_PACKET:\n${JSON.stringify(packet,)}`,
        media,
      },),
    },
  ];
 ...
 * @example
 * ```ts
 * const messages = candidateBallotVerifierMessages({
 *   manifest,
 *   shell,
 *   ledger,
 *   candidate,
 ...
 * ```
 */
export function candidateBallotVerifierMessages({
  manifest,
  shell,
  ledger,
  candidate,
  ...
  return [
    {
      role: 'system',
      content: CANDIDATE_BALLOT_VERIFIER_SYSTEM,
    },
    {
      role: 'user',
      content: promptContent({
        text: `CANDIDATE_BALLOT_VERIFIER_PACKET:\n${JSON.stringify(packet,)}`,
        media,
      },),
    },
  ];
