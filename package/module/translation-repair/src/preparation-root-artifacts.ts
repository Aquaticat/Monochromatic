...
import type { PreparationRootReference, PreparationRootReferenceBinding, PreparationRootReferenceRole, } from './preparation-root-reference-model.ts';
import type { MatchedPreparationArtifact, PreparationSelectionEvidence, } from './preparation-selection-evidence-model.ts';
...
export type PreparationRootArtifacts = {
  ...
  readonly frame: (input: { readonly file: string; readonly hash: string; readonly consumer: string; readonly expectedText: string; }) => void;
  /** Returns an owned parsed note only after the journal's path/hash relationship matches. */
  readonly note: (input: { readonly path: string; readonly hash: string; readonly consumer: string; }) => Readonly<Record<string, unknown>>;
 ...
 * ```
 */
export function preparationRootArtifacts({ evidence, l, }: {
  ...
  const pl = tagged({ tag: preparationRootArtifacts.name, l, },);
  /** Original ordering is part of the supported frozen producer format. */
  const [pool, journal, priorJournal,] = evidence.artifacts;
  if ((pool === undefined) || (journal === undefined) || (priorJournal === undefined))
    throw new PreparationRootError({ kind: 'reference-role', },);
  /** The byte matcher already established exact unique path membership. */
  const byPath = new Map(evidence.artifacts.map(function keyed(artifact,): readonly [string, MatchedPreparationArtifact] { return [artifact.path, artifact,]; },),);
   ...
   * ```
   */
  function attribute({ artifact, role, consumer, }: { readonly artifact: MatchedPreparationArtifact; readonly role: PreparationRootReferenceRole; readonly consumer: string; },): void {
    ...
    if (existing.some(function duplicate(binding,): boolean { return (binding.role === role) && (binding.consumer === consumer); },))
      return;
    bindings.set(artifact.path, [...existing, { role, consumer, },],);
  }
  ...
  function textOf(artifact: MatchedPreparationArtifact,): string {
    try {
      ...
      throw new PreparationRootError({ kind: 'reference-role', input: artifact.path, },);
    }
  }
  ...
  function jsonOf(artifact: MatchedPreparationArtifact,): Readonly<Record<string, unknown>> {
    try {
      return preparationRootRecord(JSON.parse(textOf(artifact,),),);
    }
    catch (error) {
   ...
   * ```
   */
  function frame({ file, hash, consumer, expectedText, }: { readonly file: string; readonly hash: string; readonly consumer: string; readonly expectedText: string; },): void {
    /** Both the journal's exact byte identity and its filename must select one original artifact. */
    const candidates = evidence.artifacts.filter(function matching(artifact,): boolean {
      return (artifact.hash === hash) && (artifact.path.slice(artifact.path.lastIndexOf('/',) + 1,) === file);
    },);
    /** Ambiguity cannot be resolved by choosing the first matching artifact. */
    const [artifact,] = candidates;
    if ((candidates.length !== 1) || (artifact === undefined) || (textOf(artifact,) !== expectedText))
      throw new PreparationRootError({ kind: 'reading-provenance', input: consumer, },);
    attribute({ artifact, role: 'complete-entry-reading-frame', consumer, },);
  }
   ...
   * ```
   */
  function noteOf({ path, hash, consumer, }: { readonly path: string; readonly hash: string; readonly consumer: string; },): Readonly<Record<string, unknown>> {
    ...
    attribute({ artifact, role: 'reading-note', consumer, },);
    return record;
  }
  ...
  function referencesOf(): readonly PreparationRootReference[] {
    return evidence.artifacts.map(function reference(artifact,): PreparationRootReference {
      return { path: artifact.path, hash: artifact.hash, bytes: artifact.bytes,
        bindings: (bindings.get(artifact.path,) ?? [{ role: 'opaque-selection-support', consumer: 'selection', },]).map(function owned(binding,): PreparationRootReferenceBinding { return { ...binding, }; },), };
    },);
  }

  attribute({ artifact: pool, role: 'policy-pool', consumer: 'selection', },);
  attribute({ artifact: journal, role: 'reading-journal', consumer: 'selection', },);
  attribute({ artifact: priorJournal, role: 'prior-reading-journal', consumer: 'selection', },);
  return { pool: jsonOf(pool,), journal: jsonOf(journal,), priorJournal: jsonOf(priorJournal,), priorJournalHash: priorJournal.hash, frame, note: noteOf, references: referencesOf, };
}
