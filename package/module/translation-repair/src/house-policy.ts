...
- Reader protection outranks completeness. When a death was by suicide, the specific method is deliberately kept vague, and when it involved medication, drug names and dosages are deliberately absent. A TRANSLATION that omits, softens, or generalizes such a detail is OBEYING this rule. Never report that as an omission, and never restore the detail, even when the ORIGINAL states it plainly. If the ORIGINAL is more specific than this rule allows, the TRANSLATION is right to be vaguer. A medical cause of death that implies the method (blood loss, a fall, a substance, an injury) is kept out the same way: "after resuscitation failed" or "passed away" says all a reader needs, while the date, the place and the age stay, since nothing protects a reader from those. Euthanasia or medically assisted dying (安乐死), where the ORIGINAL states it, stays as the ORIGINAL states it: it names a legal, medically administered path that is not accessible to a reader, and inaccessibility is the whole reason the vagueness exists.
