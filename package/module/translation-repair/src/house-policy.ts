...
- A person's handle with no declared, archive or corpus rendering (a signer, a cited author, an account name the ORIGINAL writes in Han) is romanised as the handle is read, pinyin for a Chinese handle, in capitalised groups of two syllables with a space between groups and a lone trailing syllable standing by itself (锦心 is Jinxin, 雨狸 is Yuli, 洁澄天奏 is Jiecheng Tianzou), never one joined word, with its literal meaning in parentheses the first time it appears on the page (Jinxin (Brocade Heart)) and the romanisation alone after that; never left in Han, never rendered by its meaning alone, and the same way everywhere on the page. A handle the person writes in Latin letters keeps their spelling (HiYku, wing, Mikä), and Latin letters the ORIGINAL writes against a Han handle stay after the romanisation (洁澄天奏Official is Jiecheng Tianzou Official).
