...
import type { BoundedCandidate, BoundedVerifierResponse, } from './prototype-bounded-verdict-model.ts';
...
function longestSlot({ candidate, }: {
...
}): { readonly slotKey: string; readonly text: string; } {
  const entries = Object.entries(candidate.slots,).toSorted(function length(
    left,
    right,
  ) {
  ...
  if ((selected === undefined) || (selected[1].length < 3))
    throw new Error('bounded envelope candidate lacks three anchor characters');
  return { slotKey: selected[0], text: selected[1], };
}

/** Builds three disjoint one-character anchors at highest offsets. */
function maximalAnchors({ candidate, }: {
  readonly candidate: BoundedCandidate;
}): readonly RealizationTargetAnchor[] {
  const { slotKey, text, } = longestSlot({ candidate, });
  return [0, 1, 2,].map(function anchor(distance,) {
    const startOffset = text.length - 3 + distance;
...
export function maximalBoundedVerifierResponse({
  ...
  const response: BoundedVerifierResponse = {
    candidates: candidates.map(function row(candidate,) {
      ...
      return {
        ...
        obligationStatuses: ledger.obligations.map(function preserved() {
          return 'p' as const;
        },),
        globalStatuses: REALIZATION_GLOBAL_CRITERIA.map(function defect(
          ...
          return index <= BOUNDED_VERDICT_FINDING_CAP ? 'd' as const : 'c' as const;
        },),
        overflow: true,
        findings: Array.from(
          { length: BOUNDED_VERDICT_FINDING_CAP, },
          function finding(_value, index,) {
            return {
              scope: 'g' as const,
              manifestIndex: index,
  ...
  if (!boundedVerifierResponseGuard({ ledger, candidates, })(response,))
