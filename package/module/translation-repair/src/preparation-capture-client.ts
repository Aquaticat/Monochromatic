 ...
 * ```
 */
export function preparationCaptureClient({ provider, transport, }: {
  readonly provider: ProviderName;
  readonly transport: ModelTransport;
},): ModelCaller {
  /** Capture changes delivery retries only; native model payload defaults stay intact. */
  const options = { apiKey: 'preparation-request-capture-only', transport, retryPolicy: { limit: 0, baseMs: 1, }, };
  ...
  if (provider === 'bedrock')
    return createBedrockClient({ ...options, ledger: CAPTURE_LEDGER, },);
