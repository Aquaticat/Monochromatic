 ...
 * ```
 */
export function nativePreparationClients({
  transports,
  retryPolicy,
}: {
   ...
   */
  const options = {
    apiKey: 'independent-native-fixture-only',
    retryPolicy,
  };
  return {
    synthetic: createSyntheticClient({
      ...options,
      transport: transports.synthetic,
    },),
    hyper: createHyperClient({
      ...options,
      transport: transports.hyper,
    },),
    bedrock: createBedrockClient({
      ...options,
      transport: transports.bedrock,
      ledger: FIXTURE_LEDGER,
    },),
    openrouter: createOpenRouterClient({
      ...options,
      transport: transports.openrouter,
    },),
  };
}
