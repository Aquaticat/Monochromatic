...
export function footnoteRewriteMap(
  { map, markers, }: { readonly map: readonly FootnoteLabelRewrite[]; readonly markers: readonly ActiveFootnoteMarker[]; },
  ...
  for (const move of map) {
    ...
    const marker = gfmMarkerAt({ text: syntax, offset: 0, },);
    if (marker === undefined || marker.endOffset !== syntax.length || marker.rawLabel !== move.to)
    ...
    if (earlier === undefined)
      lookup.set(key, move.to,);
  }
  /** Existing identities include unmoved occupants, not just map destinations. */
  const before = new Set(markers.map(function identity(marker,): string { return marker.identifier; },),);
