...
export async function readPreparationRootEntry({
  ...
  if (inherited.kind === 'whole-page') {
    ...
    return {
      ...
      exclusion: {
        entryId,
        kind: 'production-whole-page-original',
        archiveHash: read.archive
          .identity
          .effectiveHash,
        noteHash: hashContent({ content: inherited.note, },),
      },
      rawDocuments,
    };
  }
  ...
  if (normalized.kind === 'whole-page') {
    ...
    return {
      ...
      exclusion: {
        entryId,
        kind: 'normalized-whole-page-original',
        archiveHash: read.archive
          .identity
          .effectiveHash,
        targetHash: target.documentHash,
        noteHash: hashContent({ content: normalized.note, },),
      },
      rawDocuments,
    };
  }
  ...
  return {
    ...
    entry: {
      ...
      sourceText: read.source
        .text,
      archiveText: read.archive
        .text,
      targetText: archive.text,
      ...
      targetHash: target.documentHash,
      originalPolicy: {
        inherited: inherited.kind,
        normalized: normalized.kind,
        spans,
      },
      archiveLines: archive.lines,
      sourceFindings: source.parseFindings,
      targetFindings: target.parseFindings,
