...
import { nodeConfig, nodeExternal, } from '@monochromatic-dev/config-rolldown/.node.ts';
 ...
 * ```
 */
async function packageVersion({ path, expectedName, }: {
  ...
  if ((typeof value !== 'object') || (value === null) || Array.isArray(value,)
    || !('name' in value) || (value.name !== expectedName) || !('version' in value)
 ...
 * ```
 */
async function binaryIdentity(path: string,): Promise<{ readonly bytes: number; readonly sha256: string; }> {
  ...
  for await (const chunk of handle.createReadStream({ start: 0, end: before.size - 1, highWaterMark: HASH_CHUNK_BYTES, autoClose: false, },)) {
  ...
  return { bytes: observed.bytes, sha256: hash.digest('hex',), };
}
...
const nativePath = createRequire(satteriEntry,).resolve(NATIVE_PACKAGE,);
/** JavaScript package version is evidence from its installed manifest. */
const satteriVersion = await packageVersion({ path: join(dirname(satteriEntry,), '..', 'package.json',), expectedName: 'satteri', },);
/** Native package must match the JavaScript package, not a stale generated version string. */
const nativeVersion = await packageVersion({ path: join(dirname(nativePath,), 'package.json',), expectedName: NATIVE_PACKAGE, },);
  ...
  generateBundle: {
    ...
    handler(_options, bundle,): void {
      ...
      const files = Object.values(bundle,).filter(function executableChunk(item,): boolean {
        ...
        return { path: item.fileName, bytes: Buffer.byteLength(item.code, 'utf8',), sha256: createHash('sha256',).update(item.code,).digest('hex',), };
      },);
      /** Native bytes belong to the same application inventory as JavaScript. */
      const native = { path: NATIVE_FILE, bytes: nativeBytes.length, sha256: createHash('sha256',).update(nativeBytes,).digest('hex',), };
      this.emitFile({ type: 'asset', fileName: NATIVE_FILE, source: nativeBytes, },);
      ...
      const manifest = {
        ...
        node: { version: process.version, versions: process.versions, executable, },
        native: { package: NATIVE_PACKAGE, version: nativeVersion, ...native, },
        loaderEnvironment: { policy: 'must-be-absent-before-import', names: LOADER_ENVIRONMENT, },
        systemLibraries: 'Runner must separately bind its operating-system image and native shared-library inputs.',
        files: [...files, native,].toSorted(function byPath(left, right,): number {
          return left.path < right.path ? -1 : left.path > right.path ? 1 : 0;
        },),
      };
      this.emitFile({ type: 'asset', fileName: 'sealed-runtime.json', source: JSON.stringify(manifest, null, 2,), },);
      l.info(`sealed build described ${String(manifest.files.length,)} application files without review or execution authority`,);
    },
  },
};
...
const sealedConfig: ReturnType<typeof nodeConfig> = { ...config, plugins: [config.plugins, sealedRuntimeAssets,], };
