 ...
 */
const SOURCE_MARKER = `${sep}package${sep}module${sep}conf-fork${sep}src${sep}`;
...
/**
 ...
 script outside the package) is never a target. Unit test files
 (`*.unit.test.ts`) are excluded through their `.test.` marker, and the
 test-only helper `test-support.ts` is excluded explicitly: it is unreachable
 from the package's public API and is covered by its own unit test and the
 mutation run instead.
...
/**
 ...
 @example
 ```ts
 packageRelative('file:///repo/package/module/conf-fork/src/store-file.ts');
 // => 'store-file.ts'
 ```
 */
