...
/**
 ...
 ```
 */
export function invokeCollectionFactory({ invocation, target, owner, restoring, }: {
  ...
  try {
    ...
    guardCollectionFakes({ value: result, previous, owner, restoring, },);
    return result;
  }
  catch (error) {
    /** Retained partial members must share lifetime guards even after their rollback. */
    guardCollectionFakes({ value: target, previous, owner, restoring, },);
    /** Each introduced member has independent cleanup, including accessor spies. */
    const steps = collectionFakes({ value: target, previous, },).map(function partialRestorer(fake: object,): () => void {
      /** Capture before another member's restoration changes the destination descriptor. */
      const restore: unknown = Reflect.get(fake, 'restore',);
      return function restorePartialFake(): void {
        if (typeof restore === 'function')
          Reflect.apply(restore, fake, [],);
      };
    },);
    try {
      ...
      throw new SandboxCleanupError([error, rollbackError,], `${invocation.operation} failed and partial replacement restoration failed`,);
    }
    throw error;
  }
}
