...
/**
 ...
 ```
 */
export function requireUnownedProperty({ target, key, operation, }: {
  readonly target: object;
  readonly key: PropertyKey;
  readonly operation: string;
},): void {
  /** Aliases and inherited accessors must not bypass the shared lease. */
  const slot = findMethodSlot({ target, key, },);
  if (slot !== undefined) {
...
/**
 ...
 ```
 */
export function requireUnownedObject({ target, operation, }: {
  readonly target: object;
  readonly operation: string;
},): void {
  ...
  while (cursor !== null && cursor !== Object.prototype) {
    ...
    for (const key of Reflect.ownKeys(cursor,))
      requireUnownedProperty({ target: cursor, key, operation, },);
