...
import { isSandboxTarget, requireUnownedProperty, } from './sandbox-target.ts';
...
/**
 ...
 ```
 */
export function guardFakeMutators({ value, owner, restoring, target, key, contextualMethod = false, }: {
  ...
  function guardResult(current: unknown,): void {
    ...
    for (const operation of ['get', 'set', 'value', 'restore',] as const) {
      ...
      const method: unknown = Reflect.get(current, operation,);
      if (typeof method !== 'function')
        continue;
      Reflect.set(current, operation, new Proxy(method, {
    ...
    for (const selector of ['onCall', 'onFirstCall', 'onSecondCall', 'onThirdCall',] as const) {
      /** These public selectors expose the root stub through Sinon's behavior object. */
      const method: unknown = Reflect.get(current, selector,);
      if (typeof method !== 'function')
        continue;
      Reflect.set(current, selector, new Proxy(method, {
        apply(original: typeof method, receiver: unknown, args: unknown[],): unknown {
