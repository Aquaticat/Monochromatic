...
import { isSandboxTarget, sandboxPropertyKey, SINON_VALIDATES_PROPERTY, } from './sandbox-value.ts';
...
/**
 ...
 ```
 */
export function guardCollectionFakes({ value, owner, restoring, }: {
  ...
  for (const property of Reflect.ownKeys(value,)) {
    ...
    const member: unknown = Object.getOwnPropertyDescriptor(value, property,)?.value;
    if (typeof member !== 'function' || Reflect.get(member, 'isSinonProxy',) !== true)
      continue;
    /** Sinon records target metadata on descriptor-changing stubs, but not on detached spies. */
    const target: unknown = Reflect.get(member, 'rootObj',);
    /** Only primitive keys can be preflighted without coercion. */
    const key = sandboxPropertyKey(Reflect.get(member, 'propName',),);
    guardFakeMutators({
      value: member,
      owner,
      restoring,
...
/**
 ...
 ```
 */
export function guardTimerController({ value, owner, restoring, }: {
  readonly value: unknown;
  readonly owner: SandboxOwner;
  readonly restoring: () => boolean;
},): void {
  ...
  for (const property of ['restore', 'uninstall', 'setTickMode',] as const) {
    ...
    wrappers.set(method, guarded,);
    Reflect.set(value, property, guarded,);
  }
}
