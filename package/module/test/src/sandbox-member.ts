...
/**
 ...
 ```
 */
export function collectionFakes({ value, previous = {}, }: {
  readonly value: unknown;
  readonly previous?: PropertyDescriptorMap;
},): readonly object[] {
  if (!isSandboxTarget(value,))
    return [];
  return Reflect.ownKeys(value,).flatMap(function collectPropertyFakes(property: PropertyKey,): object[] {
    /** Reading descriptors does not execute application accessors. */
    const descriptor = Object.getOwnPropertyDescriptor(value, property,);
    return (['value', 'get', 'set',] as const).flatMap(function collectDescriptorFake(field,): object[] {
      ...
      if (typeof member !== 'function' || member === previous[property]?.[field]
        || Reflect.get(member, 'isSinonProxy',) !== true)
        return [];
      return [member,];
    },);
  },);
