/**
 Tests for the core cache surface: storage, presence, deletion, member
 shape, and the string renderings.
 
 Every case here pins observable behavior of upstream `quick-lru` 7.3.0,
 including its duplicate-key accounting and its non-enumerable
 class-prototype member flags, so the fuzz sidecar's differential oracle
 can compare both implementations directly.
  ...
  children: [
    ...
    it({
      ...
      fn: async () => {
         ...
         */
        const value = function greet(): string {
          return 'hi';
        };
        lru.set({
          key: 'fn',
    ...
    it({
      ...
      fn: async () => {
        ...
        expect(inspectable[Symbol.for('nodejs.util.inspect.custom',)](),).toBe(lru.toString(),);
      },
    },),
