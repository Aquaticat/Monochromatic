...
**Spec mode**: When in spec mode, do not draft a spec or attempt to exit spec mode until the user explicitly asks for it. Continue researching, investigating, and gathering context as needed.
**Act, don't annotate**: When a change clearly belongs somewhere specific -- a different file, a new file, a gitignore entry -- move it there immediately. Research the target location if unfamiliar, create files, and update supporting config as needed. A comment saying "this should be elsewhere" is not a substitute for putting it there. When unsure, propose a concrete edit and location rather than vaguely noting something could be improved.
**Managed files**: Some root-level files (e.g. `CLAUDE.md`) are generated by file-enforcer. Before editing any root config file, check `file-enforcer.config.ts` to see if it is a managed output -- if so, edit the source file instead and run file-enforcer.
**"Educated guess" means research first**: When the user asks for opinions, guesses, or analysis, search for evidence before responding. Check web sources, session history, or codebase as appropriate. An informed opinion backed by even brief research is worth far more than speculation presented as analysis.
**Run what you build**: After creating or modifying runnable code -- scripts, packages, tests, CLI tools -- always execute it to verify it works. Building without running is an incomplete task.
**Hooks and automation must be loop-safe**: When creating hooks, scheduled tasks, or any automation that spawns new agent sessions (e.g. `droid exec`), always consider whether the spawned session will re-trigger the same hook. Add an explicit guard (transcript size check, env var flag, session type filter) to break the cycle. An unguarded recursive hook can burn tens of thousands of tokens in seconds.
