...
Always use the current date from the system prompt environment.
...
When asked for opinions, guesses, or analysis, search for evidence before responding.
Check web sources, session history, or codebase as appropriate.
...
When a change clearly belongs somewhere specific -- a different file, a new file, a gitignore entry -- move it there immediately.
Research the target location if unfamiliar, create files, and update supporting config as needed.
When unsure, propose a concrete edit and location.

## Run what you build

After creating or modifying runnable code -- scripts, packages, tests, CLI tools -- always execute it to verify it works.

## Hooks and automation

When creating hooks, scheduled tasks, or any automation that spawns new agent sessions (e.g. `droid exec`), add an explicit guard (transcript size check, env var flag, session type filter) to break the cycle.
An unguarded recursive hook can burn tens of thousands of tokens in seconds.
