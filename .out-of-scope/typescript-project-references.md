...
not the emitted `.d.ts` artifacts, so cross-package type-checking already
...
- declaring `composite: true` everywhere, which forces `declaration: true` and
...
- handling TS6310 (`Referenced project may not disable emit`) by overriding `noEmit: false`
  in every consumer, which then leaves stray `.d.ts` files in source trees;
...
(~70 packages, dense workspace dep graph). The performance payoff is small because the
underlying problem is already mitigated by tsgo's source-import resolution.

## Prior issue

- #123 (closed 2026-05-14): proposed adopting project references; the user rejected the
...
- `tsgo` (a Go reimplementation of `tsc`) reads source via the package `exports` map's
  `./ts` style entries (`"./ts": "./src/index.ts"`), so cross-package type-checks are
  fast even without emitted `.d.ts` artifacts.
- Per-package `mise run //packages/<path>:lint:types` invokes type-checking directly.
- `composite: true` is set in the base `tsconfig.json` per the comment in
  `packages/config/typescript/README.md`, but no package actually wires up references.

If the no-references performance ever degrades to the point where references look attractive
again, revisit by measuring the type-check time first, not by re-proposing the migration
