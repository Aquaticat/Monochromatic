...
- Rust unit tests live in a co-located sidecar file, never an inline `#[cfg(test)] mod tests { ... }`. For a source file `foo.rs`, put the tests in a sibling `foo_tests.rs` and declare it from the bottom of `foo.rs` with `#[cfg(test)] #[path = "foo_tests.rs"] mod tests;`. The sidecar stays the `tests` child of `foo`, so `use super::*` and access to `foo`'s private items carry over unchanged; this mirrors the TS `foo.unit.test.ts` co-location and parks test bulk in the linter-exempt `*_tests.rs` file. The one exception is a test file that exercises several sibling modules at once (`packages/cli/forbidden-strings/src/rules/algebra_tests.rs` spans `atom` and `extract`): it has no single-file home, so it is declared in the parent `mod` instead, and the `seed-from-tests` corpus generator hardcodes those `rules/*_tests.rs` paths.
...
- Rust packages run tests through `cargo nextest run` (the package's `test` mise task), not `cargo test`; nextest comes from the root mise `cargo:cargo-nextest` tool, inherited by child config roots. The two podman-built desktop apps (`music-player`, `terminal`) install nextest inside their `Containerfile`, so a `Containerfile` edit needs `mise run //packages/desktop-app/<app>:image` to rebuild the image before the `test` task finds the binary. The `fuzz/forbidden-strings` `test` task is scoped to `cargo nextest run --lib`: the libFuzzer target bins only link under cargo-fuzz's injected RUSTFLAGS, so a full-target run fails to build them. nextest skips doctests, but the Rust crates have none (the `// ```ts` blocks live in `//` comments, not `///` doc comments).
