...
`private` field presence) is already covered by [AUDIT.consistency.md](./AUDIT.consistency.md);
...
- Build, lint, and test config files copy-pasted with drift
- Package manifest fields (`exports`, `devDependencies`) repeated verbatim across packages
...
- Single-occurrence helpers in non-shared packages whose shape is generic
  ...
  can reuse them instead of writing their own copy. These do not violate DRY today
  but encode the same future-DRY-violation risk: the second caller is going to
  reinvent them. Section: "Reusable utilities awaiting extraction".

Excluded by construction:

- Files generated by [`file-enforcer.config.ts`](./file-enforcer.config.ts):
  `CLAUDE.md`, `mise.toml` at repo root, `forbidden-strings.local.txt`,
  `forbidden-strings.append.local.txt`, `.factory/skills/*/*.md`, `.claude/skills/*/*.md`
- `packages/test-fixture/*` where duplicate inputs are the point of the fixture
- `packages/duik/teto-generated/*` (generated by sibling `teto`)
- Cross-runtime `*.browser.ts` / `*.node.ts` variants where the platform forces separate code
- `pnpm-lock.yaml` and other lockfiles

## Methodology

Per category, files were enumerated, hashed with `sha256sum`, and bucketed by hash to find
exact byte-level duplicates. Near-duplicates were sampled and diffed pairwise.
For source-level duplication, `rg` queries enumerated function definitions, type aliases,
const declarations, regex patterns, and shared filenames; the most-frequent occurrences
...
(`format*`, `parse*`, `escape*`, `normalize*`, `encode*`, `decode*`, `is*`,
`require*`, `ensure*`, `truncate*`, `chunk*`, etc.), occurrence counts captured,
and each candidate's body inspected to confirm primitive-in / primitive-out shape
and absence of package-specific dependencies. The current `module/es` taxonomy
...
Every finding cites file paths and where applicable line numbers, so claims can be
verified by opening the file. Numeric counts in headings are exact occurrence counts at
audit time, not estimates.

## Findings

The findings are grouped: build configuration first, then large structural duplication
(package forks), then function-level duplication, type-level duplication, constant-level
duplication, file-template duplication, and miscellaneous patterns.
...
- `packages/module/hyperscript/mise.toml` declares `build:js:browser`; only
  `tsdown.config.ts` is present
- `packages/figma-parsers/kiwi/mise.toml` declares `build:js:browser` and
  `build:js:node`; only `tsdown.config.ts` is present
- `packages/figma-parsers/penpot/mise.toml` declares `build:js:browser` and
  `build:js:node`; only `tsdown.config.ts` is present

Empirical effect (verified by inspecting `dist/final/`): each contains only `neutral/`
and `types/`, never `node/` or `browser/`. The declared task names misrepresent what the
build produces.

Suggested resolution: drop the unused per-platform task children from the three
`mise.toml` files, or add the matching suffixed config files if the per-platform output
is wanted.
...
`@monochromatic-dev/config-tsdown/.client.ts`. Two are byte-identical
(`packages/webapp-productivity/rss/tsdown.client.config.ts` and
`packages/webapp-search/exa-search/tsdown.client.config.ts`); the remaining five differ
...
- `src/server/pages/layout.ts`: 148 lines each, 12-line diff
- `src/lib/db.ts`: 82 vs 85 lines, 30-line diff
- `src/server/api/timer.ts`: roughly 100-line diff
- `src/server/pages/inbox.ts`: 11-line diff
- `src/server/pages/in-progress.ts`: 11-line diff
...
- `packages/pi/terminal-title/src/formatter-utils.ts`: 219 lines
- `packages/claude-code-plugins/source/src/handlers/terminal-title/formatter-utils.ts`: 154 lines
- 132 lines of diff between them

Plus parallel files:

- `index.ts` (219 vs 158 lines)
- `tool-titles.ts` (115 vs 82 lines, 160-line diff)
- `title-builder.ts` (179 lines, no exact peer; the claude-code-plugins fork splits it
...
- `stringField`, `shortPath`, `shortCommand`, `truncate`

Suggested resolution: extract the formatter and title-builder into a new
...
The four scenarios (`force-push.ts`, `wide-service.ts`, `hot-repo.ts`,
...
- `function readConfig(): <ScenarioConfig> { ... }` (one per scenario, same shape)

Suggested resolution: lift the constants and the `readConfig` shape into
...
JS+CSS), differing only in package name and the specific assets bundled:
...
- `packages/dev-script/inference-canary-viewer/src/build.ts` (199 lines, with extras)

`paper2vn` and `syllable-break-demo` differ by only 69 lines of diff out of 90/76 lines
total: roughly 70% of `build.ts` is structural copy-paste between them.
...
Suggested resolution: extract a `build-single-page-app(...)` helper into
...
each declare `function mount(root: HTMLElement,): void`. The bodies are screen-specific,
but the entry-point pattern (a single `mount` per screen) is duplicated six times.

Suggested resolution: factor a `Screen` interface and a registration table; each module
...
Each parses `--port=` from `process.argv` (via `getArgumentValue`), falls back to
`process.env.PORT`, and finally to `DEFAULT_PORT`.

Suggested home: `packages/module/es/src/path/` already hosts cross-package helpers; add
...
A fifth file, `packages/webapp-forge/stress/src/scenarios/shared.ts:getFlag`,
declares the same logic under a different name (`getFlag` instead of
`getArgumentValue`).

The `lib/args.ts` docblocks even cite each other as "mirrors", showing the duplication
is acknowledged. Three of the four packages use `@optique` elsewhere for richer CLI
parsing, so this single helper exists only because no shared module hosts it.

Notable: `webapp-forge/stress` and `webapp-forge/server` are sibling packages under
`webapp-forge/`. The fact that `stress/src/scenarios/shared.ts` already exists as a
local extraction but `webapp-forge/server` does not import from it shows the
within-`webapp-forge/` boundary is itself porous.

Suggested home: `packages/module/es/src/cli/get-argument-value.ts`.
...
Suggested home: a small `packages/module/es/src/web/` route helper, since the function
...
Suggested home: `packages/module/es/src/types/t object/` already houses similar guards;
...
- `packages/pi/terminal-title/src/formatter-utils.ts`
- `packages/claude-code-plugins/source/src/handlers/terminal-title/formatter-utils.ts`
- `packages/webapp-content/messages-demo/src/server/api/messages.ts`
- `packages/webapp-content/messages-demo/src/server/api/drafts.ts`

Suggested home: `packages/module/es/src/types/t object/`.

#### `parseStringArray` is reimplemented in four files

Two shapes (one parses JSON-encoded strings, one validates `unknown` input):
...
Suggested resolution: hoist the validation form into `task-validation` shared with
`done-h-css-test`; hoist the JSON-decode form into the shared `lib/db/`.
...
Suggested resolution: import from `@monochromatic-dev/module-hyperscript/ts` (the
canonical source) in the two consumers.

#### `renderPage` is reimplemented in seven files

The signature varies, but each returns an HTML document around app-specific content:
...
Suggested resolution: extract a `renderHtmlShell({title, body, ...})` helper into
...
Suggested resolution: hoist into `packages/module/es/src/web/json-response.ts`. Note that
the wrapper itself is one line; the duplication exists because each module wants
...
The two `done` forks share a signature; the others diverge. Listed for completeness;
...
The three functions together form a complete database-path bootstrap routine. With the
current spread, every webapp that needs a SQLite path reinvents the path resolution.

Suggested resolution: extract a `resolveSqliteDatabase({ defaultPath })` helper into
...
`Database` instance. The migration content differs per app; the runner is generic.

Suggested resolution: factor a `runMigrations(database, migrations)` helper into the
...
Already covered as part of the paper2vn fork finding; listing here for completeness in
...
Both return the absolute path of the named binary or `null`. The two implementations
likely differ by Windows-specific PATHEXT handling.

Suggested resolution: keep the platform-specific copy if needed but expose a single
public entry point that dispatches; otherwise consolidate.
...
Each represents an OpenAI-compatible message envelope with `role` and `content`. Five
implementations track to four different LLM client patterns.

Suggested resolution: a `packages/module/llm-types/` package or a section in
`packages/module/es/src/types/`. Useful given the recurring openai/anthropic/openrouter
client pattern.
...
Same justification as `ChatMessage`. The two `done`-fork copies fold into the fork
consolidation.
...
Both protocols use JSON-RPC 2.0 wire format; the type definitions are the same
substring.

Suggested resolution: extract `packages/module/json-rpc/` (or place under
...
Three copies per fork. Folds into the fork consolidation.
...
The two webapp-forge copies are intra-package; the two done copies fold into the fork
consolidation. Two intra-package webapp-forge copies should be unified directly.
...
Two pairs (server-side and client-side type for the same payload). Folds into the
fork consolidation.
...
Suggested resolution: extract a `packages/module/typesafe-i18n-types/` package, or
...
Each cited the others in its docblock as the inspiration. The first two are
byte-near-identical; they should share the export.

Suggested resolution: extract a `packages/module/highlight-groups/` package exporting a
...
`task-card-render.ts`) and the corresponding paths under `done-h-css-test/`. All fold
...
Each plugin entry-point file is 26 to 38 lines, follows the same template:
...
Per-file diff is ~12 lines (handler name in 4 imports, command alias in the example
JSON, plugin description in the docblock). Most of the file is repeated docblock
(`@example` JSON config, `@module`).

The runtime `runHookPlugin` already centralizes the dispatch logic; the shells exist
only because each marketplace-installable plugin needs its own `dist/final/node/index.mjs`
output. So the duplication is per-build-target, not per-feature.

Suggested resolution: a `dev-script/file-enforcer` rule that generates the seven
`index.ts` files from a single template plus a per-plugin manifest (handler name,
command alias, hook event, description). The shells then exist as generated files (like
...
(`defineTool`, `runStdioServer`, etc.) from it. No JSON-RPC plumbing is reimplemented
...
Already covered in the type-level section. The JSON-RPC base could host this triple
...
The TROUBLESHOOTING.* tree (46 files) groups by leading segment as follows
...
build, bundle, caching, infrastructure, runtime, monitoring. Each child file has a
distinct topic; the duplication is only in the heading conventions
(`# X Performance Todo` / `## Y Optimization`) which is structural, not behavioral.
Listed as a positive finding.

### Numeric and string constant duplication

> Note: this section mixes two related-but-distinct categories. **Redeclaration**
> (multiple files declare the same name with the same value) is straightforward DRY:
> consolidate into a single export. **Name collision with mixed values** (multiple
> files declare the same name with *different* values) is a probable bug surface:
> the resolution is renaming or value alignment rather than consolidation. Bullets
...
Suggested home: a `packages/module/es/src/time/units.ts` exporting `MS_PER_SECOND`,
...
Suggested home: `packages/module/es/src/types/t string/t parse-int/`.
...
All 10 are inside `packages/webapp-productivity/done-h-css-test/src/client/`, mostly in
...
A single import from a sibling module would dedupe all 10. The cause is the project
convention "magic literals as named const (exception: -2 through 2)": each style file
imports from the module-hyperscript CSS helpers but cannot import a const from a sibling
without a circular import in some cases, so each file declares its own.

Suggested home: a `packages/webapp-productivity/done-h-css-test/src/client/constants.ts`
local module, or hoist into `module-hyperscript`.

#### `BYTES_PER_KIB = 1_024` declared under five names in 13 files

Already in scope. Names:

- `BYTES_PER_KIB` (6): `runtime-error/bun/src/oom.ts`,
  `webapp-content/messages-demo/src/{client/composer, lib/markdown-stream, lib/seed,
  server/pages/edit, server/pages/message}.ts`
- `KIB` (3): `cli/mvm/src/download-progress.ts`,
  `webapp-forge/server/src/server/routes/git.cli.unit.test.ts`,
  `webapp-forge/stress/src/scenarios/force-push.ts`
- `BYTES_PER_KB` (2): `desktop-daemon/{editord/src/constants.ts,
  hall-monitor/src/cycle.ts}`
- `BYTES_PER_UNIT` (1): `dev-script/page-weight/src/index.ts`
- `BYTES_PER_KILOBYTE` (1): `test-fixture/file-enforcer-perf/src/validate-benchmarks.ts`

Suggested home: `packages/module/es/src/types/t number/t bytes/`.
...
These are forced by the project rule "compose fractional values from -2 to 2". The
constants themselves can still be exported once.

Suggested home: `packages/module/es/src/types/t number/t fraction/`.
...
- `packages/webapp-forge/server/src/storage/adapter-s3.ts` (HTTP_NOT_FOUND, HTTP_OK)
...
Suggested home: `packages/module/es/src/web/http-status.ts`.

#### `DEFAULT_PORT` declared in 8 packages with 4 different values

- `packages/webapp-forge/server/src/index.ts`: 3000
- `packages/webapp-productivity/done/src/server.ts`: 3000
- `packages/webapp-productivity/done-h-css-test/src/server.ts`: 3000
- `packages/webapp-content/messages-demo/src/server.ts`: 3000
- `packages/desktop-daemon/editord/src/server/index.ts`: 4400
- `packages/webapp-search/ai-tree/src/index.ts`: 4111
- `packages/webapp-search/exa-search/src/port.ts`: 4115
- `packages/webapp-productivity/rss/src/port.ts`: 4112

Four packages use the same default (3000) and would collide if running together. The
overlap is undocumented and a likely UX bug.

Suggested resolution: a port-allocation document or registry. For DRY purposes, no
shared module is needed; the constant is per-app by design.

#### `DEFAULT_DATABASE_PATH` declared in 5 files

- `packages/webapp-content/messages-demo/src/lib/db.ts`: `./data/messages-demo.db`
- `packages/webapp-forge/server/src/lib/auth.ts`: `./data/forge.db`
- `packages/webapp-forge/server/src/data/db.ts`: `./data/forge.db` (duplicated within
  webapp-forge-server)
- `packages/webapp-productivity/done-h-css-test/src/lib/db.ts`: `./data/done.db`
- `packages/webapp-productivity/done/src/lib/db.ts`: `./data/done.db`
...
Six files in one package each declare the same constant. A single
...
Suggested home: `packages/webapp-forge/seed/src/lib/constants.ts` (a single shared
...
Same value, same name; folds into the stress-scenarios shared shell finding.

#### `STYLES` const declared in 23 places

23 files declare `const STYLES = [...]` or `const STYLES = $({...})`. These are mostly
in `desktop-daemon/editord/src/client/` and `webapp-productivity/done-h-css-test/`.
Each is intra-package and represents a per-component style block, so this is a name
collision, not a DRY violation.

Listed for completeness so the next audit does not re-flag it.

#### `LINE_HEIGHT` declared with mixed values in 4 files

- `packages/desktop-daemon/editord/src/client/editor/editor-pane.styles.ts`: `(2 + 1) / 2`
...
Three of four use `4`; one uses `8`. Should converge on a single per-popup constants
module.

#### `FONT_WEIGHT_NORMAL = 400` and `FOCUS_OFFSET = -(1/2/2/2)` declared 3+ times in done-h-css-test

`FONT_WEIGHT_NORMAL` appears in three component style files; `FOCUS_OFFSET` in four.
...
helpers (`hasArgument`). The docblocks self-acknowledge the duplication.
...
Both define the same 7 HTTP status code constants. The second file's docblock cites
...
Each defines `DEFAULT_DATABASE_PATH`, `normalizeDatabasePath`, `resolveDatabasePath`,
`ensureDatabaseDirectoryExists`, plus the `Database` instance constructor.
...
Suggested resolution: extract the `*.css` declaration into a shared
`@monochromatic-dev/config-typescript/env.d.ts` that downstream consumers
`/// <reference types="@monochromatic-dev/config-typescript/env" />`. The distinct
...
`backup-path, inference-canary-viewer, paper2vn, cli/git`, plus all 7
...
Resolution path is not obvious: pnpm has no "default devDependencies" mechanism. The
catalog already deduplicates version pins. A generator script (in
`packages/dev-script/file-enforcer/`) could enforce one of the three shapes per package
type, but this is informational rather than actionable today.

#### Vendored copy of `pi-coding-agent` helpers in morph-compact

`packages/pi/morph-compact/src/pi-utils.ts:1-485` reimplements `convertToLlm` and
...
heavy transitive runtime closure including AWS Bedrock SDK, chalk, marked,
cli-highlight). Listed for completeness so future audits do not re-flag it.

The maintenance cost is real: when upstream changes either function, the vendored copy
must be updated by hand. Worth tracking in
[`TROUBLESHOOTING.pi-safeguard.md`](./TROUBLESHOOTING.pi-safeguard.md).

### Self-acknowledged inline copies

These findings are duplications where the file's own docblock cites the source. Each
one is ready to consolidate today; the inline comment is a marker the author left
...
182 lines (webapp-forge) vs 217 lines (ssg-test); 101-line diff. Both define
`isSafeHtml`, `isDangerousHtml`, the `SafeHtml` type, and the JSX-to-HTML conversion
loop.

Rationale ("each surface can pin its own escape semantics") is recorded but no
divergence has occurred yet, so the design rationale is paying dividends in
"flexibility we have not used" while costing maintenance now.

Suggested resolution: consolidate into a `@monochromatic-dev/module-jsx-to-html`
package with optional escape-semantics overrides; if escape semantics genuinely diverge
later, swap the override.
...
which `messages-demo` does not have. Suggesting consolidation into a
...
`packages/webapp-forge/server/src/data/db.ts`. The auth.ts docblock acknowledges this:
...
Both files exist within `packages/webapp-forge/server/`, so the natural fix is to lift
...
For completeness, the following self-acknowledged mirror comments were found and are
...
Suggested resolution: migrate the four hold-outs to `nano-spawn`.
...
Listed as a minor inconsistency; not a DRY violation but worth normalizing during a
drive-by edit.
...
- `type QuestionProps`, `type QuestionOption`
- `const HEX_RADIX = 16` (already noted)
- The render skeleton

Suggested resolution: extract a `<QuestionGroup type="radio|checkbox">` component
that takes the input shape as a discriminator. The radio and checkbox specifics
become two short adapters.

#### `cli/vmsync/src/config.ts` and `cli/mvm/src/config.ts` share `validateName` and config shape

Both files export `validateName(name: string): void`. The two CLIs (vmsync = VM file
synchronizer; mvm = VM manager) are siblings under `cli/` and have similar config
needs (a name, a directory, validation rules).

Sizes: 286 (vmsync) vs 84 (mvm); 315-line diff.

Suggested resolution: lift the shared `validateName` into a `cli/vm-shared/` package
or into `module/es`. Both packages already depend on workspace utilities.
...
Different domains, different signatures. Listed for completeness; this is name
...
`packages/webapp-productivity/done-h-css-test/src/lib/db-migrations.ts`: 15-line diff
out of ~104 lines. Same `tasks` / `attachments` / `settings` tables with the same
schema, same migration order. Folds into the `done` fork finding.
...
- Declare local `BYTES_PER_KIB`, `DECIMAL_RADIX`, `PREVIEW_MAX_LENGTH`
...
shells are duplicated within a single package. Folds into the `isRecord`,
`stringField`, and `DECIMAL_RADIX` findings; listed here so reviewers can group the
work.
...
Same start-stop-reset timer endpoints. Folds into the fork finding.
...
Four files; two pairs (create + update) per fork side. Folds into the fork finding.

### Test patterns

#### `describe`/`it` test framework imports from a single canonical source

All 144 test files import from `@monochromatic-dev/module-test`. No `bun:test` or
external `vitest`/`jest` import appears. Listed as a positive finding and already-shared
category.
...
file) is consistent across all 12 callers. No `fileURLToPath(import.meta.url)` variant
exists in test files. Listed as a positive finding.

### Reusable utilities awaiting extraction

These helpers live in a single non-shared package today, so they are not DRY violations
yet. Each is shaped like a library function (primitive in, primitive out, no domain
types, no hidden dependency on package-specific state), and the next module that needs
...
storage quota, log files) will want this function.

Suggested resolution: extract to `module/es` next to the byte-size constants the audit
already prescribes a home for. Caller in `cli/mvm/src/download-progress.ts` becomes
a one-line import.
...
thousands separators). Both are shape-identical to general utilities; both will be
needed by any future package that renders timing metrics or counts.

Suggested resolution: move to `module/es` alongside `formatBytes`. The viewer keeps
the call site, drops the local definition.
...
`SECONDS_PER_DAY` constants. Same algorithm, different format string set. Could be
unified as `formatRelative({ deltaSeconds, format: 'ago' | 'compact' })` or as two
separate functions sharing the threshold helper.

Suggested resolution: extract the threshold-ladder logic to `module/es`; let each
caller pick its own format. Companion fix to the duration-constant duplication
...
Both `export function nowIso(): string { return new Date().toISOString(); }`. This is
within the done/done-h-css-test fork already documented and resolves when the fork
consolidates. The fold-out-to-`module/es` case is weak: the body is a single stdlib
call; a single named entry point adds little over inlining `new Date().toISOString()`
at each callsite. Listed here only so a future audit does not re-flag the
duplication after the fork merges; the DRY violation itself is captured under the
...
sub-namespace. Two-way conversion lives next to its inverse, the constants are shared,
and the unit table (G, M, K) is parsed in one place.
...
Bodies are identical except for the variable name `concat` vs `concatChunks`; the
inputs and outputs are byte-for-byte the same. This is also a DRY violation that the
function-level audit missed (the function-level pass surfaced the count but not the
unification).

Suggested resolution: a single export from `module/es`, named `concatBytes` to
distinguish from `Array.prototype.concat`. All six call sites become single-line
imports.
...
`crypto.randomUUID()` when available, otherwise a `${Date.now()}-${random}` synthetic
ID. The same fallback shape will be wanted by any browser-side package that needs an
ID generator and cares about older runtimes (the codebase's `firefox140` baseline does
support `crypto.randomUUID`, but the fallback is still useful for SSR or non-browser
contexts).

Suggested resolution: extract to `module/es`. `messages-demo` drops the local copy.

#### Test-only `uniqueId(tag: string): string` repeated in six test files

The same closure-counter pattern, identical body, in:
...
Suggested resolution: ship `uniqueId({ prefix: string }): string` from
`@monochromatic-dev/module-test`. All six test files become one-line imports plus the
prefix string literal.

#### Inline `function isDefined(x): x is T { return x !== undefined; }` filter callbacks

Anonymous-named filter-callback typeguards repeating the same body, in seven files:
...
- `packages/pi/morph-compact/src/pi-utils.ts`

Each occurrence is `.filter(function isDefined(x): x is T { return x !== undefined; })`.
The standard library has no built-in narrowing typeguard for this; everyone writes
their own. The function is the canonical "I am writing this for the third time, why
isn't it shared" candidate.

Suggested resolution: export `isDefined<T>(x: T | undefined): x is T` from
`module/es`. All seven sites become `.filter(isDefined)`. The narrowing returns
through the imported signature, so callers get the type refinement without writing
...
Suggested resolution: extract `readJson` (localStorage variant) to a shared module
under a distinct name like `readLocalStorageJson` or `readPersistedJson` to avoid the
collision with the fetch variant. The fetch variant (one-line wrapper) is too thin
to extract.
...
Both byte-identical: read the `<script id="page-data">` element, throw if missing,
JSON-parse the textContent. This pattern (server emits page-scoped data as an inline
script tag, client reads it back) is a generic SSR-to-client handoff convention,
...
exports a typed `querySelector` that throws on null. The pattern is the workaround
for the TypeScript narrowing-into-function-declarations limitation, documented
verbatim in `TROUBLESHOOTING.typescript.md` as the canonical fix. Once a workaround
is documented in the troubleshooting file, the next session that hits the bug will
write a fresh local copy unless a shared helper exists to import.

Suggested resolution: add `requireElement<T extends Element>(selector: string): T`
to `module/dom`. Update `TROUBLESHOOTING.typescript.md` to point at the import path
...
Suggested resolution: a single `ensureDirForFile(filePath)` plus an `ensureDir(dirPath)`
in `module/es` (the path/ subdirectory already exists). Three call sites collapse to
imports.
...
captures only as a body-level duplicate, not as a fold-out target. The shape is
generic: name, fallback, decimal radix.

Suggested resolution: bundle `getFlag`, `intFlag`, `boolFlag`, `stringFlag` into a
...
covers. The body is generic ("if longer than max, slice and append `…`") and would
be useful in any package that emits text into a fixed-width display surface (CLI
tables, terminal titles, status lines).

Suggested resolution: extract from the terminal-title fork into `module/es` rather
than just consolidating the fork. The downstream value is bigger.
...
`text.padEnd(width)` directly. The named-params convention does not apply to method
...
Both use a `GLOB_META` regex to find the first metacharacter, then partition. Tuple
shapes differ slightly (file-enforcer also returns the `originalPrefix`), but the
core logic is the same. Any package that needs to plumb glob patterns through `cwd`
boundaries will want this; the next caller will write a third copy.

Suggested resolution: extract to `module/es/path/glob.ts` (sibling to the existing
`path/` helpers). Both call sites import; the file-enforcer site adds the
...
for number utilities). Generalize to `roundTo(value, decimals)`; provide
...
Suggested resolution: extract to `module/es/types/t string/...` next to other string
helpers. Provide `escapeForSingleQuoted` and `escapeForDoubleQuoted` variants since
...
rejecting; it is a domain-specific specialization, but it does so by reimplementing
the timer wiring rather than by composing with the shared utility.

Suggested resolution: rewrite the local site to compose with the shared
`withTimeout`: `await withTimeout({ promise, ms: PROBE_TIMEOUT_MS, label: 'probe' })
.catch(() => false)`. The local function declaration goes away; the timer logic
stays in one place.
...
factor through `@monochromatic-dev/mcp-stdio`; the cli/{mvm, vmsync} pair has no
analogous shared base. This is both a reusable-utility candidate and a DRY violation
...
`Object.fromEntries` shim to match the per-call shape. The `editord` site predates
or ignores `Map.groupBy` and writes its own loop. Different result containers
(`Record` vs `Map`), but the same operation underneath.

Suggested resolution: ship a single `groupBy<T, K>({items, keyFn}): Map<K, T[]>` from
`module/es` (returning `Map` rather than `Record` so non-string keys work). Provide
a thin `groupByToRecord` companion for the `Record<string, T[]>` callers. The five
sites collapse to four `groupBy(...)` calls plus one `groupByToRecord` call; each
...
(formatBytes, parseMemoryToBytes, isDefined, randomId, formatMs, formatNumber,
concatBytes, round1, encodeS3Key, escapeString, splitGlob, ensureDir, groupBy).
`module/test` is the home for the test-only `uniqueId({ prefix })`. `module/dom`
gets `requireElement` and `readPageData`. The spawn-tagged helper plausibly lives in
`module/logger` (since the value-add is the tagged log line) or as its own tiny
package alongside other I/O-shaped helpers.

A useful test for whether each fold-out is right-sized: does the helper contain any
package-specific assumption (a hardcoded constant tied to the source package, an
import from the source package, a call to a logger tagged with the package name)?
If yes, the extraction needs a config parameter; if no, the extraction is a direct
move. Most candidates above are direct moves.

### Already-shared categories

These show heavy structural duplication, but the duplication is the consequence of an
existing shared base. Listed here so they are not flagged in future audits.
...
naming taxonomy (`t` = type, `f` = function, `r s` = return sync, `p p` = positional
parameters, etc., as documented in module-es's README).

Listed as a positive finding (already-shared category) so the next audit does not
re-flag them. A code generator could produce them, but the gain is small.

#### Per-package `mise.toml` files (81 total)

The largest cluster of 7 byte-identical files contains only `extends = "..."` re-exports
of root-level task templates. 17 packages share one of two stub shapes (7 + 7 + 3). The
duplication is structural and unavoidable: each package needs to opt into the task
...
[AUDIT.consistency.md](./AUDIT.consistency.md).
