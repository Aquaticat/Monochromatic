...
Custom geometric typeface. Regular weight, uppercase only (A--Q, X--Z).

## Build pipeline

1. Letter shapes are authored as SVG paths in a single glyph strip (`src/glyphs.svg`)
2. `src/build-font.ts` parses the strip, extracts per-cell paths, expands stroked
   outlines into filled contours, and assembles an OpenType font via opentype.js
