//! Decoding: turn an audio file (or an Android `content://` file descriptor) into
//! interleaved `f32` PCM samples.
//!
//! One demux path (symphonia probes the container and demuxes packets) feeds two
//! decode paths: symphonia's own decoders for FLAC/WAV/MP3/Vorbis/AAC/ALAC, and the
//! `opus` crate (libopus) for Opus, because the symphonia 0.6 meta-crate exposes no
//! Opus decoder (the `symphonia-codec-opus` crate exists but is not wired into the
//! `all` feature set).
//!
//! Three entry points all funnel into `open_media_source`, which probes and routes:
//! `open()` takes a filesystem `&Path` (used by desktop-style callers and tests);
//! `open_borrowed_fd()` takes a raw OS file descriptor handed over by Android (a
//! `content://` `ParcelFileDescriptor`) and is the path the on-device engine uses;
//! `open_media_source()` is the shared tail that both build on. Every returned
...
//           types. A file descriptor is just a small integer the OS uses to name an
//           open file/socket. `RawFd` is a plain type alias for `i32` (the raw integer
//           itself; siblings a reader might expect: `u32` would be wrong because the OS
//           uses `-1` to mean "no fd"). `BorrowedFd<'fd>` is a SAFE wrapper that says "I
//           am only LOOKING at this fd, and I will NOT close it when I'm done"; its
//           sibling `OwnedFd` is the opposite ("I own this fd and WILL close it on
//           drop"). The `'fd` is a lifetime: it ties the borrow to how long the real
//           owner stays alive.
// Why:      Android hands us a bare integer fd that the JVM still owns; we must borrow
//           (not own) it, then duplicate it into our own `OwnedFd` so we never close
//           the JVM's copy.
// TS map:   no clean equivalent — mentally: `RawFd` is `type RawFd = number;` and
//           `BorrowedFd` is "a number plus a promise to the compiler that I won't free
//           it". TS/Node has no fd-ownership concept; the GC and libuv hide all of this.
//
// In TS you'd write (pseudocode):
// ```ts
...
// Why:      `open` takes `&Path` because it only reads the path, it does not need to own it.
...
//           under `formats`.
// Why:      We pass the file extension so probing is fast and reliable (and pass an EMPTY
//           hint on the fd path, where no filename exists).
...
//           types. `MediaSource` is the TRAIT (interface) symphonia reads raw bytes
//           through: anything that can be read AND seeked (a `File` qualifies). `MediaSourceStream`
//           is the buffered wrapper symphonia layers on top of a `MediaSource`.
// Why:      `open_media_source` accepts any `Box<dyn MediaSource>` (a file, or an fd
//           wrapped in a `File`) and feeds it to a `MediaSourceStream` for the probe.
// TS map:   `MediaSource` is like a `{ read(); seek(); }` interface; `MediaSourceStream`
//           is a `ReadableStream`-like adapter around it.
//
// In TS you'd write (pseudocode):
// ```ts
...
// Why:      `open_media_source()` constructs an `OpusSource` when the track is Opus.
...
// Why:      Callers (engine, UI, tests) need rate + channels to configure the audio output
//           and the seek/position bar's total length.
...
pub struct AudioSpec {
    ...
    //           choice.
    // Why:      The audio output needs the native rate to set up the stream.
    // TS map:   `rate: number`.
    //
    // In TS you'd write (pseudocode):
    // ```ts
    ...
    //           choice.
    // Why:      Interleaving and the output layout both need the channel count.
...
// Why:      Open a file by path, build a hint from its extension, then hand it to the shared
//           probe/route tail. Used by path-based callers and tests; the on-device engine
//           uses `open_borrowed_fd` instead.
...
pub fn open(path: &Path) -> Result<Box<dyn Source>, PlayerError> {
    ...
    // What:     `open_media_source(Box::new(file), hint)`. `Box::new(file)` heap-allocates
    //           the `File` and ERASES its concrete type to the `dyn MediaSource` trait object
    //           the tail wants. No trailing `;`, so this is the function's tail expression and
    //           its `Result` becomes our return value.
    // Why:      Delegate the probe-and-route work to the one shared place, passing the file as
    //           a generic media source plus the extension hint.
    // TS map:   `return openMediaSource(file, hint);`
    //
    // In TS you'd write (pseudocode):
    // ```ts
...
//           The shared probe-and-route tail. `source` is an owning, heap, type-erased media
//           source (a `File`, or an fd wrapped in a `File`); we take it BY VALUE (moved in)
//           because the stream below consumes it. `hint` is also moved in. `Box<dyn Source>`
//           is the single-owner boxed decoder (siblings `Rc`/`Arc` would be shared; one owner
//           suffices).
// Why:      Both the path entry (`open`) and the fd entry (`open_borrowed_fd`) need identical
//           probe + track-routing logic; keeping it here means the format detection lives in
//           exactly one place. The fd path carries no filename, so its caller passes an empty
//           `Hint` and relies on symphonia's content sniffing.
// TS map:   `function openMediaSource(source: MediaSource, hint: Hint): Source { ... }`
//
// In TS you'd write (pseudocode):
// ```ts
...
pub fn open_media_source(
    ...
    // What:     `MediaSourceStream::new(source, Default::default())`. `source` is the already
    //           boxed media source, moved in. `Default::default()` builds the default options
    //           struct (type inferred from the arg).
    // Why:      symphonia reads bytes through this buffered stream wrapper.
    // TS map:   `const mss = new MediaSourceStream(source, {});`
    //
    // In TS you'd write (pseudocode):
    // ```ts
    // const mss = new MediaSourceStream(source, {});
    // ```
...
// What:     `pub fn open_borrowed_fd(fd: RawFd) -> Result<Box<dyn Source>, PlayerError>`.
//           `fd: RawFd` is a bare OS file-descriptor integer (`RawFd` is an alias for `i32`;
//           Android obtained it via `ParcelFileDescriptor.getFd()`). `Box<dyn Source>` is the
//           single-owner boxed decoder (siblings `Rc`/`Arc` shared; one owner suffices).
// Why:      The on-device engine plays `content://` audio that has no real filesystem path:
//           the JVM hands us an open fd it still owns. We DUPLICATE that fd into our own
//           descriptor (an `OwnedFd`) so our side owns only the copy and never closes the
//           JVM's original. Closing the JVM's original would be a deterministic fdsan
//           `SIGABRT` on API 30+ (the PFD fdsan-tags its fd). The caller must pass `fd >= 0`
//           (`BorrowedFd::borrow_raw` panics on -1) and call this SYNCHRONOUSLY while the PFD
//           is still alive, so the dup lands before Kotlin's `use {}` closes the original.
// TS map:   no clean equivalent — mentally: Node hands you a numeric fd and the GC/libuv
//           own its lifetime; here we must manually `dup()` so we don't accidentally close
//           someone else's file. `function openBorrowedFd(fd: number): Source { ... }`.
// Gotcha:   Passing `fd == -1` PANICS (a hard crash), not a recoverable error; the contract
//           is "caller guarantees fd >= 0". And the dup is mandatory: borrowing the fd and
//           skipping the dup would mean we read through a descriptor the JVM may close out
//           from under us.
//
// In TS you'd write (pseudocode):
// ```ts
// function openBorrowedFd(fd: number): Source {
//   const dup = duplicateFd(fd);           // our own copy; closing it never touches the JVM's
//   const file = fileFromFd(dup);
...
pub fn open_borrowed_fd(fd: RawFd) -> Result<Box<dyn Source>, PlayerError> {
    ...
    // What:     `let borrowed = unsafe { BorrowedFd::borrow_raw(fd) };`. `BorrowedFd::borrow_raw`
    //           wraps the raw integer `fd` into a safe `BorrowedFd` value (one that will NOT
    //           close the fd on drop; sibling `OwnedFd::from_raw_fd` WOULD take ownership and
    //           close it). `unsafe { ... }` is an escape hatch promising the compiler we have
    //           checked a soundness invariant it cannot verify itself: that `fd` is a real,
    //           open descriptor and stays alive for the duration of this borrow (the SAFETY
    //           comment above states exactly why that holds). `borrow_raw` also PANICS if
    //           `fd == -1`.
    // Why:      We need a typed, ownership-aware view of the JVM's fd so we can duplicate it
    //           on the next line without ever taking ownership of (and thus closing) it.
    // TS map:   no clean equivalent — mentally: `const borrowed = wrapFdWithoutOwning(fd);`,
    //           inside a block where you have personally guaranteed `fd` is open. TS has no
    //           `unsafe` keyword because TS never lets you touch raw OS handles like this.
    // Gotcha:   `unsafe` does NOT mean "this is dangerous magic"; it means "the compiler trusts
    //           ME here, so the SAFETY comment is load-bearing". Violating that promise is
    //           undefined behaviour, not a catchable exception.
    //
    // In TS you'd write (pseudocode):
    // ```ts
    // const borrowed = wrapFdWithoutOwning(fd); // we promise fd is open + alive
    // ```
    let borrowed = unsafe { BorrowedFd::borrow_raw(fd) };

    // What:     `let owned = borrowed.try_clone_to_owned()?;`. `try_clone_to_owned()` asks the
    //           OS to DUPLICATE the borrowed descriptor (a `dup`/`F_DUPFD_CLOEXEC` syscall),
    //           returning `Result<OwnedFd, io::Error>` — a brand-new fd that WE own and that
    //           closes itself when dropped (sibling `BorrowedFd`, which `borrowed` is, owns
    //           nothing). `?` unwraps the `OwnedFd` or returns the i/o error.
    // Why:      Our side must own an independent fd: when the JVM later closes its original
    //           (Kotlin's `use {}` block ends), our `owned` copy stays valid for the whole
    //           lifetime of the returned `Source`.
    // TS map:   no clean equivalent — mentally: `const owned = duplicateFd(borrowed);` where
    //           `owned` is a fresh handle that you, not the JVM, are responsible for closing.
    // Gotcha:   `try_clone_to_owned` is a real syscall and CAN fail (out of descriptors); that
    //           is why it returns a `Result` and we propagate with `?`.
    //
    // In TS you'd write (pseudocode):
    // ```ts
    // const owned = duplicateFd(borrowed); // throws on failure; we now own this copy
    // ```
    let owned = borrowed.try_clone_to_owned()?;

    // What:     `let file = File::from(owned);`. `File::from(...)` is a conversion constructor
    //           that CONSUMES the `OwnedFd` and turns it into a `std::fs::File` (the `File`
    //           now owns the fd and will close it on drop; the `owned` binding is moved away
    //           and can no longer be used). Sibling conversions exist via `From`/`Into`, but
    //           `File::from(OwnedFd)` is the canonical one.
    // Why:      symphonia reads bytes through a `MediaSource`, and `std::fs::File` implements
    //           `MediaSource` (it can read AND seek); wrapping the dup'd fd in a `File` lets us
    //           reuse the same probe/route tail as the path-based `open`.
    // TS map:   `const file = fileFromFd(owned);` — a `File` object backed by our fd.
    //
    // In TS you'd write (pseudocode):
    // ```ts
    // const file = fileFromFd(owned);
    // ```
    let file = File::from(owned);

    // What:     `open_media_source(Box::new(file), Hint::new())`. `Box::new(file)` heap-allocates
    //           the `File` and ERASES its type to the `dyn MediaSource` trait object the tail
    //           wants. `Hint::new()` is an EMPTY hint (no extension to give). No trailing `;`,
    //           so this is the tail expression and its `Result` is our return value.
    // Why:      A regular on-disk file (which a `content://` fd backs) is a seekable media
    //           source; hand it to the shared probe/route tail. We pass an empty hint because
    //           an fd carries no filename, so we rely on symphonia's byte-content sniffing.
    // TS map:   `return openMediaSource(file, new Hint());`
    //
    // In TS you'd write (pseudocode):
    // ```ts
...
//           this type.
// Why:      So `open_media_source()` can return it as `Box<dyn Source>`.
// TS map:   `class SymphoniaSource implements Source { ... }`
//
// In TS you'd write (pseudocode):
// ```ts
