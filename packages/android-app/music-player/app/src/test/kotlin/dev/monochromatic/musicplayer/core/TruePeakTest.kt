...
//           `import { normalizationGain, catmullRom, measureTruePeak, CEILING, HALF } from
//           ".../core/TruePeak"`, made implicit by the same-package rule.
//
...
// TS map:   `import { abs } from "...";` — though in practice TS uses the built-in `Math.abs`,
...
// TS map:   `describe("TruePeak", () => { ... })`; the helper would be a plain function inside it.
//
// In TS you'd write (pseudocode):
// ```ts
// describe("TruePeak", () => {
...
class TruePeakTest {
    ...
    //           rounding error.
    // TS map:   `function approxEq(a: number, b: number): boolean { ... }` — TS has only `number`,
    //           so `Float` maps to `number`.
    //
    // In TS you'd write (pseudocode):
    // ```ts
    ...
    private fun approxEq(a: Float, b: Float): Boolean {
        ...
        // TS map:   `const tolerance = 1e-4;` — TS allows the same scientific notation; `number`
        //           erases the Float/Double distinction.
        ...
        // TS map:   `return Math.abs(a - b) < tolerance;` — Kotlin's free `abs(x)` is TS's
        //           `Math.abs(x)`.
        //
        // In TS you'd write (pseudocode):
        // ```ts
    ...
    fun normalizationGainLeavesSilenceUnchanged() {
        ...
        // TS map:   `expect(approxEq(normalizationGain(0.0), 1.0)).toBe(true);`
        //
        // In TS you'd write (pseudocode):
        // ```ts
    ...
    fun normalizationGainDoesNotBoostBelowCeiling() {
        ...
        // TS map:   `expect(approxEq(normalizationGain(HALF), 1.0)).toBe(true);`
        //
        // In TS you'd write (pseudocode):
        // ```ts
    ...
    fun normalizationGainAttenuatesFullScaleToCeiling() {
        ...
        // TS map:   `expect(approxEq(normalizationGain(1.0), CEILING)).toBe(true);`
        //
        // In TS you'd write (pseudocode):
        // ```ts
    ...
    fun normalizationGainAttenuatesLoudToHalfCeiling() {
        ...
        //           ceiling.
        // TS map:   `expect(approxEq(normalizationGain(2.0), CEILING * HALF)).toBe(true);`
        //
        // In TS you'd write (pseudocode):
        // ```ts
    ...
    fun catmullRomPassesThroughSegmentStartAtZero() {
        ...
        // TS map:   `expect(approxEq(catmullRom(p0, p1, p2, p3, 0.0), p1)).toBe(true);`
        //
        // In TS you'd write (pseudocode):
        // ```ts
    ...
    fun catmullRomPassesThroughSegmentEndAtOne() {
        ...
        // TS map:   `expect(approxEq(catmullRom(p0, p1, p2, p3, 1.0), p2)).toBe(true);`
        //
        // In TS you'd write (pseudocode):
        // ```ts
    ...
    fun meterReportsInterSamplePeak() {
        ...
        // TS map:   `const signal = new Float32Array([0.0, 0.0, 0.9, -0.9, 0.9, -0.9, 0.0, 0.0]);`
        //           — Kotlin's `FloatArray` is TS's `Float32Array` (a typed array); a plain
        //           `number[]` would be the boxed-array analogue.
        ...
        // TS map:   `const rawPeak = Math.max(...Array.from(signal, (x) => Math.abs(x)));` — TS
        //           typed arrays have no `.maxOf`, so map to magnitudes then spread into `Math.max`.
        ...
        // TS map:   `const measured = measureTruePeak({ channels: 1, chunks: [signal] });` — Kotlin
        //           named args become an options object; `sequenceOf(x)` is roughly `[x]` but LAZY
        //           (a generator/iterable), so think of it as a one-item iterable of chunks.
        ...
        // TS map:   `expect(measured >= rawPeak - 1e-4).toBe(true);` — JUnit's message arg has no
        //           `expect()` analogue; Kotlin's `"$x"` is TS's template `\`${x}\``.
        ...
        // TS map:   `expect(measured < 4.0).toBe(true);`
        //
        // In TS you'd write (pseudocode):
        // ```ts
    ...
    fun processSampleAppliesGainThenClamps() {
        ...
        // TS map:   `expect(approxEq(processSample(0.0, 1.0), 0.0)).toBe(true);`
        //
        // In TS you'd write (pseudocode):
        // ```ts
        ...
        // TS map:   `expect(approxEq(processSample(HALF, 1.0), HALF)).toBe(true);`
        //
        // In TS you'd write (pseudocode):
        // ```ts
        ...
        // TS map:   `expect(approxEq(processSample(0.8, HALF), 0.4)).toBe(true);`
        //
        // In TS you'd write (pseudocode):
        // ```ts
        ...
        // TS map:   `expect(approxEq(processSample(1.5, 1.0), 1.0)).toBe(true);`
        //
        // In TS you'd write (pseudocode):
        // ```ts
        ...
        // TS map:   `expect(approxEq(processSample(-2.0, 1.0), -1.0)).toBe(true);`
        //
        // In TS you'd write (pseudocode):
        // ```ts
    ...
    fun chunkBoundariesDoNotChangeTheMeasuredPeak() {
        ...
        // TS map:   `const signal = new Float32Array([0.0, 0.0, 0.9, -0.9, 0.9, -0.9, 0.1, 0.2, -0.3, 0.0]);`
        //
        // In TS you'd write (pseudocode):
        // ```ts
        ...
        // TS map:   `const whole = measureTruePeak({ channels: 1, chunks: [signal] });`
        //
        // In TS you'd write (pseudocode):
        // ```ts
        ...
        // TS map:   `const split = measureTruePeak({ channels: 1, chunks: [signal.slice(0, 3), signal.slice(3, 4), signal.slice(4, 10)] });`
        //           — Kotlin's `copyOfRange(from, to)` is TS's `slice(from, to)` (to exclusive).
        ...
        //           change the result.
        // TS map:   `expect(approxEq(whole, split)).toBe(true);`
        //
        // In TS you'd write (pseudocode):
        // ```ts
    ...
    fun maxInteriorAbsMatchesCatmullRom() {
        ...
        // TS map:   `const windows = [new Float32Array([...]), new Float32Array([...]), ...];` — an
        ...
        // TS map:   `for (const w of windows) { ... }` — Kotlin's `for (w in windows)` is TS's
        ...
        for (w in windows) {
            ...
            // TS map:   `const reference = Math.max(Math.abs(catmullRom(w[0], w[1], w[2], w[3], 0.25)), Math.abs(catmullRom(w[0], w[1], w[2], w[3], 0.5)), Math.abs(catmullRom(w[0], w[1], w[2], w[3], 0.75)));`
            //           — Kotlin's `maxOf(a, b, c)` is `Math.max(a, b, c)`; `w[i]` is the same index
            //           read.
            //
            // In TS you'd write (pseudocode):
            // ```ts
            ...
            // TS map:   `const actual = maxInteriorAbs(w[0], w[1], w[2], w[3]);`
            //
            // In TS you'd write (pseudocode):
            // ```ts
            ...
            // TS map:   `expect(approxEq(actual, reference)).toBe(true);`
            //
            // In TS you'd write (pseudocode):
            // ```ts
