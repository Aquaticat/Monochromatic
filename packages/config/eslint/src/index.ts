...
const myGlobalIgnores: Linter.Config = globalIgnores([
  '**/dist/**',
  '**/bak/**',
  '**/temp/**',
  '**/node_modules/**',
...
const globalOptions: Linter.Config = {
  languageOptions: {
    parserOptions: {
...
const testOptions: Linter.Config = {
  files: ['**/*.{test,bench}.ts',],
...
const oxlintOffOptions: Linter.Config = {
  rules: {
    ...eslintRules,
    ...importRules,
...
// Declared @eslint/config-helpers to depend on eslint in pnpm-workspace to fix this.
const _default_1: (Linter.Config)[] = defineConfig(
  myConfigArray,
);
