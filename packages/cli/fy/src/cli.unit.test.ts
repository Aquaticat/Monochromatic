...
describe('cli-fy integration', () => {
  ...
  test('calls node:path basename with ext argument', async () => {
    const result = await runCliFy({ args: ['node:path', 'basename', '/tmp/foo.txt', '.txt'] });
  ...
  test('prints default export value from a local fixture', async () => {
    const result = await runCliFy({ args: ['./packages/cli/fy/src/fixtures/return1.ts', 'default'] });
    expect(result.stdout).toBe('1');
    expect(result.exitCode).toBe(0);
  });

  test('calls default export function and prints its return value', async () => {
    const result = await runCliFy({ args: ['./packages/cli/fy/src/fixtures/return1-fn.ts', 'default'] });
    expect(result.stdout).toBe('1');
    expect(result.exitCode).toBe(0);
  });
