...
- `autounattend-virtio.ts`: includes `viofs` in `VIRTIO_DRIVER_DIRS`
- `template-windows.ts`: downloads the Windows ISO, virtio-win ISO, and WinFsp MSI
- `template-windows.ts`: installs WinFsp and VirtioFsSvc dependencies via guest agent after QEMU-GA is reachable
- `autounattend.ts`: does not embed mise. Windows lifecycle tests install mise at runtime.
