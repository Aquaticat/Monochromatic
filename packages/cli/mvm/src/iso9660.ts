...
export function createIso({ files, volumeId, }: {
  ...
  writeDirEntry(iso, view, pvd + L.PVD_ROOT_DIR_RECORD_OFFSET, { isDir: true,
  ...
  for (const [ptSector, le,] of [[L.PATH_TABLE_LE_SECTOR, true,], [L.PATH_TABLE_BE_SECTOR,
    false,],] as const)
  {
  ...
  pos += writeDirEntry(iso, view, pos, { isDir: true, name: '\u0000',
