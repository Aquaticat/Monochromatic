...
export function collectionReplacements(options: {
  ...
  if (options.node.type === 'ArrayExpression') {
    /**
     * Whether the array literal has any elements.
     */
    const hasElements = Array.isArray(options.node.elements,)
      && (options.node.elements.length > 0);
  ...
  if (options.node.type === 'ObjectExpression') {
    /**
     * Whether the object literal has any properties.
     */
    const hasProperties = Array.isArray(options.node.properties,)
      && (options.node.properties.length > 0);
