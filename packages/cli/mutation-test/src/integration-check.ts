 ...
 * @example
 * ```bash
 * mise run '//packages/cli/mutation-test:test:integration'
 * ```
 */
 ...
 */
const FIXTURE_PACKAGE = 'packages/cli/mutation-test.fixture';
...
export async function runIntegrationCheck(): Promise<void> {
  ...
  await spawn(
    ...
    [
      join(
        cwd(),
        'packages/cli/mutation-test/dist/final/node/cli.mjs',
      ),
      '--package',
      FIXTURE_PACKAGE,
      '--report',
