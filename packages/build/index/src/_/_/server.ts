...
import ensureCrt from './_/ensureCrt.ts';

const l = getLogger(['app', 'server']);
...
const staticWebServerOptions = () => ({
  ...
  advanced: {
    rewrites: [
      ...
      {
        source: '/subtle/{*}/{l,li,lin,link,[!l]*,l[!i]*,li[!n]*,lin[!k]*,link[!s]*,links?*}',
...
export default async (): Promise<State> => {
  ...
  const wrote = await fs.outputFile(staticWebServerConfFilePath, stringify(staticWebServerOptions()));
