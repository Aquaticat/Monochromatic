...
import type { ContextAction, FileTree, } from '../file-tree/file-tree.ts';
...
fileTree.fetchDir = async function fetchDir(path: string,): Promise<DirEntry[]> { const r = await ws.request({ type: 'listDir', path, },); return 'entries' in r ? r.entries : []; };
fileTree.onContextAction = function handleContextAction(action: ContextAction,): void {
  void (async function dispatch(): Promise<void> { try { await dispatchFsAction({ action, ws, },); } catch (error) { showFixedToast({ message: `Action failed: ${String(error,)}`, },); } })();
};
...
searchOverlay.onSearch = async function handleSearch(query: string,): Promise<SearchResult[]> {
  ...
  const r = await ws.request({ type: 'search', query, scope, },); return 'results' in r ? r.results : [];
};
 ...
 */
function recordFileOpen(path: string,): void { recentFiles.push(path,); fileTree.updateRecency({ paths: recentFiles.paths, },); void fileTree.revealFiles({ paths: [path,], },); }
...
async function loadFileSafe({ path, line, character, }: { path: string; line?: number | undefined; character?: number | undefined }): Promise<void> {
  const kind = await loadFile({ ws, editorPane, binaryViewer, token, path, line, character, },);
  if (kind !== null) state.currentFileKind = kind;
}

/** LSP feature callbacks returned from wiring. */
const { formatDocument, requestCompletions, refreshInlayHints, gotoDefinitionAtCursor, expandSelection, shrinkSelection, } = wireLsp({ ws, editorPane, hoverPopup, completionPopup, referencesPopup, getCurrentFilePath: function get() { return state.currentFilePath; }, loadFileSafe, },);

wireSelectEvents({ fileTree, searchOverlay, referencesPopup, state, recordFileOpen, loadFileSafe, refreshInlayHints, },);

/** Saves the current editor content to the server. Skips non-text files. */
async function saveCurrentFile(): Promise<void> {
  if (state.currentFilePath === null || state.currentFileKind !== 'text') return;
  try { await ws.request({ type: 'save', path: state.currentFilePath, content: editorPane.getText(), },); }
...
wireKeybindings({
  ...
  gotoDefinition: gotoDefinitionAtCursor, deleteCurrentLine: editorPane.deleteCurrentLine.bind(editorPane,),
  selectAndCopyCurrentLine: editorPane.selectAndCopyCurrentLine.bind(editorPane,),
  indentLines: editorPane.indentLines.bind(editorPane,), unindentLines: editorPane.unindentLines.bind(editorPane,),
  duplicateLineDown: editorPane.duplicateLineDown.bind(editorPane,),
  swapLineDown: editorPane.swapLineDown.bind(editorPane,), swapLineUp: editorPane.swapLineUp.bind(editorPane,),
  openTerminalAtCurrentFile: function openTerminal() {
    const dir = state.currentFilePath !== null ? state.currentFilePath.slice(0, state.currentFilePath.lastIndexOf('/'),) : ws.rootDir;
    void ws.request({ type: 'openInTerminal', path: dir, },);
  },
  requestCompletions, expandSelection, shrinkSelection,
  navigateToRecentFile: function navigateToRecent(index: number,) {
    const path = recentFiles.paths[index]; if (path === undefined) return;
    state.currentFilePath = path; recentFiles.push(path,); fileTree.updateRecency({ paths: recentFiles.paths, },);
    void (async function revealLoadAndScroll(): Promise<void> {
      await fileTree.revealFiles({ paths: [path,], },); fileTree.scrollToFile({ path, },);
      await loadFileSafe({ path, },); if (state.currentFileKind === 'text') refreshInlayHints();
    })();
  },
