...
import { fetchChain, } from './selection-range-fetch.ts';
import { strictlyContains, toFlat, } from './selection-range-utils.ts';
    ...
    try {
      ...
      for (const entry of chain) {
        ...
        if (strictlyContains({ outer: flat, inner: currentSel, },)) {
          ...
          expandLog.info(`expand: ${flat.startLine}:${flat.startCharacter}-${flat.endLine}:${flat.endCharacter}`,);
          return;
        }
      }
