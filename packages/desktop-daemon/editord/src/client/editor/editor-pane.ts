...
export class EditorPane extends HTMLElement {
  ...
  connectedCallback(): void {
    this.#editor = h({ tag: 'div', class: 'editor', attrs: { contenteditable: 'true', spellcheck: 'false', }, },);
  ...
  disconnectedCallback(): void { this.#resizeObserver?.disconnect(); this.#resizeObserver = null; cancelAnimationFrame(this.#highlightFrame,); cancelAnimationFrame(this.#resizeMeasureFrame,); }
   ...
   */
  setParser(parser: Parser | null,): void { this.#parser = parser; if (parser === null) clearHighlights(); else this.#scheduleHighlight(); }
  ...
  setText(text: string,): void {
    ...
    this.#editor.replaceChildren(...text.split('\n',).map(function createLineDiv(line,) { return h({ tag: 'div', text: line === '' ? '\n' : line, },); },),); this.#scheduleHighlight();
  }
  ...
  getText(): string { if (this.#editor === null) return ''; return [...this.#editor.children,].map(function readLine(child,) { const t = child.textContent ?? ''; return t === '\n' ? '' : t; },).join('\n',); }
   ...
   */
  scrollToLine({ line, }: { line: number }): void { if (this.#editor === null) return; const child = this.#editor.children[Math.max(0, Math.min(line - 1, this.#editor.children.length - 1,),)]; if (child !== undefined) child.scrollIntoView({ block: 'center', },); }
   ...
   */
  applyTextEdits(edits: TextEdit[],): void { if (this.#editor === null || edits.length === 0) return; this.setText(applyEditsToText({ text: this.getText(), edits, },),); }

  /** Deletes the line at the current cursor position. */
  deleteCurrentLine(): void { this.#lineOp(function op(e, p,) { return deleteLineAt({ editor: e, ...p, },); },); }
  /** Duplicates the current line below. */
  duplicateLineDown(): void { this.#lineOp(function op(e, p,) { return duplicateLineAt({ editor: e, ...p, },); },); }
  /** Swaps the current line with the next line. */
  swapLineDown(): void { this.#lineOp(function op(e, p,) { return swapLineDown({ editor: e, ...p, },); },); }
  /** Swaps the current line with the previous line. */
  swapLineUp(): void { this.#lineOp(function op(e, p,) { return swapLineUp({ editor: e, ...p, },); },); }
  ...
  selectAndCopyCurrentLine(): boolean {
    ...
    const range = getComposedRange({ shadow: this.#shadow, },); const pos = this.getCursorPosition();
    if (range === null || pos === null) return false; return selectAndCopyLine({ editor: this.#editor, line: pos.line, composedRange: range, },);
  }

  /** Indents the current line or selected lines. */
  indentLines(): void { this.#indentOp(doIndent,); }
  /** Unindents the current line or selected lines. */
  unindentLines(): void { this.#indentOp(doUnindent,); }
   ...
   */
  setSelection({ startLine, startCharacter, endLine, endCharacter, }: { startLine: number; startCharacter: number; endLine: number; endCharacter: number }): void { if (this.#editor === null) return; setSel({ editor: this.#editor, coords: { startLine, startCharacter, endLine, endCharacter, }, },); }
  /**
   * Reads the current editor selection.
   *
   * @returns selection coordinates, or null
   */
  getSelection(): { startLine: number; startCharacter: number; endLine: number; endCharacter: number } | null { if (this.#editor === null) return null; return getSel({ editor: this.#editor, shadow: this.#shadow, },); }
   ...
   */
  getCursorPosition(): EditorPosition | null { return this.#editor !== null ? cursorPos({ editor: this.#editor, shadow: this.#shadow, },) : null; }
  /**
   * Measures the caret's bounding rectangle for popup positioning.
   *
   * @returns DOMRect of the caret, or null
   */
  getCursorRect(): DOMRect | null { return cursorRect({ shadow: this.#shadow, },); }
   ...
   */
  restoreCursor({ line, character, }: { line: number; character: number }): void { if (this.#editor === null) return; restoreCur({ editor: this.#editor, line, character, },); }
   ...
   */
  #lineOp(fn: (editor: HTMLDivElement, pos: { line: number; character: number },) => { line: number; character: number } | null,): void {
    if (this.#editor === null) return;
    const pos = this.getCursorPosition();
    if (pos === null) return;
    const result = fn(this.#editor, pos,);
    if (result !== null) this.restoreCursor(result,);
    this.#dispatchContentChange(); this.#scheduleHighlight();
  }
  ...
  #indentOp(fn: typeof doIndent,): void {
    ...
    const nonCollapsed = sel !== null && !(sel.startLine === sel.endLine && sel.startCharacter === sel.endCharacter) ? sel : null;
    const result = fn({ editor: this.#editor, cursorLine: pos.line, cursorCharacter: pos.character, selection: nonCollapsed, },);
    if (result.isSelection) this.setSelection(result.selection,); else this.restoreCursor(result.cursor,);
    this.#dispatchContentChange(); this.#scheduleHighlight();
  }

  /** Dispatches a `contentchange` event. */
  #dispatchContentChange(): void { this.dispatchEvent(new CustomEvent('contentchange', { bubbles: true, composed: true, },),); }

  /** Schedules diagnostic highlights. */
  #scheduleDiagnosticHighlights(): void { if (this.#editor !== null) scheduleDiagnosticHighlights({ editor: this.#editor, diagnostics: this.#diagnostics, },); }
  /** Schedules inlay annotations. */
  #scheduleInlayAnnotations(): void { if (this.#editor !== null) scheduleInlayAnnotations({ editor: this.#editor, hints: this.#inlayHints, diagnostics: this.#diagnostics, },); }
  /** Schedules inlay re-measurement. */
  #scheduleInlayMeasure(): void { if (this.#editor !== null) { cancelAnimationFrame(this.#resizeMeasureFrame,); this.#resizeMeasureFrame = scheduleInlayMeasure({ editor: this.#editor, },); } }
  /** Schedules syntax highlighting. */
  #scheduleHighlight(): void { cancelAnimationFrame(this.#highlightFrame,); if (this.#editor !== null) this.#highlightFrame = scheduleHighlight({ editor: this.#editor, parser: this.#parser, },); }
}
