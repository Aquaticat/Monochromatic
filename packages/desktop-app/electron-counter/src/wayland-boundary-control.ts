/**
 * Control-socket and observed-state helpers for the Wayland boundary test.
 *
 * @example
 * ```ts
...
export async function waitForPath(
  ...
  while (Date.now() < deadline) {
    try {
      // oxlint-disable-next-line eslint/no-await-in-loop -- sequential polling must probe the latest filesystem state before sleeping.
      await access(path,);
      return;
    }
    catch (error: unknown) {
 ...
 *
 * @throws Error when the socket yielded an unexpected payload type.
 *
 * @example
 * ```ts
