...
The GTK original (`packages/desktop-app/file-manager`) positions panes on fixed canvases and
...
- `mise run //packages/desktop-app/file-manager-electron:build` builds the staged Electron app.
- `mise run //packages/desktop-app/file-manager-electron:test:unit` runs the model, band-geometry, and sort unit tests against the built artifacts.
- `mise run //packages/desktop-app/file-manager-electron:test:wayland` drives the app inside `packages/cli/nested-wayland-session` and asserts spawn, dedup, close, zero pane overlaps, and the root pane pinning while scrolled.
- `mise run //packages/desktop-app/file-manager-electron:test` runs both.
