...
// TS map:   `watch.integration.test.ts` beside `watch.ts`.
...
// TS map:   `import * as parent from "./watch";`
use super::*;
...
// TS map:   `test("watcher fires on file creation", () => { ... })`
#[test]
fn watcher_fires_on_file_creation() {
    ...
    // TS map:   `const dir = join(os.tmpdir(), `mp_watch_${nanos}`);`
    ...
    // TS map:   `const { tx, rx } = makeChannel();`
    ...
    // TS map:   `const watcher = SourceWatcher.new(() => tx.send());`
    ...
    // TS map:   `await sleep(ARM_SLACK_MS);`
    ...
    // TS map:   `writeFileSync(join(dir, "new.flac"), "x");`
    ...
    // TS map:   `const got = await race(rx.recv(), timeout(WAIT_SECS));`
    ...
    // TS map:   `watcher.close(); rmSync(dir, { recursive: true });`
    drop(watcher);
