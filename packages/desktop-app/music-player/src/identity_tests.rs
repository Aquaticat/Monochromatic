...
// TS map:   `import * as identity from "./identity";` then read its members.
use super::*;
...
// TS map:   `test("info plist carries the bundle identifier", () => { ... })`
#[test]
fn info_plist_carries_the_bundle_identifier() {
    ...
    // TS map:   `const plist = readFileSync("../macos/Info.plist", "utf8");`
    ...
    // TS map:   `expect(plist).toContain(MACOS_BUNDLE_IDENTIFIER);`
    assert!(
...
// TS map:   `test("desktop StartupWMClass matches APP_ID", () => { ... })`
#[test]
fn desktop_file_wm_class_matches_app_id() {
    ...
    // TS map:   `const desktop = readFileSync(".../monochromatic.music-player.desktop", "utf8");`
    ...
    //           somewhere.
    // TS map:   `const expected = `StartupWMClass=${APP_ID}`;`
    ...
    // TS map:   `expect(desktop).toContain(expected);`
    assert!(
