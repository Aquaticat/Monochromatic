...
// What:     `#[cfg(test)] #[path = "peakcache_tests.rs"] mod tests;`
//           declares a test-only submodule whose code lives in the sibling
//           file `peakcache_tests.rs`. `#[cfg(test)]` gates it to test
//           builds only; `#[path = "..."]` aims the module at a flat sibling
//           file instead of the default `peakcache/tests.rs`
//           subdirectory lookup. The file stays the `tests` CHILD of
//           peakcache, so its `use super::*` reaches the module items
//           (including private ones) unchanged.
// Why:      Keep `peakcache.rs` to production code; the tests live
//           beside it without inflating this file or its max-lines budget
//           (sibling `*_tests.rs` files are exempt from the linter).
// TS map:   the `peakcache.unit.test.ts` file beside
//           `peakcache.ts`, excluded from the production bundle.
//
