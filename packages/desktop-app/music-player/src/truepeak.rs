...
impl TruePeakMeter {
    ...
    fn feed(&mut self, chunk: &[f32]) {
        ...
        //           out by value (deref in the pattern).
    ...
    fn push(&mut self, channel: usize, s: f32) {
        ...
        //           `self` while we also write `self.peak` below.
        ...
        //           `w` would not touch `self.win`. TS arrays would alias by reference.
        //
        // In TS you'd write (pseudocode):
        // ```ts
        // const w = this.win[channel].slice(); // value copy
        // ```
        ...
        if self.filled[channel] == WINDOW {
            ...
            for t in [QUARTER, HALF, THREE_QUARTERS] {
                ...
                // In TS you'd write (pseudocode):
                // ```ts
                // const v = Math.abs(catmullRom(shifted[0], shifted[1], shifted[2], shifted[3], t));
                // ```
...
pub(crate) fn measure_true_peak(path: &Path) -> Result<f32, PlayerError> {
    ...
    //           count and `as usize` widens it for indexing.
    ...
    // What:     `loop { ... }`. An UNCONDITIONAL loop (Rust's `while (true)`); exited
    //           with `break` below.
...
pub(crate) fn normalization_gain(true_peak: f32) -> f32 {
    ...
    //           -> return.
    // Why:      Louder-than-ceiling tracks get attenuated to the ceiling; quieter tracks
    //           are left as-is (gain 1.0).
...
// TS map:   the `truepeak.unit.test.ts` file beside `truepeak.ts`, excluded from the
//           production bundle.
//
// In TS you'd write (pseudocode):
// ```ts
