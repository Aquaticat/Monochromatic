...
// TS map:   `test("no args yields no path and paused", () => { ... })`
#[test]
fn no_args_yields_no_path_and_paused() {
    ...
    // TS map:   `expect(parse([])).toEqual({ start_playing: false, path: null });`
    assert_eq!(
        parse(&[]),
        Cli {
            start_playing: false,
...
// TS map:   `test("single folder loads paused", () => { ... })`
#[test]
fn single_folder_loads_paused() {
    // What:     parse one folder path.
    // Why:      It is captured in `path` and `start_playing` stays false.
    // TS map:   `expect(parse(["/music/folder"])).toEqual({ start_playing: false, path: "/music/folder" });`
    assert_eq!(
        parse(&["/music/folder"]),
        Cli {
            start_playing: false,
...
// TS map:   `test("single file is accepted", () => { ... })`
#[test]
fn single_file_is_accepted() {
    // What:     parse one file path.
    // Why:      It is captured in `path`.
    // TS map:   `expect(parse(["/music/a.flac"]).path).toEqual("/music/a.flac");`
    assert_eq!(
        parse(&["/music/a.flac"]),
        Cli {
            start_playing: false,
...
// TS map:   `test("flag with path enables start playing", () => { ... })`
#[test]
fn flag_with_path_enables_start_playing() {
    ...
    // TS map:   `expect(parse(["--start-playing", "/music/a.flac"])).toEqual({ start_playing: true, path: "/music/a.flac" });`
    assert_eq!(
...
// TS map:   `test("second path is rejected", () => { ... })`
#[test]
fn second_path_is_rejected() {
    ...
    // TS map:   `expect(() => parseArgs(["music-player", "a", "b"])).toThrow();`
