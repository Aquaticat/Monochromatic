...
```txt
user@bazzite:~/Monochromatic$ mise run //packages/desktop-app/terminal:run
[//packages/desktop-app/terminal:run] $ let home = ($env.HOME? | default '')
    Finished `release` profile [optimized] target(s) in 0.85s
[//packages/desktop-app/terminal:stage:r…] $ let lib_dirs = (glob "target/relea…
