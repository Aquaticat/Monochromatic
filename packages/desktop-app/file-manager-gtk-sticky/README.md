...
Sticky-band variant of the GTK file manager (`packages/desktop-app/file-manager`).
...
- `mise run //packages/desktop-app/file-manager-gtk-sticky:build` builds the release binary.
- `mise run //packages/desktop-app/file-manager-gtk-sticky:test` runs the pure band-math unit tests.
- `mise run //packages/desktop-app/file-manager-gtk-sticky:test:wayland` drives the app inside `packages/cli/nested-wayland-session` and asserts spawn, dedup, close, zero pane overlaps, and the root pane pinning while scrolled.
- `mise run //packages/desktop-app/file-manager-gtk-sticky:lint:clippy` and `:lint:rust` run the linters.
- `mise run //packages/desktop-app/file-manager-gtk-sticky:run` runs the GUI on native Wayland.
