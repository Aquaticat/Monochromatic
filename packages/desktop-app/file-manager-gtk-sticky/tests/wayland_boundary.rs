...
// compositor (packages/cli/nested-wayland-session) with synthetic keyboard input and asserts the
// observed-state JSON the app mirrors, using the same steps and key set as the Electron
// prototype's boundary test (packages/desktop-app/file-manager-electron). Exempt from
