  ...
  children: [
    ...
    it({
      ...
      fn: async function testExclusions() {
        expect(isFinalNewlineExcluded('packages/fuzz/forbidden-strings/seed/a',),).toBe(true,);
        expect(isFinalNewlineExcluded('packages/rust-module/forbidden-regex.fuzz/seed/a',),).toBe(true,);
        ...
        expect(isFinalNewlineExcluded('packages/fuzz/other/seed/a',),).toBe(false,);
    ...
    it({
      name: 'checks every lifecycle-provided candidate after commit',
      fn: async function testPostCommitPassThrough() {
        ...
        const landed = [
          ...
          candidate({ path: 'changed.txt', value: 'changed', },),
        ];
        /** Read-only post-commit findings across the whole landed delta. */
        ...
        expect(findings.map(function findingPath(finding,) {
          return finding.path;
        },),).toEqual([
          'stable.txt',
          'changed.txt',
