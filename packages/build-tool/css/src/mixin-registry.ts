...
function expandApplyInNodes(nodes: readonly ChildNode[]): ChildNode[] {
  ...
  function accumulateExpandedNode(result: ChildNode[], node: ChildNode): ChildNode[] {
    if (isAtRule(node) && node.name === 'apply') {
      ...
      if (mixinNodes.length > 0) {
        ...
        const expanded = expandApplyInNodes(mixinNodes.map(function cloneChild(childNode) { return childNode.clone(); }));
        result.push(...expanded);
      }
    } else if ('nodes' in node && Array.isArray(node.nodes)) {
      /** Deep clone so mutations don't affect the original mixin registry */
      const cloned = node.clone();
      cloned.nodes = expandApplyInNodes(node.nodes.map(function cloneChild(childNode) { return childNode.clone(); }));
      result.push(cloned);
    } else {
  ...
  return nodes.reduce<ChildNode[]>(function accumulate(result, node) { return accumulateExpandedNode(result, node); }, []);
}
...
export function expandMixinBodies(): void {
  ...
  while (hasChanges) {
    ...
    for (const [mixinName, nodes] of mixins) {
      ...
      const originalStr = nodes.map(function nodeToString(node) { return node.toString(); }).join('');
      /** Serialized expanded body for change detection */
      const expandedStr = expanded.map(function nodeToString(node) { return node.toString(); }).join('');
