...
type PenpotShapeType = "frame" | "group" | "bool" | "rect" | "circle" | "path" | "text" | "image" | "svg-raw";
...
interface PenpotShape {
  ...
  points: [PenpotPoint, PenpotPoint, PenpotPoint, PenpotPoint];
...
interface PenpotDocument {
  ...
  thumbnails: { path: string; data: Record<string, unknown> }[];
}
...
const IDENTITY_TRANSFORM: PenpotTransform = { a: 1, b: 0, c: 0, d: 1, e: 0, f: 0 };
...
function nextUuid(): Uuid {
  ...
  try {
    ...
    const c = uuidCounter;
    const h = (n: number, w: number) => (n >>> 0).toString(16).padStart(w, "0");
    const a = h(c & 0xffffffff, 8);
    const b = h((c >> 32) & 0xffff, 4);
    return `${a}-${b}-4${h(c & 0xfff, 3).slice(-3)}-8${h(c & 0xfff, 3).slice(-3)}-${h(c & 0xffff, 4)}${h((c >> 4) & 0xffff, 4)}${h((c >> 8) & 0xffff, 4)}`.slice(0, 36);
  }
}

/** Generate a stable UUID from Figma GUID (sessionID + localID). */
function guidToUuid(sessionId: unknown, localId: unknown): Uuid {
  ...
  const h = (n: number, w: number) => (n >>> 0).toString(16).padStart(w, "0");
  const a = h(sid, 8);
  const b = h((lid >>> 16) & 0xffff, 4);
  const c = `4${h((lid >>> 4) & 0xfff, 3)}`;
  const yNibble = ((lid & 0xf) | 0x8).toString(16);
  const d = `${yNibble}${h((sid >>> 16) & 0xfff, 3)}`;
  const e = `${h(sid & 0xffff, 4)}${h((lid >>> 8) & 0xff, 2)}${h(lid & 0xff, 2)}${h((sid >>> 8) & 0xff, 2)}${h((sid >>> 24) & 0xff, 2)}`;
  return `${a}-${b}-${c}-${d}-${e}`.slice(0, 36);
}
...
function figmaPaintToStroke(paint: Record<string, unknown>, strokeWeight: number, strokeAlign: string): PenpotStroke | null {
  const paintType = String(paint.type ?? paint.__type ?? "");
  if (paintType === "PaintType.SOLID" || paintType.includes("SOLID")) {
    const color = paint.color as Record<string, unknown> | undefined;
    if (!color) return null;
    return {
      strokeStyle: "solid",
      strokeAlignment: strokeAlign === "OUTSIDE" ? "outer" : strokeAlign === "INSIDE" ? "inner" : "center",
...
function computeSelRect(x: number, y: number, width: number, height: number): {
  selrect: PenpotSelRect;
  points: [PenpotPoint, PenpotPoint, PenpotPoint, PenpotPoint];
} {
  const x2 = x + width;
  const y2 = y + height;
  return {
    selrect: { x, y, width, height, x1: x, y1: y, x2, y2 },
    points: [
      { x, y },
      { x: x2, y },
      { x: x2, y: y2 },
...
function figmaTransformToPenpot(transform: Record<string, unknown> | undefined | null): PenpotTransform {
 ...
 */
function convertFigmaToPenpot(figmaFile: FigmaFile, options: ConvertOptions = {}): PenpotDocument {
  ...
  const nodeChanges = (figmaFile.document?.nodeChanges ?? []) as Record<string, unknown>[];
  ...
  for (const nc of nodeChanges) {
    ...
    if (guid) {
      ...
      const key = `${sid}:${lid}`;
      figmaGuidToUuid.set(key, guidToUuid(sid, lid));
    }
  }
  ...
  const storageObjects = new Map<Uuid, { meta: PenpotStorageObject; data: Uint8Array }>();
  ...
  const thumbnails: { path: string; data: Record<string, unknown> }[] = [];
  ...
  for (const nc of nodeChanges) {
    ...
    const lid = typeof guid.localID === "number" ? guid.localID : 0;
    const key = `${sid}:${lid}`;
    nodeByGuid.set(key, nc);

    // Track parent-child relationships
    const parentIndex = parseParentIndex(nc.parentIndex as Record<string, unknown> | undefined | null);
    if (parentIndex) {
      const parentKey = `${parentIndex.parentGuid.sessionId}:${parentIndex.parentGuid.localId}`;
  ...
  const pageSourceNodes: { key: string; nc: Record<string, unknown> }[] = [];
  if (isDeck) {
    ...
    for (const nc of nodeChanges) {
      ...
      if (nodeType === "NodeType.SLIDE") {
        ...
        const sid = typeof guid.sessionID === "number" ? guid.sessionID : 0;
        const lid = typeof guid.localID === "number" ? guid.localID : 0;
        pageSourceNodes.push({ key: `${sid}:${lid}`, nc });
      }
    }
  } else {
    ...
    for (const nc of nodeChanges) {
      ...
      if (nodeType === "NodeType.CANVAS") {
        ...
        if (internalOnly && name.toLowerCase().includes("internal")) continue;
  ...
  let pageIndex = 0;
  for (const { key: sourceKey, nc } of pageSourceNodes) {
    ...
    pages.set(pageId, {
      id: pageId,
      name: pageName,
      background: bgColor ? figmaColorToHex(bgColor) : (isDeck ? "#FFFFFF" : "#F5F5F5"),
      index: pageIndex,
    });
    ...
    shapes.set(rootFrameId, rootShape);
  ...
  if (pages.size === 0) {
    const pageId = nextUuid();
    const rootFrameId = ZERO_UUID;
    pages.set(pageId, { id: pageId, name: "Page 1", background: "#FFFFFF", index: 0 });
    const rootShape: PenpotShape = {
      id: rootFrameId,
      name: "Root Frame",
      type: "frame",
      ...
      transform: { ...IDENTITY_TRANSFORM },
      transformInverse: { ...IDENTITY_TRANSFORM },
      parentId: rootFrameId, frameId: rootFrameId,
  ...
  const manifest: PenpotManifest = {
    ...
    referer: "penpot",
    files: [{ id: fileId, name: fileName, features: [...DEFAULT_FEATURES] }],
    relations: [],
  };
...
function convertNode(
  ...
  if (penpotType === null || penpotType === undefined) return null;

  const shapeUuid = guidToUuidMap.get(nodeKey) ?? nextUuid();

  // Extract geometry
  const transform = figmaTransformToPenpot(nc.transform as Record<string, unknown> | undefined);
  ...
  const { selrect, points } = hasGeometry
    ? computeSelRect(effectiveX, effectiveY, effectiveW, effectiveH)
    : {
        selrect: { x: 0, y: 0, width: 0, height: 0, x1: 0, y1: 0, x2: 0, y2: 0 },
        points: [{ x: 0, y: 0 }, { x: 0, y: 0 }, { x: 0, y: 0 }, { x: 0, y: 0 }] as [PenpotPoint, PenpotPoint, PenpotPoint, PenpotPoint],
      };
  ...
  for (const paint of strokePaints) {
    const stroke = figmaPaintToStroke(paint, strokeWeight, strokeAlign);
  ...
  const shape: PenpotShape = {
    ...
    frameId: penpotType === "frame" ? shapeUuid : frameUuid,
    flipX: null,
    flipY: null,
    proportion: 1,
    proportionLock: false,
...
function convertTextContent(nc: Record<string, unknown>): Record<string, unknown> {
  ...
  const fontId = `gfont-${fontFamily.toLowerCase().replace(/\s+/g, "-")}`;
  ...
  if (fills.length === 0) {
    fills.push({ fillColor: "#000000", fillOpacity: 1 });
  }
...
async function serializePenpotZip(doc: PenpotDocument): Promise<Uint8Array> {
  ...
  zip.file(`files/${fileId}.json`, JSON.stringify(doc.file, null, 2));

  // Pages
  for (const [, page] of doc.pages) {
    const pageDir = `files/${fileId}/pages/${page.id}`;
    zip.file(`${pageDir}.json`, JSON.stringify(page, null, 2));

    // Root frame (always UUID zero)
    const rootFrameId = ZERO_UUID;
    const rootShape = doc.shapes.get(rootFrameId);
    if (rootShape && rootShape.pageId === page.id) {
      const shapeJson = JSON.stringify(rootShape, null, 2);
      zip.file(`${pageDir}/${rootFrameId}.json`, shapeJson);
    }

    // Shapes for this page
    for (const [shapeId, shape] of doc.shapes) {
      if (shape.pageId === page.id && shapeId !== rootFrameId) {
        zip.file(`${pageDir}/${shapeId}.json`, JSON.stringify(shape, null, 2));
      }
    }
  }
  ...
  for (const [mediaId, mediaObj] of doc.media) {
    zip.file(`files/${fileId}/media/${mediaId}.json`, JSON.stringify(mediaObj, null, 2));
  }

  // Storage objects
  for (const [objectId, { meta, data }] of doc.storageObjects) {
    zip.file(`objects/${objectId}.json`, JSON.stringify(meta, null, 2));
    const ext = mtypeToExtension(meta.contentType);
    zip.file(`objects/${objectId}${ext}`, data);
  }

  // Components
  for (const [compId, compData] of doc.components) {
    zip.file(`files/${fileId}/components/${compId}.json`, JSON.stringify(compData, null, 2));
  }

  // Colors
  for (const [colorId, colorData] of doc.colors) {
    zip.file(`files/${fileId}/colors/${colorId}.json`, JSON.stringify(colorData, null, 2));
  }

  // Typographies
  for (const [typoId, typoData] of doc.typographies) {
    zip.file(`files/${fileId}/typographies/${typoId}.json`, JSON.stringify(typoData, null, 2));
  }

  // Tokens
  if (doc.tokens) {
    zip.file(`files/${fileId}/tokens.json`, JSON.stringify(doc.tokens, null, 2));
  }

  // Thumbnails
  for (const thumb of doc.thumbnails) {
    zip.file(thumb.path, JSON.stringify(thumb.data, null, 2));
  }
...
async function figmaToPenpot(
  ...
  const doc = convertFigmaToPenpot(figmaFile, options);
