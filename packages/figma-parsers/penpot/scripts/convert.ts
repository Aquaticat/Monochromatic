...
import { convertFigmaToPenpot, serializePenpotZip } from "../src/index.ts";
...
const outPath = outputPath ?? inputPath.replace(/\.(fig|deck|jam)$/, ".penpot");

await writeFile(outPath, zipBuffer);

const nodeCount = (figmaFile.document?.nodeChanges ?? []).length;
console.log(`Converted ${figmaFile.fileType} (${nodeCount} nodes) -> ${outPath} (${zipBuffer.length} bytes)`);
