...
- Binary/hex tools (`xxd`, `hexdump`, `base64`, `tar`, `gzip`, etc.)
- Output redirections (`> file`, `>> file`)
- Commands already using the filter (`ccbof-filter`)
- Background commands (`&`, `nohup`, `setsid`)
...
with a `Bash` matcher. No manual configuration needed beyond installing the plugin.
