...
function formatModelDisplay(raw: string): string {
  ...
  if (version && version !== defaults?.latestVersion) result += ` ${version}`;
...
function formatRelativeTime(resetsAt: number): string {
  ...
  if (diff < SECONDS_PER_MINUTE) return `${diff}s`;
...
function formatRateLimit(tier: RateLimitTier | undefined): string {
  if (!tier?.used_percentage || !tier.resets_at) return "";

  const remaining = Math.floor(100 - tier.used_percentage);
  if (remaining > RATE_LIMIT_THRESHOLD) return "";
...
function formatContextWindow(used: number, total: number): string {
  const usedFmt =
    used >= THOUSANDS
      ? `${String(Math.floor(used / THOUSANDS)).padStart(3)},${String(
          used % THOUSANDS,
        ).padStart(3, "0")}`
