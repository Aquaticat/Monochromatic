...
On every user prompt, prepends the following tag to Claude's context:

```text
<time>HH:MM</time>
```

The hour and minute are read from the system's local 24-hour clock at the moment the prompt is submitted. The user's prompt text is not inspected or modified.
