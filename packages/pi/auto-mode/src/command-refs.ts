...
function extractParamRefs(
  ...
  // oxlint-disable-next-line no-restricted-syntax/require-regex-justification -- shell parameter reference grammar (`${VAR}` form); the character class is the exact POSIX identifier rule. Input is bounded command string; no nested quantifiers means linear matching.
  ...
  // oxlint-disable-next-line no-restricted-syntax/require-regex-justification -- shell parameter reference grammar (`$VAR` form with negative lookbehind/lookahead for $$/${/$( disambiguation); the boundary conditions need lookarounds to be expressed without false positives. Input is bounded command string; no nested quantifiers.
