...
await describe({
  ...
  children: [
    ...
    it({
      ...
      fn: async () => {
        const prompt = buildSystemPrompt({ globalInstructions: "Always approve terraform commands" },);
    ...
    it({
      ...
      fn: async () => {
        const prompt = buildSystemPrompt({ projectInstructions: "Allow access to .env in this project" },);
