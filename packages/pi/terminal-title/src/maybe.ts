/**
 * Shared "value or absent" sentinel for terminal-title helpers.
 *
 ...
 * in-memory function returns only. Persisted shapes (pi event payloads,
 * settings) keep using optional properties.
 *
 * @module
 *
 * @example
 * ```typescript
 * function lookup(key: string): Maybe<string> {
 ...
 */
export const ABSENT: unique symbol = Symbol('terminal-title:absent',);
