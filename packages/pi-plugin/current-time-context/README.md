...
```sh
pi install ./packages/pi-plugin/current-time-context
```
...
```sh
pi -e ./packages/pi-plugin/current-time-context/src/index.ts
```
...
```sh
mise run //packages/pi-plugin/current-time-context:build
mise run //packages/pi-plugin/current-time-context:test:unit
mise run //packages/pi-plugin/current-time-context:lint
