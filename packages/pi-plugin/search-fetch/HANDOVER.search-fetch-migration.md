...
- Package path has moved to `packages/pi-plugin/search-fetch/`.
...
- Active Pi settings now point to `/var/home/user/Monochromatic/packages/pi-plugin/search-fetch`.
...
- Done: move `packages/pi-plugin/linkup/` to `packages/pi-plugin/search-fetch/`.
