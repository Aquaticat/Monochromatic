 ...
 */
async function mirrorConfig(config: { readonly dir: string; readonly file: string }): Promise<void> {
...
await Promise.all([
  ...
  ...MIRROR_CONFIGS.map(function runMirror(config) { return mirrorConfig(config); }),
