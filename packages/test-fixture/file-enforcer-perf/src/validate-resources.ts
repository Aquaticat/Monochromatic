...
console.error(`[validate] IO: ${ioResult.ms.toFixed(1)}ms (${(IO_FILE_COUNT / ioResult.ms * MS_PER_SECOND).toFixed(0)} files/sec)`);
...
const result = {
  ...
  parallel: {
    ...
    hashesPerSec: Math.round((WORKER_COUNT * HASHES_PER_WORKER) / parallelMs * MS_PER_SECOND),
  },
  memory: {
