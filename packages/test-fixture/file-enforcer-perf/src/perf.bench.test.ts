...
describe('perf: micro-benchmarks', () => {
  ...
  test('expandGlob: 200 files across 20 directories', async () => {
    ...
    console.log(
      `expandGlob (200 files, ${String(IO_ITERATIONS,)} iters): ${
        elapsed.toFixed(1,)
      }ms, avg ${(elapsed / IO_ITERATIONS).toFixed(2,)}ms/call`,
    );
  ...
  test('cat(string[]): concatenate 20 files', async () => {
    ...
    console.log(
      `cat(string[20]) (${String(IO_ITERATIONS,)} iters): ${elapsed.toFixed(1,)}ms, avg ${
        (elapsed / IO_ITERATIONS).toFixed(2,)
      }ms/call`,
    );
  ...
  test('cat(string): glob-read 60 files', async () => {
    ...
    console.log(
      `cat(glob) 60 files (${String(EXPENSIVE_ITERATIONS,)} iters): ${
        elapsed.toFixed(1,)
      }ms, avg ${(elapsed / EXPENSIVE_ITERATIONS).toFixed(2,)}ms/call`,
    );
  ...
  test('overwrite: skip path (content unchanged)', async () => {
    ...
    console.log(
      `overwrite skip (${String(IO_ITERATIONS,)} iters): ${elapsed.toFixed(1,)}ms, avg ${
        (elapsed / IO_ITERATIONS).toFixed(2,)
      }ms/call`,
    );
    expect(elapsed,).toBeLessThan(MAX_IO_MS,);
  ...
  test('overwrite: write path (content different each time)', async () => {
    ...
    console.log(
      `overwrite write (${String(IO_ITERATIONS,)} iters): ${elapsed.toFixed(1,)}ms, avg ${
        (elapsed / IO_ITERATIONS).toFixed(2,)
      }ms/call`,
    );
    expect(elapsed,).toBeLessThan(MAX_IO_MS,);
  ...
  test('mirrorGlobPath: 1000 path transformations', () => {
    ...
    console.log(
      `mirrorGlobPath (${String(MIRROR_ITERATIONS,)} calls): ${
        elapsed.toFixed(1,)
      }ms, avg ${(elapsed / MIRROR_ITERATIONS).toFixed(3,)}ms/call`,
    );
  ...
  test('dedup: 2000-line content with 50% duplicates', () => {
    ...
    console.log(
      `dedup (${String(LINE_COUNT,)} lines, ${String(FAST_ITERATIONS,)} iters): ${
        elapsed.toFixed(1,)
      }ms, avg ${(elapsed / FAST_ITERATIONS).toFixed(2,)}ms/call`,
    );
    expect(elapsed,).toBeLessThan(MAX_FAST_MS,);
  });
  ...
  test('getProperty: extract from 1KB JSON', () => {
    ...
    console.log(
      `getProperty (1KB JSON, ${String(FAST_ITERATIONS,)} iters): ${
        elapsed.toFixed(1,)
      }ms, avg ${(elapsed / FAST_ITERATIONS).toFixed(2,)}ms/call`,
    );
    expect(elapsed,).toBeLessThan(MAX_FAST_MS,);
  ...
  test('classifyEvent + stat: 100 classifications', async () => {
    ...
    console.log(
      `classifyEvent+stat (${String(FAST_ITERATIONS,)} calls): ${
        elapsed.toFixed(1,)
      }ms, avg ${(elapsed / FAST_ITERATIONS).toFixed(2,)}ms/call, ${
        String(classificationCount,)
      } non-ignore`,
    );
    expect(elapsed,).toBeLessThan(MAX_IO_MS,);
  });
  ...
  test('deep glob: expand 6-level nested paths across 20 dirs', async () => {
    ...
    const elapsed = await measureAsync(async () => {
      for (let iterIndex = 0; iterIndex < EXPENSIVE_ITERATIONS; iterIndex++) {
        ...
        const matches = await expandGlob(
          join(tempDir, 'pkg-*/lib/deep/nested/very/deep/module.ts',),
        );
    ...
    console.log(
      `deep glob (6 levels, ${String(EXPENSIVE_ITERATIONS,)} iters): ${
        elapsed.toFixed(1,)
      }ms, avg ${(elapsed / EXPENSIVE_ITERATIONS).toFixed(2,)}ms/call`,
    );
  ...
  test('multiple overlapping globs: 5 patterns across 20 dirs', async () => {
    ...
    const elapsed = await measureAsync(async () => {
      for (let iterIndex = 0; iterIndex < EXPENSIVE_ITERATIONS; iterIndex++) {
        // oxlint-disable-next-line no-await-in-loop -- sequential benchmark
        const results = await Promise.all(patterns.map(function expandPattern(pattern,) {
    ...
    const GLOB_COUNT = 5;
    console.log(
      `${String(GLOB_COUNT,)} overlapping globs (${
        String(EXPENSIVE_ITERATIONS,)
      } iters): ${elapsed.toFixed(1,)}ms, avg ${
        (elapsed / EXPENSIVE_ITERATIONS).toFixed(2,)
      }ms/call`,
    );
    expect(elapsed,).toBeLessThan(MAX_EXPENSIVE_MS,);
  });
