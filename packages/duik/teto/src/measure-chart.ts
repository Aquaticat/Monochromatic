...
import { contentToAbsY, widthAtRelY } from './measure-profile-query.ts'
...
export function generateWidthProfileOutput(params: WidthProfileOutputParams): void {
  ...
  /** Also render to PNG. */
  try {
    run(`magick "${tmpDir}/width_profile_chart.svg" "${tmpDir}/width_profile_chart.png"`)
  } catch {
    /* SVG is sufficient if PNG rendering fails. */
  }

  console.error(`Width profile chart: ${tmpDir}/width_profile_chart.svg  (blue=ref, red=composite)`)
}
