...
const fm: { path: { name: string; }; lang: string; slug: string; title: string; defaultLang: string; } = inject(
  'frontmatter',
)!;
const fms: (typeof fm)[] = inject('frontmatters')!;
const matchingFms = fms.filter((potentialMatchingFm) => potentialMatchingFm.path.name === fm.path.name);
