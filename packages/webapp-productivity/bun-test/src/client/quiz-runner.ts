...
export function buildQuizRunner({ parent, cards, deckId }: {
  ...
  function updateUI(): void {
    scoreEl.textContent = `Card ${currentIndex + 1} of ${cards.length} | Correct: ${correctCount} | Wrong: ${wrongCount}`;

    if (currentIndex >= cards.length) {
      cardEl.style.display = "none";
      controls.style.display = "none";
      scoreEl.textContent = `Done! Correct: ${correctCount} | Wrong: ${wrongCount} out of ${cards.length}`;
  ...
  function handleCorrect(): void { void answer(true); }

  /** Marks the current card as wrong and advances. */
  function handleWrong(): void { void answer(false); }
