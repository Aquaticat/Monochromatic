...
function requireParam(event: Parameters<typeof getRouterParam>[0], name: string): string {
  ...
  if (value === undefined) throw new HTTPError({ status: HTTP_BAD_REQUEST, message: `missing route parameter: ${name}` });
  return value;
}
...
app.get('/', defineHandler(function handleInbox() { return inboxPage(); }));
app.get('/in-progress', defineHandler(function handleInProgress() { return inProgressPage(); }));
app.get('/tasks/:id', defineHandler(function handleTaskDetails(event) { return taskDetailsPage(requireParam(event, 'id')); }));
app.get('/search', defineHandler(function handleSearch(event) { return searchPage(event.url); }));
app.get('/settings', defineHandler(function handleSettings() { return settingsPage(); }));

app.post('/api/tasks', defineHandler(function handleCreateTaskRoute(event) { return handleCreateTask(event.req); }));
app.put('/api/tasks/:id', defineHandler(function handleUpdateTaskRoute(event) { return handleUpdateTask(event.req, requireParam(event, 'id')); }));
app.delete('/api/tasks/:id', defineHandler(function handleDeleteTaskRoute(event) { return handleDeleteTask(requireParam(event, 'id')); }));
app.post('/api/tasks/:id/start', defineHandler(function handleStartTimerRoute(event) { return handleStartTimer(requireParam(event, 'id')); }));
app.post('/api/tasks/:id/stop', defineHandler(function handleStopTimerRoute(event) { return handleStopTimer(requireParam(event, 'id')); }));
app.post('/api/tasks/:id/complete', defineHandler(function handleCompleteTaskRoute(event) { return handleCompleteTask(requireParam(event, 'id')); }));
app.post('/api/ai/autofill', defineHandler(function handleAutofillRoute(event) { return handleAutofill(event.req); }));
