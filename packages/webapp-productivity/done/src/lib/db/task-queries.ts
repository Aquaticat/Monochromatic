...
export async function listInboxUnblockedTasks(): Promise<Task[]> {
  ...
  return rows.map(function toTask(row) { return mapTask(row); });
}
...
export async function listBlockedInboxTasks(): Promise<BlockedTaskLink[]> {
  ...
  return rows.map(function toBlockedLink(row) { return { blockerId: row.blocker_id, task: mapTask(row) }; });
}
...
export async function listAllTags(): Promise<string[]> {
  ...
  return rows.map(function extractTag(row) { return row.tag; });
}
...
export async function searchTasks(searchQuery: string): Promise<SearchTask[]> {
  ...
  try {
    ...
    return rows.map(function toSearchTask(row) { return { ...mapTask(row), isBlocked: row.is_blocked === 1 }; });
  } catch {
    // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- database LIKE query
    const rows = await db
      .prepare(SQL_SEARCH_LIKE)
      .all(`%${normalizedSearchQuery}%`, `%${normalizedSearchQuery}%`) as (TaskRow & { is_blocked: number })[];
    return rows.map(function toSearchTask(row) { return { ...mapTask(row), isBlocked: row.is_blocked === 1 }; });
  }
}
