 ...
 * const e = makeError({
 *   message: 'boom',
 *   frames: ['at userFn (packages/foo/src/bar.ts:9:19)',],
 * });
 * ```
 */
function makeError({
 ...
 * dropped.
 */
const USER_FRAME = 'at userFn (packages/foo/src/bar.ts:9:19)';
    ...
    it({
      ...
      fn: async () => {
        const error = makeError({
          ...
          frames: [
            USER_FRAME,
            'at runFnOnce (packages/module/test/dist/final/neutral/index.mjs:120:10)',
    ...
    it({
      ...
      fn: async () => {
        const error = makeError({
          message: 'boom',
          frames: [
            'at firstUserFn (packages/foo/src/a.ts:9:19)',
            'at secondUserFn (packages/bar/src/b.ts:33:7)',
          ],
        },);
        const joined = (await formatErrorDeep(error,)).join('\n',);
        expect(joined,).toContain('packages/foo/src/a.ts',);
        expect(joined,).toContain('packages/bar/src/b.ts',);
      },
    },),
    ...
    it({
      ...
      fn: async () => {
        const root = makeError({
          message: 'root',
          frames: ['at rootFn (packages/foo/src/root.ts:1:1)',],
        },);
