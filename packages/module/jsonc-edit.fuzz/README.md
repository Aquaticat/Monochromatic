...
mise run //packages/module/jsonc-edit.fuzz:test:unit

# Longer fast-check campaign (default 5000 runs per property):
mise run //packages/module/jsonc-edit.fuzz:fuzz
mise run //packages/module/jsonc-edit.fuzz:fuzz --runs 20000

# Coverage-reachability gate (check against the frozen baseline):
mise run //packages/module/jsonc-edit.fuzz:fuzz:coverage
# Refreeze the baseline after intentionally adding reachable runtime functions:
mise run //packages/module/jsonc-edit.fuzz:fuzz:coverage --write
```

The gate measures the runtime package's `src/` reachability, not this sidecar: its
`SOURCE_MARKER` targets `packages/module/jsonc-edit/src`.
