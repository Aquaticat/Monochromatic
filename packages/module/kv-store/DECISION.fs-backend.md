...
The trigger:
 `packages/dev-script/file-enforcer` hand-rolled a persisted staleness manifest
(in-memory cache,
 dirty-set,
...
`packages/module/logger` already ships browser-specific and node-specific backends,
 so it is the
reference.
...
`packages/dev-script/file-enforcer/src/io/staleness-manifest.ts:248`),
...
`packages/module/memoize/src/cache-key.ts:24`) and never deletes the previous salt's entry;
 a salt
...
-   Type libs are sufficient.
    ...
    `lib: ['ESNext', 'DOM', 'WebWorker']` (`packages/config/typescript/README.md:12`),
     so both platform
