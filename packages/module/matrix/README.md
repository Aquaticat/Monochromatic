...
Test matrix runner.
Runs test files across a cartesian product of OS, user context, and JS runtime
...
The package handles container lifecycle, prerequisite installation,
runtime setup, user creation, and result collection via `describe`/`it`
...
filters out excluded combinations, and executes each via the appropriate backend.
...
For large matrices, increase or decrease `concurrency` to match available resources:
