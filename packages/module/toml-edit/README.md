...
Default is `splice`, which emits unmutated regions verbatim from the source.
...
In v1, `parseTomlEdit({ source, mode: 'canonical' })` falls back to splice emission when the source is non-empty.
