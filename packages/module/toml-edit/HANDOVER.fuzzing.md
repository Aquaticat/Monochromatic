...
`packages/module/toml-edit/**` triggered it),
...
needed.
 Most work lives under `packages/module/toml-edit/src/fuzz/` and
`src/conformance/`,
...
- Add `packages/module/toml-edit/src/fuzz-budget.ts` with
  `TOML_EDIT_FUZZ_BUDGET_MS`.
- Add a `fuzz` task to `packages/module/toml-edit/mise.toml`,
   mirroring
