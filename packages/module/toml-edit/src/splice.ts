...
export function spliceEmit({ edit, }: { edit: TomlEditState; },): string {
  ...
  for (const [node, edit_,] of edit.edits)
    events.push(computeReplaceEvent({ node, edit: edit_, },),);

  for (const node of edit.deletions)
    events.push(computeDeleteEvent({ node, state: edit, },),);

  for (const ins of edit.insertions) {
    const at = resolveAnchor({ anchor: ins.anchor, state: edit, },);
    events.push({ kind: 'insert', at, text: ins.text, },);
  }

  events.sort(function byOffset(a, b,) {
  ...
  const out: string[] = [];
  const cursor = events.reduce(function step(c, ev,) {
    if (ev.kind === 'insert') {
      if (ev.at > c) out.push(edit.source.slice(c, ev.at,),);
      out.push(ev.text,);
      return Math.max(c, ev.at,);
    }
    if (ev.start > c) out.push(edit.source.slice(c, ev.start,),);
...
function computeReplaceEvent(
  { node, edit, }: { node: AST.TOMLNode; edit: Edit; },
): { kind: 'replace'; start: number; end: number; text: string; } {
...
function valueRangeOf({ node, }: { node: AST.TOMLNode; },): readonly [number, number] {
...
function computeDeleteEvent(
  { node, state, }: { node: AST.TOMLNode; state: TomlEditState; },
): { kind: 'delete'; start: number; end: number; } {
  return computeDeletionRange({ node, state, },);
}
...
function computeDeletionRange(
  ...
  const endIncludingComment = trailingInlineComment === undefined
    ? lineEndExclusive
    : Math.max(lineEndExclusive, trailingInlineComment.range[1] + 1,);
  return { kind: 'delete', start, end: endIncludingComment, };
}

/** Resolve an `AnchorKind` to a byte offset in the source. */
function resolveAnchor(
  { anchor, state, }: { anchor: AnchorKind; state: TomlEditState; },
): number {
  if (anchor === 'eof')
    return state.source.length;
  if (anchor.position === 'after-node')
    return endOfLineAt({ source: state.source, at: anchor.node.range[1], },);
  if (anchor.position === 'before-node')
    return anchor.node.range[0];
  if (anchor.position === 'same-line-after')
    return anchor.node.range[1];
  return resolveInsideTable({ table: anchor.table, source: state.source, },);
}

/** Offset of the character after the next newline at or after `at`. */
function endOfLineAt({ source, at, }: { source: string; at: number; },): number {
...
function resolveInsideTable(
  { table, source, }: { table: AST.TOMLTable | AST.TOMLTopLevelTable; source: string; },
): number {
  if (table.body.length === 0) {
    if (table.type === 'TOMLTable')
      return endOfLineAt({ source, at: table.key.range[1], },);
    return 0;
  }
  const last = nonNullishOrThrow(table.body[table.body.length - 1],);
  return endOfLineAt({ source, at: last.range[1], },);
}
