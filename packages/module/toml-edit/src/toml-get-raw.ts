...
export function tomlGetRaw(
  ...
  if (result.kind === 'keyvalue')
    return edit.source.slice(result.node.value.range[0], result.node.value.range[1],);
  if (result.kind === 'value')
    return edit.source.slice(result.node.range[0], result.node.range[1],);
  if (result.kind === 'table' || result.kind === 'top-level')
    return edit.source.slice(result.node.range[0], result.node.range[1],);
  const first = nonNullishOrThrow(result.nodes[0],);
  const last = nonNullishOrThrow(result.nodes[result.nodes.length - 1],);
  return edit.source.slice(first.range[0], last.range[1],);
}
