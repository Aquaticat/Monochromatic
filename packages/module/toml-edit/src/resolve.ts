...
export type ResolveResult =
  | { readonly kind: 'top-level'; readonly node: AST.TOMLTopLevelTable; }
  | { readonly kind: 'keyvalue'; readonly node: AST.TOMLKeyValue; }
  | { readonly kind: 'value'; readonly node: AST.TOMLContentNode; }
  | { readonly kind: 'table'; readonly node: AST.TOMLTable; }
  | { readonly kind: 'array-of-tables'; readonly nodes: readonly AST.TOMLTable[]; }
  | {
...
export function resolveByPath(
  ...
  const root = edit.program.body[0];
  if (path.length === 0)
    return { kind: 'top-level', node: root, };
  return walk({ container: root, segments: path, consumed: 0, },);
}
...
function walk(
  ...
  if (container.type === 'TOMLArray')
    return walkArray({ container, segments, consumed, },);
  return walkTable({ container, segments, consumed, },);
}
...
function walkArray(
  ...
  if (typeof segment !== 'number')
    return { kind: 'missing', deepest: container, consumed, };
  const element = container.elements[segment];
  if (element === undefined)
    return { kind: 'missing', deepest: container, consumed, };
  if (segments.length === 1)
    return { kind: 'value', node: element, };
  if (element.type === 'TOMLArray' || element.type === 'TOMLInlineTable')
    return walk({ container: element, segments: segments.slice(1,), consumed: consumed + 1, },);
  return { kind: 'missing', deepest: element, consumed: consumed + 1, };
}
...
function walkTable(
  ...
  const directKeyValue = findKeyValueByPrefix({ container, segments, },);
  if (directKeyValue !== null) {
    const { node, matchedLen, } = directKeyValue;
    if (segments.length === matchedLen)
      return { kind: 'keyvalue', node, };
    const rest = segments.slice(matchedLen,);
    if (node.value.type === 'TOMLArray' || node.value.type === 'TOMLInlineTable')
      return walk({ container: node.value, segments: rest, consumed: consumed + matchedLen, },);
    return { kind: 'missing', deepest: node, consumed: consumed + matchedLen, };
  }
  ...
  const descendable = container.body.filter(function isDescendable(child,): child is AST.TOMLTable {
    ...
    return rk.every(function eq(k, i,) {
      return segments[i] === k;
    },);
  },);
  ...
  const tablesUnder = container.body.filter(function isUnder(child,): child is AST.TOMLTable {
    ...
    const rk = child.resolvedKey;
    if (rk.length <= segments.length) return false;
    return segments.every(function eq(s, i,) {
      return rk[i] === s;
    },);
  },);

  if (tablesUnder.length > 0)
    return { kind: 'array-of-tables', nodes: tablesUnder, };
...
function findKeyValueByPrefix(
...
): { node: AST.TOMLKeyValue; matchedLen: number; } | null {
  for (const child of container.body) {
    ...
    const allMatch = keys.every(function eq(k, i,) {
      return segments[i] === k;
    },);
    if (allMatch) return { node: child, matchedLen: keys.length, };
  }
  return null;
}
