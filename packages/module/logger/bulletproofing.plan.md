...
- This package has no `test:unit` mise task yet (Phase 1 adds it).
  ...
  `mise run //:test:unit packages/module/logger/src/<file>.unit.test.ts`.
   Passing
...
- Lint with `mise run //packages/module/logger:lint:oxlint`.
...
- `.github/workflows/logger-fuzz.yml`,
   path-filtered to
  `packages/module/logger/**` and the decision doc,
   running type-check,
   build,
