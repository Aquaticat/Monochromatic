...
(no benchmark, fuzz, or conformance tooling), which keeps a whole-package mutation run scoped
to real runtime files. See the fuzz and conformance sidecars for the other tooling.

## Run

```sh
mise run //packages/module/jsonc-edit.bench:bench
```
...
(where it uses the structured parser). Numbers are machine-dependent; run the task to reproduce
them locally.
