...
describe(unionIterablesAsync, () => {
  ...
  it('returns a Set of unique elements from multiple async iterables', async () => {
    ...
    const result = await unionIterablesAsync(asyncIterable1(), asyncIterable2(), [3,
      4,],);
...
describe(unionIterablesAsyncGen, () => {
  ...
  it('yields unique elements from multiple async iterables in order', async () => {
    ...
    const result = [];
    for await (
      const item of unionIterablesAsyncGen(asyncIterable1(), asyncIterable2(), [3, 4,
        1,],)
    ) {
      result.push(item,);
    }
  ...
  it('preserves the correct types with type inference', async () => {
    ...
    const numResult = [];
    for await (const item of numResultGen)
      numResult.push(item,);
    expect(numResult,).toEqual([1, 2, 3, 4,],);

    const strResult = [];
    for await (const item of strResultGen)
      strResult.push(item,);
    expect(strResult,).toEqual(['a', 'b', 'c', 'd',],);

    const mixedResult = [];
    for await (const item of mixedResultGen)
      mixedResult.push(item,);
    expect(mixedResult,).toContain(1,);
    expect(mixedResult,).toContain('a',);
  });
},);
