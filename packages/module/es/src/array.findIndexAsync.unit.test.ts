...
describe(arrayFindIndexAsync, () => {
  ...
  test('returns -1 for empty array', async () => {
    const empty: number[] = [];
    const isEvenAsync = async (n: number) => n % 2 === 0;
    const index = await arrayFindIndexAsync(empty, isEvenAsync);
    expect(index).toBe(-1);
  });
  ...
  test('works with object arrays', async () => {
    ...
    const index = await arrayFindIndexAsync(users, isActiveAsync);
    expect(index).toBe(1);
  });
  ...
  test('handles async predicate that rejects', async () => {
    ...
    await expect(arrayFindIndexAsync(numbers, failingPredicate)).rejects.toThrow('Test error');
  });
  ...
  test('stops execution after finding first match', async () => {
    ...
    const index = await arrayFindIndexAsync(numbers, countingPredicate);
    expect(index).toBe(3);
