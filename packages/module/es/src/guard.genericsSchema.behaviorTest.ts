 ...
 */
export function isRealSchema_Unknown(value: unknown): value is RealSchema {
 ...
 */
export function isRealSchema_Generic<const MyValue = unknown>(
  value: MyValue,
 ...
 */
export function isRealSchema_Typed<Input = unknown, Output = Input>(
 ...
 */
export function isRealSchema_GenericExtends<
 ...
 */
export function isRealSchema_GenericExtendsInfer<
  const T extends RealSchema<infer Input, infer Output> = RealSchema<unknown, unknown>
>(
  value: T
 ...
 * Async Schema Guards
 */
export function isRealSchemaAsync_Unknown(value: unknown): value is RealSchemaAsync {
  if (value === null) return false;
  if (typeof value !== 'object') return false;
  if (!('parseAsync' in value)) return false;
  return typeof value.parseAsync === 'function';
}

export function isRealSchemaAsync_Generic<const MyValue = unknown>(
  value: MyValue,
...
export function isRealSchemaAsync_GenericExtends<
 ...
 */
export function isRealMaybeAsyncSchema_Unknown(value: unknown): value is RealMaybeAsyncSchema {
  return isRealSchema_Unknown(value) || isRealSchemaAsync_Unknown(value);
}

export function isRealMaybeAsyncSchema_Generic<const MyValue = unknown>(
  value: MyValue,
...
export function isRealMaybeAsyncSchema_GenericExtends<
...
>(
  ...
  return (
    ...
    (typeof value === 'object' && value !== null && 'parseAsync' in value && typeof value.parseAsync === 'function')
  );
}
...
const realGenericTestValues = {
  ...
  // Branded schema
  brandedStringSchema: {
    parse: (value: string) => Object.assign(value, { __validated: true }) as string & { readonly __validated: true },
  ...
  promisableSchema: {
    parse: (value: string) => Math.random() > 0.5 
      ? value.length 
...
const testRealGenericWeightedString = (function() {
  ...
  if (isRealSchema_Unknown(value)) {
    ...
    // @ts-expect-error -- Unknown loses additional properties
    value.weight;
    // @ts-expect-error -- Unknown loses additional properties  
    value.priority;
  }
  ...
  // @ts-expect-error -- WeightedStringSchema is not directly assignable to RealSchema
...
const testRealGenericNamedUser = (function() {
  ...
  if (isRealSchema_Unknown(value)) {
    ...
    // @ts-expect-error -- Unknown loses naming properties
    value.schemaName;
    // @ts-expect-error -- Unknown loses naming properties
    value.version;
  }
...
const testRealGenericEdgeCases = (function() {
  ...
  if (isRealSchema_Unknown(unionValue)) {
    unionValue.parse; // Should work after union narrowing
 ...
 * 4. ⚡ Robust intersection type handling (Schema & AdditionalProps)
 * 5. ⚡ Strong compile-time safety with minimal casting requirements
 * 6. ⚡ Superior handling of complex generic constraints and branded types
 * 
 ...
 */
//endregion Comprehensive Generic Analysis Matrix
