...
describe(someIterable, () => {
  ...
  test('works with strings as iterables', () => {
    ...
    const failStr = 'xyz';
    expect(someIterable((char: string,) => 'aeiou'.includes(char,), failStr,),).toBe(
      false,
    );
  });
...
describe(someIterableAsync, () => {
  ...
  test('works with async iterables', async () => {
    ...
    await expect(
      someIterableAsync(async (num: number,) => num % 2 === 0, asyncIterable,),
    )
      .resolves
      .toBe(true,);
...
describe(someFailIterableAsync, () => {
  test('returns true when at least one element fails the test', async () => {
    const array = [2, 4, 5, 8,];
    expect(await someFailIterableAsync(async (num: number,) => num % 2 === 0, array,),)
      .toBe(
        true,
      );
  });
  ...
  test('works with validation scenarios', async () => {
    ...
    expect(await someFailIterableAsync(async user => user.age >= 18, users,),).toBe(
      true,
    );
