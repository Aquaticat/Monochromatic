...
/**
 * Determines if a range position is inside double quotes by counting non-escaped quotes preceding it.
 *
...
export function $(
  ...
  // Odd or even?
  const effectiveDoubleQuotesInStrBefore =
    Array.from(strBefore.matchAll(/(?<!\\)(?:\\\\)*"/g,),).length;
  return (effectiveDoubleQuotesInStrBefore % 2 !== 0);
}

// doubleQuotes({value: {startInclusive: 3, endInclusive: 3}, str: `"""a"`}) // true
// doubleQuotes({value: {startInclusive: 3, endInclusive: 3}, str: `""""""`}) // true - expected current limitation
// doubleQuotes({value: {startInclusive: 0, endInclusive: 0}, str: `a"`}) // false - expected current limitation
// doubleQuotes({value: {startInclusive: 0, endInclusive: 0}, str: `a""`}) // false
// doubleQuotes({value: {startInclusive: 1, endInclusive: 1}, str: `"a"`}) // true
// doubleQuotes({value: {startInclusive: 1, endInclusive: 1}, str: `"a`}) // true - expected current limitation
