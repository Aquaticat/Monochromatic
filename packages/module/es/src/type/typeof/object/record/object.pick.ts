...
export async function objectPick<
  const TObject extends Record<string, unknown>,
  const TKeys extends keyof TObject | MaybeAsyncSchema<TObject> | MaybeAsyncIterable<
...
>({
  ...
  TKeys extends MaybeAsyncSchema<TObject, infer Output> ? Output
    : TKeys extends MaybeAsyncIterable<infer U>
      ? Pick<TObject,
        Extract<keyof TObject,
          U extends MaybeAsyncSchema<keyof TObject, infer SchemaOutput> ? SchemaOutput
            : U>>
