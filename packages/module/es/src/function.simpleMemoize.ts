...
export function argsAlmostEqual(
  ...
  if (args1.length !== args2.length)
    return false;
  for (const [argumentIndex, element,] of args1.entries()) {
    if (element !== args2[argumentIndex])
      return false;
  }
  return true;
}
 ...
 * const memoizedFnWithArgs = simpleMemoize(fnWithArgs);
 ...
 * ```
 */
export function simpleMemoize<const Args extends readonly unknown[], Result,>(
  fn: (...args: Args) => Result,
): (...args: Args) => Result {
