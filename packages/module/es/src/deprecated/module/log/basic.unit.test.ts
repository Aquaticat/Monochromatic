import { consoleLogger, } from '@monochromatic-dev/module-es';
import type { Logger, } from '@monochromatic-dev/module-es';
import {
  describe,
  expect,
  expectTypeOf,
...
describe('consoleLogger', () => {
  ...
  test('should call appropriate console methods when used', () => {
    const traceSpy = spyOn(console, 'trace',).mockImplementation(() => { /* No-op stub */ },);
    const debugSpy = spyOn(console, 'debug',).mockImplementation(() => { /* No-op stub */ },);
    const infoSpy = spyOn(console, 'info',).mockImplementation(() => { /* No-op stub */ },);
    const warnSpy = spyOn(console, 'warn',).mockImplementation(() => { /* No-op stub */ },);
    const errorSpy = spyOn(console, 'error',).mockImplementation(() => { /* No-op stub */ },);
