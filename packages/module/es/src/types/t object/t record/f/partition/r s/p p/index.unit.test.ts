...
describe('partition (sync positional)', () => {
  test('partitions with sync predicate and sync iterable', () => {
    const numbers = [1, 2, 3, 4, 5];
    const result = partition((n: number) => n % 2 === 0, numbers);
  ...
  test('handles empty iterable', () => {
    const result = partition((n: number) => n > 0, []);
  ...
  test('preserves item order in each category', () => {
    ...
    const result = partition((n: number) => n > 5, numbers);
  ...
  test('works with non-array iterables', () => {
    ...
    const result = partition((n: number) => n % 2 === 0, set);
  ...
  test('works with strings as iterables', () => {
    const result = partition((c: string) => c >= 'c', 'abcde');
