...
describe('partition (async positional)', () => {
  test('partitions with sync predicate and sync iterable', async () => {
    const numbers = [1, 2, 3, 4, 5];
    const result = await partition((n: number) => n % 2 === 0, numbers);
  ...
  test('partitions with sync predicate and async iterable', async () => {
    ...
    const result = await partition((n: number) => n % 2 === 0, asyncNumbers());
  ...
  test('handles empty iterable', async () => {
    const result = await partition((n: number) => n > 0, []);
  ...
  test('preserves item order in each category', async () => {
    ...
    const result = await partition((n: number) => n > 5, numbers);
