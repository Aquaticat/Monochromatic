 ...
 */
/* @__NO_SIDE_EFFECTS__ */ export function $(input: string): JsoncValue {
  ...
  function skipWhitespace({ input, position }: { input: string; position: number }): number {
  ...
  function skipQuotedString({ input, position }: { input: string; position: number }): number {
  ...
  function extractComments({ input, position }: { input: string; position: number }): { comment: JsoncComment | undefined; newPosition: number } {
    ...
    while (position < input.length) {
      if (input[position] === '/' && position + 1 < input.length) {
        if (input[position + 1] === '/') {
          ...
          if (position < input.length) position++; // Skip newline
  ...
  function containsJsoncFeatures({ input, start, end }: { input: string; start: number; end: number }): boolean {
  ...
  function parseValue({ input, position }: { input: string; position: number }): { value: JsoncValue; newPosition: number } {
    ...
    if (char === '{' || char === '[') {
      ...
      if (!containsJsoncFeatures({ input, start, end })) {
        ...
        return { value: convertNativeValue({ value: nativeValue, comment }), newPosition: end };
      }
  ...
  function findNodeEnd({ input, position }: { input: string; position: number }): number {
    const char = input[position];
    
    if (char === '"') return skipQuotedString({ input, position });
    ...
    if (char === '{' || char === '[') {
      ...
      while (position < input.length && bracketStack.length > 0) {
        ...
        } else if (input[position] === closeChar || input[position] === (closeChar === '}' ? ']' : '}')) {
    ...
    if (char === '-' || /[0-9]/.test(char)) {
      ...
      if (position < input.length && input[position] === '0') {
        ...
        position++;
        while (position < input.length && /[0-9]/.test(input[position])) position++;
      }
      ...
      if (position < input.length && /[eE]/.test(input[position])) {
        position++;
        if (position < input.length && /[+-]/.test(input[position])) position++;
        while (position < input.length && /[0-9]/.test(input[position])) position++;
      }
    ...
    if (/[a-z]/.test(char)) {
      if (input.substring(position, position + 4) === 'true') return position + 4;
      if (input.substring(position, position + 5) === 'false') return position + 5;
      if (input.substring(position, position + 4) === 'null') return position + 4;
    }
    
    throw new Error(`Unexpected character '${char}' at position ${position}`);
  }

  function convertNativeValue({ value, comment }: { value: unknown; comment?: JsoncComment }): JsoncValue {
    if (typeof value === 'string') {
      ...
      return comment ? { type: 'null', value: null, comment } : { type: 'null', value: null };
    } else if (Array.isArray(value)) {
      return comment
        ? { type: 'array', value: value.map(item => convertNativeValue({ value: item })), comment }
        : { type: 'array', value: value.map(item => convertNativeValue({ value: item })) };
      ...
      return comment ? { type: 'record', value: entries, comment } : { type: 'record', value: entries };
    }
    
    throw new Error(`Unexpected value type: ${typeof value}`);
  }

  function parseObject({ input, position, comment }: { input: string; position: number; comment?: JsoncComment }): { value: JsoncRecord; newPosition: number } {
    ...
    while (position < input.length && input[position] !== '}') {
      // Parse key
      const { comment: keyComment, newPosition: keyCommentPosition } = extractComments({ input, position });
      ...
      // Parse value
      const { value: recordValue, newPosition: valueEndPosition } = parseValue({ input, position });
    ...
    return { value: comment ? { type: 'record', value: entries, comment } : { type: 'record', value: entries }, newPosition: position };
  }

  function parseArray({ input, position, comment }: { input: string; position: number; comment?: JsoncComment }): { value: JsoncArray; newPosition: number } {
    ...
    while (position < input.length && input[position] !== ']') {
      const { value: item, newPosition: itemPosition } = parseValue({ input, position });
      items.push(item);
    ...
    return { value: comment ? { type: 'array', value: items, comment } : { type: 'array', value: items }, newPosition: position };
  }

  function parsePrimitive({ input, position, comment }: { input: string; position: number; comment?: JsoncComment }): { value: JsoncValue; newPosition: number } {
    const char = input[position];
    ...
    if (char === '"') {
      ...
      return {
        value: comment ? { type: 'string', value: parsedValue, comment } : { type: 'string', value: parsedValue },
    ...
    if (char === '-' || /[0-9]/.test(char)) {
      ...
      if (!numberValidation.success) {
        throw new Error(`Invalid number format '${numberString}' at position ${position}`);
      }
      
      return {
        value: comment ? { type: 'number', value: numberValidation.data, comment } : { type: 'number', value: numberValidation.data },
        newPosition: endPosition,
      };
    }
    ...
    if (/[a-z]/.test(char)) {
      ...
      if (keyword === 'true') {
        return {
          value: comment ? { type: 'boolean', value: true, comment } : { type: 'boolean', value: true },
          newPosition: endPosition,
        };
      } else if (keyword === 'false') {
        return {
          value: comment ? { type: 'boolean', value: false, comment } : { type: 'boolean', value: false },
          newPosition: endPosition,
        };
      } else {
        return {
          value: comment ? { type: 'null', value: null, comment } : { type: 'null', value: null },
          newPosition: endPosition,
        };
      }
    }
  ...
  // Main parsing logic
  const { value: result, newPosition: finalPosition } = parseValue({ input, position: 0 });
