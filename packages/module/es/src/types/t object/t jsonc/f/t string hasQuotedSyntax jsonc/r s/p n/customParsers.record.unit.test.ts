...
const $ = types.object.jsonc.from.stringHasQuotedSyntaxJsonc.sync.named.customParserForRecord;
...
describe($, () => {
  ...
  test('single pair', ({ expect }) => {
    ...
    expect((val as any).value).toBe(1);
  });

  test('multiple pairs with trailing comma and comments', ({ expect }) => {
    const out = $({ value: '{"a":1, /* c */ "b": 2, /* d */ }X' as FragmentStringJsonc });
    const entries = Array.from(out.value.entries());
    expect(entries.map(([k, v]: any) => [k.value, v.value])).toEqual([["\"a\"", 1], ["\"b\"", 2]]);
  ...
  test('nested containers', ({ expect }) => {
    ...
    const entries = Array.from(out.value.entries());
    const aVal = entries[0][1] as any;
    const bVal = entries[1][1] as any;
    expect(Array.isArray(aVal.value)).toBe(true);
    expect((aVal.value[0] as any).value).toBe(1);
  ...
  test('first key receives inside comment', ({ expect }) => {
    ...
    const firstKey = Array.from(out.value.keys())[0] as any;
  ...
  test('missing colon after key', ({ expect }) => {
    expect(() => $({ value: '{"a" 1}' as FragmentStringJsonc })).toThrow(/expected ':' after key/);
  });

  test('missing comma between pairs', ({ expect }) => {
    expect(() => $({ value: '{"a":1 "b":2}' as FragmentStringJsonc })).toThrow(/expected ',' or '}/);
  });
  //endregion Errors
});
