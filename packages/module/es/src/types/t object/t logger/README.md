...
- `t/index.ts` -- `Logger`, `LogRecord`, `Sink`, `Verify`, `Level` type definitions
- `f/t never/r s/p p/index.ts` -- default multi-sink logger singleton with eager initialization
- `f/t object/t logger/tagged/r s/p n/index.ts` -- `tagged()` wrapper for composable prefixes
- `t sink/t console/r s/p p/index.ts` -- console sink with verbose-mode gating
- `t sink/t file/p p/index.ts` -- Node.js file sink (JSONL via `appendFile`)
- `t sink/t opfs/p p/index.ts` -- browser OPFS sink with persistent writable stream
- `t sink/t sessionStorage/r s/p p/index.ts` -- browser sessionStorage sink
- `t sink/t noop/r s/p p/index.ts` -- noop sink for testing
