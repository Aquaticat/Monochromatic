 ...
 * @example
 * ```ts
 * $.write({ level: 'info', message: 'server started', timestamp: Date.now() });
 * ```
 */
 ...
 * $.write({ level: 'info', message: 'x', timestamp: 0 });
 * ```
 */
 ...
 * @example
 * ```ts
 * $.write({ level: 'error', message: 'crash', timestamp: Date.now() });
 * await $.flush(); // guarantees the line is visible before next step
 * ```
 */
