...
function isTraversableObject(input: unknown): input is object {
  ...
  // Built-in terminals
  if (input instanceof Date || input instanceof RegExp) return false;
  if (input instanceof ArrayBuffer || input instanceof DataView) return false;
  if (isTypedArray(input)) return false;
  if (input instanceof WeakMap || input instanceof WeakSet) return false;
...
export function hasCycle(value: unknown): boolean {
  ...
  function dfs(node: unknown): boolean {
    if (!isTraversableObject(node)) return false;
    ...
    try {
      ...
      if (obj instanceof Map) {
        for (const [key, val] of obj as Map<unknown, unknown>) {
          if (dfs(key) || dfs(val)) return true;
        }
        return false;
      }
      ...
      if (isPlainObject(obj)) {
        ...
        for (const sym of symbols) {
          ...
          if (desc?.enumerable === true) {
            ...
            if (dfs(Reflect.get(obj, sym))) return true;
          }
        }
...
//endregion Cycle detection -- Detects circular references in arbitrary object graphs
