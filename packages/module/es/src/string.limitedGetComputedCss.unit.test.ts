...
describe(limitedGetComputedCss, () => {
  ...
  test('handles complex combinations', () => {
    expect(() => {
      ...
      .toThrow();
    expect(limitedGetComputedCss('var(--unknown, var(--b, var(--c)))',),).toBe(
      "'bString'",
    );
  });
...
describe(lGCC_reduceTokenToString, () => {
  const stringToken: TokenString = ['string-token', "'hello'", 0, 6, {
    value: 'hello',
  },];
  const whitespaceToken: TokenWhitespace = ['whitespace-token', ' ', 0, 0, undefined,];
  const varFuncToken: TokenFunction = ['function-token', 'var(', 0, 3, {
    value: 'var',
  },];
