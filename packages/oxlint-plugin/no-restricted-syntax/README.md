...
The package default export resolves to the prebuilt,
 self-contained
`dist/final/node/index.mjs` (run `mise run //packages/oxlint-plugin/no-restricted-syntax:build`
first).
...
```bash
mise run buildAndTest -- packages/oxlint-plugin/no-restricted-syntax/src/oxlint-no-restricted-syntax.unit.test.ts
```
