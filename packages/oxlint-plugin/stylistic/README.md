...
The package default export resolves to the prebuilt,
 self-contained
`dist/final/node/index.mjs` (run `mise run //packages/oxlint-plugin/stylistic:build` first).
...
```bash
mise run buildAndTest -- packages/oxlint-plugin/stylistic/src/oxlint-stylistic.unit.test.ts
mise run //packages/oxlint-plugin/stylistic:test:unit
```
