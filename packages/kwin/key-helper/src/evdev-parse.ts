 ...
 * ```
 */
export function reduceShiftEvent({ state, event }: {
  state: ShiftState;
  event: ShiftEvent;
}): ShiftResult {
  if (!event.isShift) {
    ...
    if (state.shiftPressed && event.value === VALUE_PRESS) {
      return { state: { ...state, otherKeyDuring: true }, doubleShift: false };
    }
    return { state, doubleShift: false };
  }
  if (event.value === VALUE_PRESS) {
    return {
      state: { ...state, shiftPressed: true, otherKeyDuring: false },
      doubleShift: false,
    };
  }
  ...
  if (!state.shiftPressed || state.otherKeyDuring) {
    return { state: { ...state, shiftPressed: false }, doubleShift: false };
  }
  ...
  if (state.lastTapTime > 0 && delta < DOUBLE_TAP_MS) {
    ...
    // immediately re-fire.
    return {
      state: { ...state, shiftPressed: false, lastTapTime: 0 },
      doubleShift: true,
    };
  }
  // First clean tap (or too slow): remember it as the start of a new window.
  return {
    state: { ...state, shiftPressed: false, lastTapTime: event.now },
    doubleShift: false,
  };
}
