 ...
 *
 * @mutates snapshots through handle.writeFile native-boundary access to snapshot bytes
 *
 * @example
 * ```ts
...
export async function prepareTrustRecord({
  ...
  try {
    ...
    await Promise.all([...snapshots.entries(),].map(
       ...
       *
       * @returns completion after bytes become durable
       *
       * @mutates entry through handle.writeFile native-boundary access to entry bytes
       */
      async function writeSnapshot(
        entry: readonly [string, Uint8Array],
      ): Promise<void> {
        /**
