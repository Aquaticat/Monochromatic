 ...
 *
 * Builds a throwaway monorepo (root manifest plus a `packages/grp/consumer`
 * importer) so the check runs against a real `packages/*\/*` layout and real
 * `package.json` files.
 *
 * @module
 */
 ...
 * `packages/grp/consumer` whose dependency fields are seeded from `deps`.
 *
...
async function makeRepo(
  ...
  /**
   * Importer directory under the `packages/*\/*` glob the check discovers.
   */
