...
The plan, this handover, and the in-tree state are mutually consistent.  Baseline tasks 1 through 10 are committed; the editord dev loop already runs on watch-restart.  Q6 tasks 11 + 13 + 14 + 15 are committed (event shape extension, type filter, cli parser extension, regex filter).  The next agent's first move is **Q6 task 16** (implement `src/filters/hidden.ts` exporting `hiddenFilter({allowHidden?})`; reject events whose `relativePath` has any segment starting with `.` when `allowHidden` is falsy; wire into `buildInternalFilter` in `start.ts` only when `options.hidden === false` or undefined — i.e. default-on hidden-exclusion; add unit tests under `src/filters/hidden.unit.test.ts`).  After task 16 the natural sequence is 17 (gitignore filter + `ignore` dep), 18 (watcher chokidar config), 19 (child process control), then 20 + 22 (README + AUDIT.md).  Each is one logical commit per AGENTS.md.
