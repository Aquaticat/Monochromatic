...
- ~~Task 13 (commit `774c9736`)~~: extended `StartWatchRestartOptions` with all Q6 fields (`includeRegex`, `excludeRegex`, `hidden`, `followSymlinks`, `gitignore`, `ignoreFiles`, `depth`, `poll`, `clear`, `killSignal`, `processGroup`); extended `cli.ts`'s optique parser, `ParsedArgs`, and `argsToOptions` with the matching flags (`--include-regex`, `--exclude-regex`, `--type`, `--hidden`/`--no-hidden`, `--follow-symlinks`/`--no-follow-symlinks`, `--gitignore`/`--no-gitignore`, `--ignore-file`, `--depth`, `--poll`, `--clear`/`--no-clear`, `--signal`, `--process-group`/`--no-process-group`). Extracted `splitCommas`, `cliEventToInternal`, and new validators (`parseTypeToken`, `parseKillSignal`, `compileRegex`, `resolveBoolPair`) to new sibling `src/cli-helpers.ts` so `cli.ts` stays under the max-lines cap. Tests cover round-trip for every new flag plus error cases (invalid type, invalid signal, invalid regex, both-positive-and-negative pair flag). No filter or watcher wiring yet; that lands in tasks 14 / 16 / 17 / 18 / 19.
- ~~Task 14 (commit `a1ae08b5`)~~: added `src/filters/regex.ts` exporting `regexFilter({include, exclude})`; sibling-to-`globFilter` shape (`event.relativePath` match, exclude-beats-include, both-empty vacuous-pass-all). Wired into `buildInternalFilter` in `start.ts` after the glob block, gated by "any includeRegex or excludeRegex present" (same shape as the glob gate at lines 219-227). Tests in `src/filters/regex.unit.test.ts` mirror `glob.unit.test.ts`'s five cases plus an extra case proving anchored alternation (`\.(test|spec|fixture)\.[jt]sx?$`) is expressible through this filter where picomatch cannot express it.
- ~~Task 16 (commit `4450355d`)~~: added `src/filters/hidden.ts` exporting `hiddenFilter({allowHidden?})`. Default-off (rejection): any segment of `event.relativePath` starting with `.` and followed by a non-dot non-separator char drops the event; `.` / `..` navigators do not count (the regex `(?:^|[/\\])\.[^./\\]` matches separator-or-start + literal-dot + non-dot-non-separator). `allowHidden: true` collapses the filter to vacuous-true; the orchestrator additionally elides the filter from the chain entirely when `options.hidden === true` so the zero-cost path stays zero-cost. Wired into `buildInternalFilter` in `start.ts` after the regex block, gated by `options.hidden !== true` (default-on rejection). Tests in `src/filters/hidden.unit.test.ts` cover: root dotfile rejection, nested dotfile rejection (swap, `.git/index`, `.cache/` dir), non-hidden admission, allowHidden pass-through, extension-only dot not treated as hidden, Windows backslash separators, no-args defaults.
- ~~Task 17 (commit `5cc6c293`)~~: added `ignore@7.0.5` (kaelzhang/node-ignore) to the pnpm catalog and the package's `dependencies` (uses `import ignore, { type Ignore } from 'ignore'`: `esModuleInterop` handles the CJS `export = ignore` form). `src/filters/gitignore.ts` exports `gitignoreFilter({roots, extraFiles?}): Promise<WatchFilter>`; async factory reads each `<root>/.gitignore` and each `extraFiles` path in parallel via `Promise.all`; ENOENT collapses to "no patterns" while other read errors propagate. Made `buildInternalFilter` in `start.ts` async; the orchestrator awaits once before the watcher starts. Wiring at start.ts: `gitignoreRoots = options.gitignore === false ? [] : options.paths` and `gitignoreExtraFiles = options.ignoreFiles ?? []`; the filter is only pushed when at least one source has patterns. Tests in `src/filters/gitignore.unit.test.ts` use temp-dir fixtures with `.gitignore` files; cases cover vacuous-no-gitignore, directory pattern, glob pattern, negation (using `dist/*` not `dist/` because git semantics forbid re-including a child of an excluded directory), `extraFiles` AND with `.gitignore`, empty-roots + extras only (the `--no-gitignore --ignore-file` case), missing extraFile ENOENT collapses to no-op.
- ~~Task 18 (commit `a8f79870`)~~: extended `WatcherOptions` with `depth?: number`, `poll?: number`, `followSymlinks?: boolean`. Wired into the chokidar factory call in `watcher.ts`: `depth` and `usePolling`/`interval` use the conditional-spread pattern (absent keeps chokidar's default); `followSymlinks` is always passed explicitly (`=== true`) so the package's safer default (false) holds regardless of chokidar's own default and does not silently flip on a chokidar version bump. start.ts forwards the three fields from `StartWatchRestartOptions` onto the Watcher constructor with the same pattern. Tests in `watcher.unit.test.ts` under a new "chokidar option pass-through" describe block: depth caps subdirectory traversal during the initial walk (depth 1 admits root + one level, rejects deeper); poll mode emits live events on file changes (50ms interval; 500ms wait); followSymlinks true pre-populates files inside a symlinked directory's target; default (followSymlinks absent) does NOT pre-populate them (chokidar treats the symlink as a leaf).
- ~~Task 19 (commit `f8b0ee2f`)~~: extended `ChildOptions` with `killSignal?: NodeJS.Signals` (default `'SIGTERM'`), `processGroup?: boolean` (default `true`), `clear?: boolean` (default `false`), plus two new dependency-injection points (`processSignal?: ProcessSignalFn`, `writeClear?: WriteClearFn`). `defaultSpawn` is now a closure factory `makeDefaultSpawn({ detached })` so the spawn options pick up `processGroup` without widening `SpawnFn`'s public arg shape. New `#sendSignal(handle, signal)` private method routes through `processSignal({ pid: -handle.pid, signal })` when processGroup mode is on and the handle has a pid; falls back to `handle.kill(signal)` otherwise. `#stopRunning` uses `#killSignal` for the first signal and `'SIGKILL'` for the always-on escalation. `#spawnAndTrack` calls `#writeClear()` before every spawn when `clear` is true (initial + restart, matching `watchexec --clear=clear`). start.ts forwards all three options to Child with the conditional-spread pattern. Existing tests in `child.unit.test.ts` and `start.unit.test.ts` got `processGroup: false` added explicitly so they exercise the direct-handle kill path (their assertions read `handle.signalsReceived`, only populated under that path). Four new tests in `child.unit.test.ts` under "Q6 options (killSignal, processGroup, clear)" cover: killSignal SIGHUP first then SIGKILL escalation; processGroup true routes through processSignal with negative pid; clear true runs writeClear before initial spawn and before restart; clear false (default) never runs writeClear.
- ~~Task 20 + 22 (commits `25ea295e`, `1f1343b2`)~~: README documents the full Q6 surface; synopsis rewritten with every Q6 flag, Flags section split into Filter / Watcher-config / Restart-driver subsections, explicit "AND-of-OR-of cap rule" subsection showing the full match condition, Library section expanded with the new built-in helpers (`regexFilter`, `typeFilter`, `hiddenFilter`, `gitignoreFilter`) and type exports, new "Examples" section with six Q6 invocations (CSS build watch, vendored + symlinked src, anchored alternation, polling mode, soft-reload signal, terminal clear), "Choices" section gains `ignore`-adoption and process-group-default rationales, Tests list expanded with the ten new Q6 cases. AUDIT.md ticks `ignore` (kaelzhang/node-ignore) as adopted, pointing at `packages/dev-script/watch-restart/src/filters/gitignore.ts` and naming the gitignore-semantics points the library handles (negation `!`, anchored leading `/`, directory-only trailing `/`, `**`). The first commit (`25ea295e`) advertised the AUDIT tick in its body but did not actually edit AUDIT.md (an Edit-tool conflict during stash juggling silently failed); the follow-up `1f1343b2` lands the missing tick. Task 21 (verify editord unchanged) is implicit; defaults (`--type file`, `--gitignore` ON, `--hidden` OFF, etc.) cover the existing case.

All Q6 work shipped. Baseline interactive verification partially GREEN as of 2026-05-12 (cases 1, 5, 7 below); cases 2, 3, 4, 6 still pending a human at the terminal. Q6 expansion is purely additive.
...
**Interactive verification (cases 5 to 11 from the plan). Partial GREEN 2026-05-12**:

1. **GREEN (2026-05-12)**: `mise run //packages/desktop-daemon/editord:dev:server` starts the bun server through `watch-restart`; the server prints startup logs and responds to HTTP on the configured port. Evidence (user-supplied terminal transcript): `[Watcher] ready` at `2026-05-12T01:53:56.832Z`; `[Child] spawned pid=663250 command=bun` at `.838Z`; `[editord] [http] listening on http://localhost:4400` at `.889Z`.
2. **PENDING**: Editing any file under `src/server/` with new content triggers exactly one restart; previous bun exits, new bun starts on the same port (no `EADDRINUSE`).
3. **PENDING**: Saving byte-identical content (either through editord's own `save.ts` flow or via an external editor's format-on-save) produces no restart.
4. **PENDING**: `touch src/server/index.ts` produces no restart (metadata-only; hash unchanged).
5. **GREEN (2026-05-12)**: Ctrl+C exits the watcher within ~1 s; the bun child exits; a subsequent `mise run //packages/desktop-daemon/editord:dev:server` succeeds (no leaked port, no zombie). Evidence (same transcript): `[Child] stopping pid=663250 (SIGTERM)` at `2026-05-12T01:54:00.265Z`; `[Child] exited pid=663250 code=0 signal=?` at `.268Z` (3 ms turnaround; user-confirmed second invocation succeeded).
6. **PENDING**: `kill -TERM <pid>` against the watcher produces the same clean shutdown.
7. **GREEN (2026-05-12)**: Bun child's stdout and stderr appear unchanged (stdio inherit). Evidence (same transcript): editord, tsdown, and watch-restart logs all co-located in the dev terminal.
...
The plan, this handover, and the in-tree state are mutually consistent. Baseline tasks 1 through 10 are committed; the editord dev loop already runs on watch-restart. Q6 tasks 11 + 13 + 14 + 15 + 16 + 17 are committed (event shape extension, type filter, cli parser extension, regex filter, hidden filter, gitignore filter). The next agent's first move is **Q6 task 18** (extend `src/watcher.ts` chokidar config: pass `options.depth` to chokidar's `depth`; `options.poll` to `usePolling: true, interval: poll`; `options.followSymlinks` to `followSymlinks`; tests verify the options reach chokidar; confirm chokidar continues to emit `addDir` / `unlinkDir` under default config; already done in task 11's commit, cross-check before declaring task 18 done). After task 18 the natural sequence is 19 (child process control), then 20 + 22 (README + AUDIT.md, with the `ignore` tick). Each is one logical commit per AGENTS.md.
