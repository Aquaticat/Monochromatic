...
The plan, this handover, and the in-tree state are mutually consistent.  Baseline tasks 1 through 10 are committed; the editord dev loop already runs on watch-restart.  Q6 tasks 11 + 13 + 15 are committed (event shape extension, type filter, and cli parser extension).  The next agent's first move is **Q6 task 14** (implement `src/filters/regex.ts` exporting `regexFilter({include, exclude})`; match against `event.relativePath` consistent with `globFilter`; wire into `buildInternalFilter` in `start.ts` only when at least one regex is given; add unit tests under `src/filters/regex.unit.test.ts` mirroring `glob.unit.test.ts`).  After task 14 the natural sequence is 16 (hidden filter), 17 (gitignore filter + `ignore` dep), 18 (watcher chokidar config), 19 (child process control), then 20 + 22 (README + AUDIT.md).  Each is one logical commit per AGENTS.md.
