...
**Status**: tasks 1 through 10 done (baseline package).  Q6 watchexec-parity expansion approved by user 2026-05-12 (see plan file `## Question 6`).  Q6 progress: tasks 11 + 13 + 14 + 15 + 16 done (event shape extension + typeFilter + cli parser extension + regexFilter + hiddenFilter).  Remaining Q6 tasks: 17, 18, 19, 20, 22 (see "Q6 task tracker" below).  Baseline interactive verification (plan cases 5 to 11) still pending a human at the terminal — Q6 expansion is purely additive and does not block that verification.
...
The plan, this handover, and the in-tree state are mutually consistent.  Baseline tasks 1 through 10 are committed; the editord dev loop already runs on watch-restart.  Q6 tasks 11 + 13 + 14 + 15 + 16 are committed (event shape extension, type filter, cli parser extension, regex filter, hidden filter).  The next agent's first move is **Q6 task 17** (add `ignore` (kaelzhang/node-ignore) to the pnpm catalog and the package's `dependencies`; implement `src/filters/gitignore.ts` exporting `gitignoreFilter({roots, extraFiles?}): Promise<WatchFilter>` — async because it reads `.gitignore` files from disk during construction; wire `--gitignore` and `--ignore-file` into `buildInternalFilter` in `start.ts` — note the orchestrator must `await` the factory once before the watcher starts; add unit tests under `src/filters/gitignore.unit.test.ts` using temp-dir fixtures with a `.gitignore` and extra-file patterns; defer ticking `ignore` in `AUDIT.md` to task 22).  After task 17 the natural sequence is 18 (watcher chokidar config), 19 (child process control), then 20 + 22 (README + AUDIT.md).  Each is one logical commit per AGENTS.md.
