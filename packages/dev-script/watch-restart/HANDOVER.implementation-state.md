...
**Status**: tasks 1 through 10 done (baseline package).  Q6 watchexec-parity expansion approved by user 2026-05-12 (see plan file `## Question 6`).  Q6 progress: tasks 11 + 13 + 14 + 15 + 16 + 17 done (event shape extension + typeFilter + cli parser extension + regexFilter + hiddenFilter + gitignoreFilter).  Remaining Q6 tasks: 18, 19, 20, 22 (see "Q6 task tracker" below).  Baseline interactive verification (plan cases 5 to 11) still pending a human at the terminal — Q6 expansion is purely additive and does not block that verification.
