...
const AUDIT_TARGET: PackageProbe = {
  ...
  daysSinceLastCommitOrNull: 1200,
  ...
  children: [
    ...
    it({
      ...
      fn: async () => {
        ...
        expect(extractDim({ probe: JS_NON_LEAF_STALE, dim: 'isLeafNumeric', },),).toBe(0,);
      },
    },),
    ...
    it({
      ...
      fn: async () => {
        ...
        expect(extractDim({ probe: JS_NON_LEAF_STALE, dim: 'licenseClassNumeric', },),).toBe(1,);
      },
    },),
    ...
    it({
      ...
      fn: async () => {
        expect(derivedBool({ probe: JS_NON_LEAF_STALE, key: 'tsMajority', },),).toBe(false,);
      },
    },),
    ...
    it({
      ...
      fn: async () => {
        ...
        expect(derivedBool({ probe: MONOREPO_UNKNOWN, key: 'hasKnownRepo', },),).toBe(false,);
      },
    },),
    ...
    it({
      ...
      fn: async () => {
        expect(searchMatches({ probe: MONOREPO_UNKNOWN, search: '/^@lezer/', },),).toBe(true,);
