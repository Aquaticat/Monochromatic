...
await describe({
  ...
  children: [
    ...
    it({
      name: 'probeAll marks monorepo-housed entries as monorepo when Linguist would mismeasure',
      fn: async () => {
        await using cacheDir = await tempPopulatedCache({
          entries: {
            '@lezer/common/_pkg': [
    ...
    it({
      ...
      fn: async ({ sinon, },) => {
        ...
        sinon.stub(globalThis, 'fetch',).rejects(new Error('network disabled for test',),);
        /* oxlint-enable typescript-eslint/no-unsafe-member-access */
        /* oxlint-enable typescript-eslint/no-unsafe-call */
        const [probe,] = await probeAll({
          entries: [
            { catalogKey: 'definitely-not-real', npmName: 'definitely-not-real', range: '^1.0.0', },
          ],
          cache,
        },);
