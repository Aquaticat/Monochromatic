...
import { renderControls, } from '../src/render-controls.ts';
import { defaultState, } from '../src/scripts/state.ts';
  ...
  children: [
    it({
      ...
      fn: async () => {
        ...
        expect(countOccurrences({ haystack: html, needle: 'class="dim-row"', },),).toBe(6,);
      },
    },),
    ...
    it({
      ...
      fn: async () => {
        ...
        expect(countOccurrences({ haystack: html, needle: 'class="toggle-row"', },),).toBe(7,);
      },
    },),
    ...
    it({
      ...
      fn: async () => {
        ...
        const radios = countOccurrences({ haystack: html, needle: '<input type="radio"', },);
    ...
    it({
      name: 'emits the search input, display checkboxes, name-labels select, counter, and reset button',
      fn: async () => {
        const html = renderControls({
          probes: PROBES,
          state: defaultState({ probes: PROBES, },),
    ...
    it({
      ...
      fn: async () => {
        ...
        expect(html,).toContain(`${PROBES.length.toString()} of ${PROBES.length.toString()} visible`,);
      },
    },),
