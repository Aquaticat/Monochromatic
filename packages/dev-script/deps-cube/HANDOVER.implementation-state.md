...
- New `probeRadiusWorld({probe, state, bounds})` accessor in `src/deck-accessors.ts` returning world-space radius scaled to 0.5–3% of the bounds diagonal (`Math.hypot(dx, dy, dz)`).
