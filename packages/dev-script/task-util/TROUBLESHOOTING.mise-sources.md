...
Source: [`src/task/task_source_checker.rs`][checker] in the mise repo.
...
After a successful run, `save_checksum` ([line 184][L184]) touches auto-output
...
Mise uses the Rust [`glob`][glob-crate] crate. In this crate, a trailing `**`
...
When `sources` is defined but `outputs` is empty, mise automatically sets
`outputs` to `TaskOutputs::Auto` ([`mod.rs` line 996--997][auto]). This creates
a synthetic touch file in `~/.local/state/mise/task-auto-outputs/` that gets
touched after each successful run. The staleness check then compares source
...
This uses blake3 to hash file contents instead of metadata, at a small
performance cost.
...
run = "mise run //packages/sibling-pkg:build"
```

On the first run, both tasks execute. On subsequent runs, `expensive-prep` is
