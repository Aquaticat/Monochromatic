...
export function renderByModel({ entries, thresholds, }: {
  ...
  const labels = [...new Set(entries.map(function getLabel(entry): string { return entry.label; }))];
  ...
  return labels.map(function renderModelSection(label): string {
    ...
    // Per-probe charts
    const probeNames = [...new Set(modelEntries.flatMap(function probeKeys(entry): string[] { return Object.keys(entry.probeScores); }))];
    const probeCharts = probeNames.map(function renderProbeChart(probe): string {
