 ...
 */
function generateMixin(index: number): { name: string; css: string; declarations: string } {
...
const mixins = Array.from({ length: MIXIN_COUNT, }).map((_, index) => generateMixin(index));
...
const recursiveMixins = Array.from({ length: recursiveMixinCount, }).map((_, index) => {
  ...
  if (baseMixin === undefined) throw new Error('Base mixin undefined');
...
for (const index of Array.from({ length: RULE_COUNT, }).keys()) {
  ...
  for (const applyIndex of Array.from({ length: applyCount, }).keys()) {
    ...
    if (index % 7 === 0 && applyIndex === 0) {
      ...
      if (recMixin === undefined) throw new Error('Recursive mixin undefined');
      applyNames.push(recMixin.name);
      expandedDecls.push(recMixin.expandedDeclarations);
    } else {
      const mixin = mixins[mixinIdx];
      if (mixin === undefined) throw new Error('Mixin undefined');
  ...
  if (index % 10 === 0) {
    ...
    if (nestedMixin === undefined) throw new Error('Nested mixin undefined');
