...
export type Probe = {
   ...
   */
  readonly score: (response: string) => number | Promise<number>;
  /**
   ...
   */
  readonly buildFixPrompt?: (response: string) => string | undefined | Promise<string | undefined>;
  /**
...
const simpleArithmetic: Probe = {
  ...
  prompt: 'What is 7 * 8?',
  score: (response) => response.trim() === '56' ? 1 : 0,
};
...
const simpleFormat: Probe = {
  ...
  score: (response) => response.trim() === '<<CANARY_OK>>' ? 1 : 0,
};
