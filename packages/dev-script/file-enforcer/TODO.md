...
- The entire config re-runs on any source change.
  For configs with many independent rules, this is wasteful when only one rule's sources changed.
  A future optimization: tag rules with their source paths and only re-run affected rules.
...
- No conditional logic built into the API (e.g., "only overwrite if source is newer").
