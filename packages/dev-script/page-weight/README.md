...
would fetch to render it, then reports the distribution:
`min`, `max`, `mean`, `median`.
...
For each counted file, the script reports the size of the `.zst`
companion when one exists alongside it on disk, otherwise the raw size.
...
External URLs (`http://`, `https://`, `//cdn...`, `data:...`) and fragment-only
