...
6. Check whether the `--fix` autofix test is the one that hangs (it writes to disk and re-lints, more I/O)
7. Look at the `afterEach` cleanup: `unlinkSync` on a non-existent file in the catch block should be fine, but verify
