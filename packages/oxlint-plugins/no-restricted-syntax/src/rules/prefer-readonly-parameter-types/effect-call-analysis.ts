 ...
 */
function addIntrinsicCallbackEffects({
  project,
  checker,
  bindingOriginBySymbolId,
  call,
  ...
  callbackEffects.forEach(function intrinsicCallbackEffect(callbackEffect,): void {
    ...
    if (callbackParameterIndex !== PARAMETER_INDEX_UNAVAILABLE) {
      callbackEffect.receiverParameterIndexes.forEach(function callbackRelation(
    ...
    if (callback === OWNED_CALLABLE_UNAVAILABLE) {
      addOpaqueEffect({
        ...
        provenance: `${call.expression.getText()} callback`,
      },);
      return;
    }
    /**
    ...
    summary.calls.push({
      ...
      arguments: callback.parameters.map(function callbackSource(
        _parameter,
        callbackArgumentIndex,
      ): number | typeof PARAMETER_INDEX_UNAVAILABLE {
      ...
      callbackKeys: callback.parameters.map(function noNestedCallback(): typeof OWNED_CALLABLE_UNAVAILABLE {
        return OWNED_CALLABLE_UNAVAILABLE;
      },),
    },);
  },);
