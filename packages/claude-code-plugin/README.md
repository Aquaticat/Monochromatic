...
1. Build the current plugin with `mise run //packages/claude-code-plugin/{plugin}:build:js:node`
...
8. Run `mise run //packages/claude-code-plugin/{plugin}:build:js:node` to
...
hand-maintained sibling files.
 With `outDir: '..'`,
 it wiped not only
`packages/claude-code-plugin/{plugin}/.claude-plugin/`,
 `README.md`,
 and
sibling source trees,
 but also `packages/config/tsdown/src/`:
 the workspace
