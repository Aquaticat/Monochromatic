...
For a loud failure, use `@monochromatic-dev/stub-throwing` instead.
For optional dependencies that consumers handle via `try { require } catch`, use global removal in `pnpm-workspace.yaml`.
