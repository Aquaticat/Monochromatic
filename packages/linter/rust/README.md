...
conventions on `.rs` files. It ships two rules: `max-lines`, a per-file code-line
budget that mirrors oxlint's `eslint/max-lines` rule for TypeScript; and
`require-rustdoc`, which requires a rustdoc comment on every documentable item,
...
A node fails when it is a documentable item but carries no rustdoc comment. A
rustdoc comment is `///` or `/** */` (outer) or `//!` or `/*! */` (inner); a plain
`//` or `/* */` comment does not count, so the repository's mandatory
`dum-dum-non-ts` `// What:` / `// Why:` blocks never satisfy the rule on their own.
Detection uses rust-analyzer's own doc-comment scanner, so the answer agrees with
how `rustdoc` itself attaches comments to items.

The rule is deliberately maximal, mirroring `require-tsdoc`'s "document every
declaration, exported or not". It requires a doc comment on:

- the file itself (a `//!` module doc)
- functions, structs, enums, unions, traits, type aliases, constants, statics,
  modules, `macro_rules!` and `macro` definitions
- extern crates, extern blocks, `use` declarations, and `impl` blocks
- enum variants and struct fields (named and tuple)
- item-position macro calls (a `macro!` used as an item, not as an expression)

It is stricter than Rust's built-in `missing_docs` lint in two ways: it covers
private items, not only public ones, and it has no trait-impl carve-out. A method
inside `impl Trait for Type` must still carry its own doc comment, even though
`rustdoc` would otherwise inherit the trait's documentation.

### Exemptions

Mirroring the max-lines rule (minus its fixtures carve-out, so this linter's own
`fixtures/` inputs are enforced too), the rule skips:
...
There is no inline suppression. The remedy for an undocumented item is to document
it, not to disable the rule. One caveat: a `///` on a private `use` or item is
accepted here but can trip `rustc`'s `unused_doc_comments` warning; that is the
...
- `src/rules/require_rustdoc.rs`: the `require-rustdoc` rule.
