...
**Assessment:** this is a medium dependency-packaging and documentation-hygiene finding,
...
The lockfile therefore records `node-domexception@1.0.0` and npm's deprecation message,
...
- **Install and audit surface:** present for every published CLI install.
- **Module-loading surface:** reached conditionally through the Google authentication request path.
- **Behavior on supported Node:** exports the native constructor and does not enter its fallback.
...
Its original README said that `pi-ai` installed the OpenAI, Anthropic, Google, Mistral, and AWS SDKs,
...
so dormancy avoids route registration and module loading but does not avoid installation
...
including `node-domexception`, two `glob` resolutions, and `tsconfck`
...
A source scan of `package.json`, `pnpm-workspace.yaml`, `scripts/`, and `.github/` found no audit,
outdated-package,
...
It uses Dependabot for npm, Python, and GitHub Actions with a configured cooldown
...
2. Decide whether multi-provider support justifies default installation.
   If it does, document the closure honestly.
   If it does not, remove `dsh-llm-pi-ai` from `dsh-base` and make provider installation a real opt-in boundary.
