...
- **Safe-load-only candidate exists:** `fearless_simd/src/generated/simd_types.rs:7990-8001,8081-8083` creates `u8x64<S>` from an exactly 64-byte array for any valid `S: Simd`, using checked transmutation; `src/generated/simd_trait.rs:9652-9654` exposes `load_array_ref`. Native conversions `src/generated/avx512.rs:17687-17700` and `src/generated/neon.rs:7644-7657` also accept any `S: Simd`. The existing table entries are `[u8; 64]` (`package/rust-module/forbidden-regex/src/dfa/sheng.rs:99-117,173-202`, `sheng2.rs:110-138,202-237`). A safe table load with a lower-tier x86 token can feed an `__m512i` inside the existing narrowly guarded intrinsic function; the analogous NEON load feeds `uint8x16x4_t`. This moves the byte-load safety obligation to the audited dependency while retaining current dispatch and state width. It does not show identical emitted loads or eliminate the unsafe x86 call.
