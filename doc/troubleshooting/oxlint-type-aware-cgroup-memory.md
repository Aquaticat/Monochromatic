...
Evidence is retained in the owned verification worktree's `.guard-sandbox/` directories
`lint-default-r2`,
`lint-one`,
`lint-go512`,
`lint-positive512` and `lint-one-recheck`.
The scratch launchers are generated by
`/var/home/user/temp/agent/prepare-root-lint-probe-20260913.mts`.
Kernel and container correlation records are
`root-lint-kernel-memory-20260913.out` and
`root-lint-default-container-events-20260913.jsonl`
under the same scratch root.
