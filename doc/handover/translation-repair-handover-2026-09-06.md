...
- Mio21 read at 20:40 UTC
  ...
  and stopped INCOMPLETE at 3h04m on slice 33,
  the teachers paragraph,
  where the gate kept the ineligible standing 4 of 5 without being told it cannot ship
  (class fifty-six,
  fixed in `139a7f99a`,
  guards red first at `90e5e2a19`;
  the pass log's "XingZ606 read" heading dated 2026-09-18 14:30 UTC;
  6.92 USD).
  `XingZ607` launched at 14:40 UTC on frozen `139a7f99a` (pid 571308,
  ...
  `XingZ611` (frozen `de25b4ed8`) confirmed class sixty live at its admission
  (all 31 tail slices admitted,
  five splits among them)
  and showed class sixty-one:
  the memorial disclosure block's third paragraph,
  absent by a majority,
  refused by an interior budget of zero while the archive's block is one paragraph short of the original's
  ...
  `XingZ612` launched at 05:39 UTC on frozen `75f67fbd4` (pid 895538,
...
05:30 UTC):
class sixty-one,
an absent passage inside a container the archive carries short of blocks is admitted on that deficit
(`insertion-container-deficit.ts`,
wired before the container halves in `pass-insertion-admission.ts`;
guard red first at `1083b9163`;
...
Confirmed live on XingZ611's admission
(all 31 tail slices admitted,
five splits among them);
running on XingZ612.
...
14:38 UTC):
class fifty-six,
the gate sheet says when the standing cannot ship and why,
and every gate ballot is logged with its reason
(`consolidate-gate-wire.ts`,
`consolidate-gate-stage.ts`,
...
the pass log's "Class fifty-six" heading dated 2026-09-18 14:36 UTC;
guards red first at `90e5e2a19`;
found by the XingZ606 INCOMPLETE).
Running on XingZ607.
...
1.  SHI_YUMIAOYA24 RUNS (entry `shi_Yumiaoya`,
    ...
    (贴贴 is on the line too,
    an ordinary word here).
    Also watch the January 16 sentence
    ("ended her life" for 自杀 two sentences before she wakes),
    the second `[^1]` marker on the 药娘 line,
    and the Han residue (补佳乐,
    贴贴,
    「逆子」).
    SHI_YUMIAOYA8 READ WITH CLASS SEVENTY-EIGHT FOUND AND FIXED
    (the pass log's heading dated 2026-09-22 02:20 UTC):
    `shi_Yumiaoya8` (frozen `0f9cd44da`) SETTLED at 02:01 UTC in 28 minutes,
    0.89 USD by the meters,
    one attempt,
    921 seats,
    52 retry rounds;
    every check as the archive
    ...
    no unendorsed or gate-kept standing,
    13 abstention lines),
    but 口香糖 shipped as "Xiaoguantang",
    a pinyin of nothing,
    while the corpus entry `a2581911655` declares "Magic Chewing Gum" and "Chewing Gum".
    Class seventy-eight:
    the sheets' declared names came only from the page's own archive.
    Fixed in `1cdbeea75` and `545bff7b9`
    (`corpus-name-index.ts` reads every entry's front matter at the pin,
    `pass-prepare.ts` joins the names the original carries to the identity context;
    forms two entries share are dropped,
    the heading says an ordinary word stays a word),
    guard red first at `e284a6ebc`,
    full suite green.
    MAIN MERGED at `b05e23022`
    (the mise Rust reinstall fix,
    `AGENTS.md` keeping both branches' rules,
    `mise.toml` and `CLAUDE.md` regenerated by file-enforcer);
    lint,
    types,
    build and the full suite green on the merged tree.
