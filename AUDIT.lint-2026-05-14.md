...
| `no-restricted-syntax(require-destructured-params)` | `function f(a, b) {…}` | refactor to `function f({a, b}) {…}` and update call sites within the same package |
...
| `tsdoc(require-example)` | exported decl missing `@example` | add a minimal but realistic example |
...
| `eslint(no-duplicate-imports)`, `(no-await-in-loop)`, `(no-magic-numbers)`, `(new-cap)`, `(init-declarations)`, `(require-await)`, `(no-alert)`, `(prefer-destructuring)` | misc core eslint rules | apply canonical fix; for `no-magic-numbers` extract named const per CLAUDE.md |
